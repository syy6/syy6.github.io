...
sap.ui.define([
	...
	function(jQuery, library, Control, SimpleForm, Label, Switch, Select, MessageBox, Table, Text, CheckBox, Input, Button,
		ExpressionAdvanced, VerticalLayout, ExpressionType, Constants) {
		"use strict";
		...
		sap.rules.ui.DecisionTableSettings.prototype._createPredefinedResultsTable = function() {
			...
			if (!bResultDataObjectChanged && !bResultDataObjectAttributesChanged) {
				...
				if (bResultDataObjectAttributesChanged) {
					resultObjId = this.getModel("settingsModel").getProperty("/resultDataObjectId");
				}
		...
		sap.rules.ui.DecisionTableSettings.prototype._updateResultAttributeJSON = function(oContext, sRequestType, sAccessMode, sExpression) {
			...
			if (sRequestType === "updatePredefinedResults") {
				...
				if (settingModel.oData.predefinedResults[sAttributeId]) {
					...
					settingModel.oData.predefinedResults[sAttributeId].updatePredefinedResults = true;
				}
		...
		sap.rules.ui.DecisionTableSettings.prototype._getLatestResultDataObjects = function(newResultDataObjectId) {
			...
			var sdataAttributesPath = "/DataObjects(VocabularyId='" + sProjectId + "',Id='" + newResultDataObjectId + "')";
			...
			try {
				vocabularyModel.read(sdataAttributesPath, {
					urlParameters: {
						"$expand": "Attributes"
					},
					success: function (resultDataObjects) {
						settingsModel.updateResultColCells = true;
						that.oColAccessTable.setModel(vocabularyModel);
						/*work around for CSN : 1880262342 - After Refresh of table type DO -sumbitChanges call is never made if there are no attributes
						and no other changes are made in DT Settings*/
						if (resultDataObjects && resultDataObjects.Attributes.results.length === 0) {
							that.attributeListEmpty = true;
						}
						that.oColAccessTable.bindItems({
							path: sdataAttributesPath + "/Attributes",
		...
		sap.rules.ui.DecisionTableSettings.prototype._predefinedResultsFactory = function(sId, oContext) {
			...
			if (!dataObjectChanged && !refreshClicked) {
				...
				if (type === "RESULT" && resultColCount > 0 && this.resultCounter < resultColCount) {
					...
					if (sSelectedAccessMode === this.editableModeText || sSelectedAccessMode === null) {
						sSelectedKey = Constants.KEY_EDITABLE;
					} else {
						sSelectedKey = Constants.KEY_HIDDEN;
					}
			...
			if (_oSettingsModel.getProperty("/refreshButtonClicked")) {
				...
				if (existingAccessMode === this.editableModeText || existingAccessMode === "") {
					existingAccessMode = Constants.KEY_EDITABLE;
				} else {
					existingAccessMode = Constants.KEY_HIDDEN;
				}

				sSelectedKey = existingAccessMode ? existingAccessMode : Constants.KEY_EDITABLE;
		...
		sap.rules.ui.DecisionTableSettings.prototype._setColumnAccessMode = function(oContext, oEvent) {
			...
			if (SelectedMode === Constants.KEY_HIDDEN) {
				...
				var expressionObject = oContext.getObject(oContext.sPath);
				var resultExpression = "";
				if (expressionObject && expressionObject.Result){
					resultExpression = expressionObject.Result["Expression"];
				}
				this._updateResultAttributeJSON(oContext, "updatePredefinedResults", this.editableModeText, resultExpression);
				element._setEditorStyle();
			}
		...
		sap.rules.ui.DecisionTableSettings.prototype._applySettingsModelChangesToOData = function(oDialog) {
			...
			var _updateModelPredefinedResultAttributes = function(sResultObjectID) {
				...
				if (newResultJsonObjects) {
					...
					for (attribute in newResultJsonObjects) {
						if ((attribute.AttributeId != null || attribute.AttributeId !== "") && newResultJsonObjects[attribute].updatePredefinedResults) {
			...
			//validate conditions - Apply only when there are no empty conditions
			var conditionsExist = _validateConditionColumns(aSettingsModelColumns);
			if (conditionsExist) {
				...
				if (!isResultChanged && isResultAttributeParameterChanged) {
					isNeedToSubmitChanges = true;
					_updateModelPredefinedResultAttributes(_settingsModel.oData.ResultDataObjectId);
				} else if (isResultChanged || isResultAttributeChanged) {
					isNeedToSubmitChanges = true;
				...
				/*work around for CSN : 1880262342 - After Refresh of table type DO -sumbitChanges call is never made if there are no attributes
				and no other changes are made in DT Settings*/
				if (oSettingsModel.getProperty("/refreshButtonClicked") && this.attributeListEmpty) {
					oDialog.setState(sap.ui.core.ValueState.Success);
					sap.ui.core.BusyIndicator.hide();
					oDialog.close();
					return;
