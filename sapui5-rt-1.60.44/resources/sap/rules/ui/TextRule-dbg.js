...
sap.ui.define(["jquery.sap.global",
	...
	ExpressionAdvanced, Link, FlexBox, Dialog, TextRuleSettings, AstYamlConverter, Constants) {
	"use strict";
	...
	var TextRule = RuleBase.extend("sap.rules.ui.TextRule", {
		...
		_adjustElseIfTitle: function(oVerticalLayout, nNewIndex, bAdded){
			...
			for ( ; iterator< oContents.length ; iterator++){
				if (oContents[iterator].getHeaderText().indexOf(sElseIf) > -1){
					oContents[iterator].getHeaderToolbar().getContent()[0].setText(sElseIf + " (" + iterator + ")");
				}
			}
		},
		...
		_formElementsFactory: function(sId, oContext) {
			...
			var formElement = new FormElement({
				...
				fields: [this._getExpressionAdvancedText(oContext, expression, businessDataType, sAttributeId)]
					//layoutData: new sap.ui.layout.ResponsiveFlowLayoutData({linebreak: true, margin: false})
			});
			return formElement;
		},
		 ...
		 */
		_getExpressionAdvancedText: function(oContext, expression, businessDataType, attributeId) {
			var that = this;
			var oExpressionLanguage = sap.ui.getCore().byId(this.getExpressionLanguage());
		...
		_getExpressionFromParseResults: function(expression, oResult) {
			if (oResult && oResult.output.decisionTableData.DecisionTable.DecisionTableColumns.results[0].Condition.parserResults && oResult.output.decisionTableData.DecisionTable.DecisionTableColumns.results[0].Condition.parserResults.converted) {
