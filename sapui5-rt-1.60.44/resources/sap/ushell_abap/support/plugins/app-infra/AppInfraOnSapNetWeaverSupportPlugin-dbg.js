...
sap.ui.define([
	...
	function(jQuery, Plugin, Support, Html, VerticalLayout, Text) {
		...
		var AppInfrastructurePlugin = Plugin.extend("sap.ushell_abap.support.plugins.app-infra.AppInfraOnSapNetWeaverSupportPlugin", {
				...
				constructor : function(oSupportStub) {
					...
					this.sModulePath = jQuery.sap.getModulePath( "sap.ushell_abap.support.plugins.app-infra" ); // "../../../../sap/ushell_abap/support/plugins"
					this.oI18n = jQuery.sap.resources({
						url : this.sModulePath + "/i18n/AppInfraOnSapNetWeaverSupportPlugin.properties"
					});
		...
		var sGreenLightIndicator = jQuery.sap.encodeHTML('@08@');
		var sYellowLightIndicator = jQuery.sap.encodeHTML('@09@');
		var sRedLightIndicator = jQuery.sap.encodeHTML('@0A@');
		AppInfrastructurePlugin.prototype.getReportLine = function( sLine, oOptions ) {
			...
			sLine = (sLine == "") ? '&nbsp;' : jQuery.sap.encodeHTML( sLine );
			sLine =
				"<div>" + sPutAtBeginning
				+ sLine
		...
		AppInfrastructurePlugin.prototype.getReportLines = function( aLines, options ) {
			var that = this;
			return aLines.reduce(
				function(sHtml, sLine) {
		...
		AppInfrastructurePlugin.prototype.writeFormatted = function( oFindings ) {
			...
			try {
				...
				var aInfoAndPanels = [], oPanel;

				// Info in from backend
				this.oInfo = new VerticalLayout( "backEndInformation", {content : [
						new Text( {text : oFindings.topic} ),
						new Text( {text : "\n"} ),
						new Text( {text : this.oI18n.getText("SYSTEM_ID") + " : " + oFindings.sapNetWeaverAsAbap.systemId
							+ " ( " +  oFindings.sapNetWeaverAsAbap.systemText + " )" } ),
						new Text( {text : this.oI18n.getText("CLIENT") + " : " + oFindings.sapNetWeaverAsAbap.client
						...
						new Text( {text : "\n"} ),
						new Text( {text : this.oI18n.getText("SAP_UI_SOFTWARE_COMPONENT") + " : " + oFindings.sapUi.softwareComponent
							+ " ( " + oFindings.sapUi.softwareComponentDescription + " )" } ),
						new Text( {text : this.oI18n.getText("VERSION") + " : " + oFindings.sapUi.version } ),
						new Text( {text : this.oI18n.getText("SUPPORT_PACKAGE_LEVEL") + " : "
							+ oFindings.sapUi.supportPackageLevel } ),
						new Text( {text : "\n"} )
				...
				oFindings.checks.forEach( function(oCheck) {
					...
					oPanel = new sap.m.Panel( { expandable: true, expanded: false, width:"auto",
						headerToolbar:
							new sap.m.Toolbar({
								content: [
									new sap.m.Image( { src : trafficLightUrl( oCheck ) } ),
									new sap.m.Title({ level : "H3", text : oCheck.outcome.message})
								]
							})
						}
					)
				...
				return new Text( { text :
					this.oI18n.getText("ERROR_RESPONSE_FROM_BACKEND_NOT_PROPERLY_FORMATTED") }
				);
			}
		}
		...
		AppInfrastructurePlugin.prototype.init = function(oSupportStub) {
			...
			if (this.runsAsToolPlugin()) {
				...
				oRenderManager.write('<div id="' + this.getId() + '-Area" class="sapUiSizeCompact"></div>').flush(this.$().get(0));
				...
				var oRenderingDone =
					this.check().done(
						...
						function( jqxhr, textStatus, error ) {
							new Text( "noBackEndResult",
								{text : that.oI18n.getText("ERROR_UNABLE_TO_RETRIEVE_INFORMATION_FROM_BACKEND") }
							)
