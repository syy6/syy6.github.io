...
sap.ui.define([
...
], function (URI, chipinstance, catalog, chipdefinition) {
    "use strict";
    ...
    var LaunchPageAdapter = function (oUnused, sParameter, oAdapterConfiguration) {
         ...
         *
         * @param {object} oLayout
         *  Layout of the page. Looks like this:
         *  <code>
         *      {
         ...
         * @param {string} sCurrentTileType
         *  "link" or "tile"
         *
         * @returns {integer}
         *  the index of the oTile if found. Otherwise -1 is returned
         *
         * @private
         */
         ...
         *
         * @param {array} aArray
         *  The array from which the value should be removed from.
         * @param {any} vValue
         *  The value to be removed.
         *
         * @returns {integer}
         *  The index of the vValue in the array or -1 if it was not found.
         *
         * @private
         */
        ...
        /**
         * Adds the given tile ID to the given Page layout in the specified index and according to
         * it's current type.
         *
         * @param {object} oLayout
         *  Layout of the page. Looks like this:
         ...
         *  The ID of the tile to be added
         * @param {integer} iIndex
         *  The index of the tile or link to be added
         * @param {string} sNewTileType
         *  "link" or "tile"; specifies if the tile is added to the order or linkOrder.
         *
         * @private
         */
        function addToLayout (oLayout, sTileId, iIndex, sNewTileType) {
            if (sNewTileType === "link") {
                // iIndex specifies index in oLayout.order.concat(oLayout.linkOrder) as Links
                // are returned by getGroupTiles as well (at the end of the array).
         ...
         *  The Page the layout is calculated for.
         * @param {sap.ushell_abap.adapters.abap.LaunchPageAdapter} oAdapter
         *  The instance of the LaunchPageAdapter (this).
         * @returns {object}
         *  The calculated Page layout
         *  <code>
         *      {
         *          order: ["tileId1", "tileId2"],
         *          linkOrder: ["linkTileId1", "linkTileId2"]
         ...
         *
         * @param {sap.ui2.srvc.ChipInstance} oChipInstance
         *   must not be a stub anymore. Also it's CHIP must not be a stub anymore.
         *
         * @returns {object}
        ...
        function getAppLauncherTileConfiguration(oChipInstance) {
         ...
         /**
         * Returns the tile configuration of the given (Smart Business) CHIP
         * instance. It logs an error message if the tile configuration cannot
         * be parsed.
         *
         * @param {sap.ui2.srvc.ChipInstance} oChipInstance
         ...
         *
         * @returns {object}
         *   the tile configuration in style of an applauncer
         */
        function getSmartBusinessTileConfiguration(oChipInstance) {
            ...
            try {
                ...
                // _getChipRawConfigurationString is used to avoid that the getChip().load() must be called
                // which will load the CHIP definition XML, which would end up in additional requests
                // in case this method is called during critical FLP start-up time.
                // Drawback: Configuration defaults (which are usually not used in Fiori) from CHIP
                // definition XML are ignored here.
        ...
        /**
         * Returns the tile configuration of the given custom CHIP
         * instance, but only if it uses the same configuration structure
         * as the standard static and dynamic app launcher or if it uses the
         * Smart Business tiles' configuration structure.
         *
         * @param {sap.ui2.srvc.ChipInstance} oChipInstance
         *   It's CHIP may still be a stub.
         *
         * @returns {object}
         *   the tile configuration or an empty object
         */
         ...
         *  ChipInstance to check for personalization support
         * @returns {boolean}
         *  If no chip instance was provided and link personalization is supported in general, the return value is true.
        ...
        this._checkTileIntentSupport = function (oTile, oTargetMappingSupport) {
            ...
            if (!isAppLauncher(oTile)) {
                ...
                // Try to read the configuration string from the CHIP, if the CHIP sticks to the same configuration
                // structure as the SAP app launcher tiles, it may be filtered in case it launches a intent which is
                // not assigned.
                oTileConfiguration = getCustomTileConfiguration(oTile);
                ...
                /* Generic Smart Business target mappings must also be supported. Generic target mappings are indicated
                   via a leading asterisk like *-analyzeSBKPIDetailsS4HANA and means that all semantic objects match,
                   and only the semantic action counts.
                   See BCP 1980297194. */
            ...
            if (jQuery.isEmptyObject(oTileConfiguration)) {
                // it seems as if there was an error in getAppLauncherTileConfiguration
         ...
         *     the index in the target group, in case this parameter is not supplied we assume the
         ...
         * @param {string} [sNewTileType]
         *    (added with 1.60) The new type of the tile
         * @returns {object}
         *     a jQuery.Promise
         * @since 1.11.0
         */
         ...
         *
         * @param {sap.ui2.srvc.ChipInstance} oTile
         *  The tile's chip data
         * @returns {object}
         *  jQuery.Promise object that is resolved to an instance of sap.m.GenericTile
         * @since 1.11.0
         */
        this.getTileView = function (oTile) {
            ...
            function doResolve () {
                ...
                if (sTileType === "link") {
                    ...
                    var oLinkTile = new sap.m.GenericTile({
                        mode: "{view>/mode}",
        ...
        function isBookmarkFor(oChipInstance, sUrl) {
            return isAppLauncher(oChipInstance)
                && getAppLauncherTileConfiguration(oChipInstance).navigation_target_url === sUrl;
        }
