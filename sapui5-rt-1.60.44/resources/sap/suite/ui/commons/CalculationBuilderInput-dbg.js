sap.ui.define([
	...
	"sap/ui/Device"
			 ...
			 ToolbarSpacer, Title, OverflowToolbarToggleButton, CustomData, Sorter, JSONModel, Device) {
	"use strict";
	...
	CalculationBuilderInput.prototype._showErrorText = function (bShow, aErrors) {
		...
		aErrors = aErrors || this.getParent().getErrors();

		if ((aErrors && aErrors.length > 0) && bShow) {
			var sTitle = "",
				TOP_ERROR_COUNT = 5;

			for (var i = 0; i < aErrors.length && i < TOP_ERROR_COUNT; i++) {
	...
	CalculationBuilderInput.prototype._fillInputToolbar = function () {
		...
		var fnCreateOperatorsMenu = function () {
			var oParent = this.getParent(),
				...
				aComparisonOperators, aLogicalOperators,
				aMenu = [];
			...
			if (bHasOperators) {
				aLogicalOperators = fnCreateCustomOperatorsMenuItems(aOperators);
				aMenu.push(new MenuItem({
					text: oResourceBundle.getText("CALCULATION_BUILDER_ADDITIONAL_OPERATOR"),
					items: aLogicalOperators
				}));
			}
