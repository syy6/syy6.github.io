sap.ui.define([
	...
	"sap/ui/thirdparty/jqueryui/jquery-ui-selectable"
			 ...
			 Label, Panel, ResponsivePopover, PlacementType, Toolbar, MessageStrip, ValidationResult, ControlProxy) {
	"use strict";
	...
	CalculationBuilderExpression.prototype._createPopoverOperatorsItems = function (oPopoverItems) {
		...
		var fnGetPanel = function (sTitle, oOperatorTypes, sType) {
			return new Panel({
				content: [
					...
					fnGetOperatorsButtons(oOperatorTypes, sType)
				]
			});
		};
		...
		if (this.getParent().getAllowComparisonOperators()) {
			oPopoverItems.operatorsItems.push(fnGetPanel(oResourceBundle.getText("CALCULATION_BUILDER_COMPARISON_TITLE_SELECT"), ComparisonOperatorType, ItemType.Operator));
		}

		if (this.getParent().getAllowLogicalOperators()) {
			oPopoverItems.operatorsItems.push(fnGetPanel(oResourceBundle.getText("CALCULATION_BUILDER_LOGICAL_TITLE_SELECT"), LogicalOperatorType, ItemType.Operator));
		}
		...
		if (sPageId === (this.getId() + Ids.PAGE_CONSTANT)) {
			...
			fnConfirm = function () {
				if (oPopoverItems.constantInput.getValue() !== "") {
					...
					if (this.getParent() && this.getParent().getAllowStringConstants() && isNaN(sValue)) {
						sValue = "\"" + sValue + "\"";
					}
