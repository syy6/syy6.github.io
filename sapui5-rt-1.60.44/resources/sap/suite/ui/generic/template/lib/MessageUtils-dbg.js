...
sap.ui.define(["sap/ui/generic/app/util/ModelUtil", "sap/ui/generic/app/util/MessageUtil", "sap/m/MessageBox"
	...
	function(ModelUtil, GenericMessageUtil, MessageBox) {
		...
		function fnNavigateToMessageTarget(oCommonUtils, oScrollDelegate, oMessage) {
			var sId = oCommonUtils.getPositionableControlId(oMessage.controlIds);
			oCommonUtils.focusControl(oScrollDelegate, sId);
		}
		
		function fnNavigateFromMessageTitleEvent(oCommonUtils, oScrollDelegate, oEvent) {
			var oMessageItem = oEvent.getParameter("item");
			var oMessage = oMessageItem.getBindingContext("msg").getObject();
			fnNavigateToMessageTarget(oCommonUtils, oScrollDelegate, oMessage);
		}
