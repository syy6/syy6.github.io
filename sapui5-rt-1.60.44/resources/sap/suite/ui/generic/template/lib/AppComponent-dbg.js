...
sap.ui.define([
	...
	Application, BusyHelper, NavigationController,  ProcessObserver, TemplateAssembler, CRUDHelper, ViewDependencyHelper, testableHelper, CommonMethods) {
	"use strict";
	...
	function getMethods(oAppComponent, oAppId) {
		...
		var	oTemplateContract = { // template contract object which is used for data interchange between framework classes
			...
			getText: getText,
			mRouteToTemplateComponentPromise: Object.create(null),	// maps the name of a route onto a Promise that res
		...
		function retemplateActiveComponents(){
					...
					oTemplateContract.oNavigationControllerProxy.getActiveComponents().forEach(function(sComponent){
						...
						aComponentRetemplatingPromises.push( 
								oNewContainer.loaded().then(function(oNewContainer){
									...
									oNewComponentRegistryEntry.activationTakt = oOldComponentRegistryEntry.activationTakt;
									 ...
									 */
									if (oBindingContext){
										oNewComponentRegistryEntry.viewRegistered.then(function(){
			...
			oRetemplatingPromise = oNewRetemplatingPromise.then(jQuery.noop); 
		...
		if (sap.ui.getCore().getConfiguration().getDesignMode()){
			jQuery.extend(oMethods, oDesignTimeMethods);
		}
		return oMethods;
	}
