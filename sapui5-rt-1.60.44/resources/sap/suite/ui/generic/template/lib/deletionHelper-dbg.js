...
sap.ui.define([], function() {
	...
	function fnNavigateToPosition(oTemplateContract, iKeepPosition){
		if (iKeepPosition === 0){
			oTemplateContract.oNavigationControllerProxy.navigateToRoot(true);	
		} else { // assumption: navigation 1 up
			var oRegistryEntry = getRegistryEntryIfActiveForLevel(oTemplateContract, iKeepPosition + 1);
			if (oRegistryEntry && oRegistryEntry.methods.navigateUp){
				oRegistryEntry.methods.navigateUp();	
			} else {
				oTemplateContract.oNavigationControllerProxy.navigateToRoot(true);	// fallback, probably not necessary
			}
		}
	}
	...
	function fnPrepareDeletionOfCreateDraft(oTemplateContract, oPromise){
	...
	// mObjectsToDelete maps the pathes of the objects that are planned to be deleted onto Promises that are resolved, resp. rejected when the deletion was successfull resp. failed.
	function fnPrepareDeletion(oTemplateContract, mObjectsToDelete){
		...
		var sComponentId;
		var oRegistryEntry;
		//n prepare to unbind components (and their children) that are bound to objects that are going to be deleted
		for (sComponentId in oTemplateContract.componentRegistry){
			...
			sPath = oElementBinding && oElementBinding.getPath();
		...
		for (sPath in mObjectsToDelete){
			...
			var oDonePromise = oDeletePromise.then(fnHandle.bind(null, iPosition, sEntitySet), fnHandle.bind(null, iPosition));
		...
		Promise.all(aDonePromises).then(function(aResults){
			...
			for (sComponentId in oTemplateContract.componentRegistry){
				...
				jQuery.extend(mRefreshInfos, mAffectedEntitySets);
	...
	return {
		...
		prepareDeletionOfCreateDraft: fnPrepareDeletionOfCreateDraft
	};
});