sap.ui.define(["jquery.sap.global", "sap/ui/base/Object"], function(jQuery, BaseObject) {
	...
	function getMethods(oComponent, oComponentRegistryEntry) {
		...
		function fnDataRequested(oEvent) {
			jQuery.sap.log.info("Request header data", oEvent.getSource().getPath(), "Class sap.suite.ui.generic.template.lib.ComponentUtils");
			bIsDataLoading = true;
			fnNewDataAvailablePromise();
		...
		function fnDataReceived(oEvent){
			jQuery.sap.log.info("Received header data", oEvent.getSource().getPath(), "Class sap.suite.ui.generic.template.lib.ComponentUtils");
			fnEndBusy();
		...
		function fnRefreshBinding(bUnconditional) {
			...
			if (bUnconditional || !jQuery.isEmptyObject(oComponentRegistryEntry.mRefreshInfos)) {
				(oComponentRegistryEntry.methods.refreshBinding || jQuery.noop)(bUnconditional, oComponentRegistryEntry.mRefreshInfos);
			...
			oViewProxy.oStatePreserverPromise = oComponentRegistryEntry.reuseComponentsReady.then(function(mReuseComponentProxies){
				var oSettings = {
					...
					applyState: function(oState, bIsSameAsLast){
						...
						oComponentRegistryEntry.oViewRenderedPromise.then(fnExecuteForAllReuseComponents.bind(null, mReuseComponentProxies, fnApplyReuseInfo));
					},
