...
sap.ui.define(["sap/f/FlexibleColumnLayout", "sap/ui/model/Filter", "sap/ui/model/FilterOperator", "sap/ui/model/json/JSONModel",
	...
	function(FlexibleColumnLayout, Filter, FilterOperator, JSONModel, Targets, ModelUtil, CRUDHelper, FlexibleColumnLayoutHandler,
		...
		function generateRoutingMetadataAndGetRootEntitySet(oNavigationControllerProxy) {
			...
			createChildRoutes("root", oTopPage, 0, null, oNavigationControllerProxy, sTargetControlId, oTopTreeNode.children);
		...
		function fnHandleEmbeddedComponent(oTreeNode, iLevel, oParent, oNavigationControllerProxy, sTargetControlId,
			sEmbeddedComponentKey, oEmbeddedComponent, sContainerId, sSectionId) {
			...
			if (oEmbeddedComponent.pages) {
				fnHandleEmbeddedComponentPages(sEmbeddedComponentKey, oEmbeddedComponent, iLevel, oParent, oNavigationControllerProxy, sTargetControlId, oEmbeddedComponentInfo.children, oCommunicationObject);
			}
		}

		function fnHandleEmbeddedComponents(oTreeNode, oRoute, iLevel, oParent, oNavigationControllerProxy, sTargetControlId) {
			oTreeNode.embeddedComponents = Object.create(null);
			if (oRoute.implementingComponent) {
				fnHandleEmbeddedComponent(oTreeNode, iLevel, oParent, oNavigationControllerProxy, sTargetControlId, "implementation", oRoute.implementingComponent, "template::ImplementingComponent");
				...
				for (var sEmbeddedComponentKey in oRoute.embeddedComponents) {
					...
					fnHandleEmbeddedComponent(oTreeNode, iLevel, oParent, oNavigationControllerProxy, sTargetControlId, sEmbeddedComponentKey,
						oEmbeddedComponent, AnnotationHelperReuseComponents.formatIdComponentContainer(oEmbeddedComponent), AnnotationHelperReuseComponents.formatIdComponentSection(oEmbeddedComponent));
				}
			}
		}
		...
		function createRoutes(vPredecessorTargets, oPage, iLevel, oParent, oNavigationControllerProxy, sTargetControlId, aNodes,
			...
			if (oPage.component) { //in case of intent there is no internal route to be created
				...
				oTreeNode.pattern = oNewRoute.pattern;
				if (aNodes) {
				...
				createChildRoutes(oNewRoute.target, oPage, iLevel, oNewRoute, oNavigationControllerProxy, sTargetControlId, oTreeNode.children, oCommunicationObject);
				fnHandleEmbeddedComponents(oTreeNode, oPage, iLevel, oNewRoute, oNavigationControllerProxy, sTargetControlId);
			}
		}
		...
		function fnResolveParameterizedEntitySet(oDataModel, oEntitySet, oStartupParameters) {
			...
			try {
				...
				if (parameterization) {
					...
					jQuery.each(aParametricSet, function() {
