sap.ui.define(["jquery.sap.global", "sap/ui/core/CustomizingConfiguration", "sap/ui/core/mvc/ViewType", "sap/ui/core/Component", "sap/ui/core/UIComponent", "sap/ui/model/json/JSONModel", "sap/ui/model/resource/ResourceModel", "sap/ui/Device", "sap/suite/ui/generic/template/lib/reuseComponentHelper", "sap/ui/core/CustomData", "sap/suite/ui/generic/template/lib/StableIdDefinition", "sap/suite/ui/generic/template/js/StableIdHelper"],
	function(jQuery, CustomizingConfiguration, ViewType, Component, UIComponent, JSONModel, ResourceModel, Device, reuseComponentHelper, CustomData, StableIdDefinition, StableIdHelper) {
		"use strict";
		...
		function createXMLView(oComponentRegistryEntry, aCacheKeys) {
			...
			oComponent.runAsOwner(function() {
				...
				if (oComponent.getDesigntimePath && oComponent.getFlexibilityPath){
					...
					oViewSettings.customData =  [new CustomData({
		...
		function getExtensionAPIEmbeddedComponent(oComponentRegistryEntry, sEmbeddedKey, oEmbeddedComponentMeta){
			...
			(oComponentRegistryEntry.methods.enhanceExtensionAPI4Reuse || jQuery.noop)(oRet, oEmbeddedComponentMeta);
			return oRet;
		}
		...
		function fnPlaceEmbeddedComponent(oComponentRegistryEntry, mReuseComponentProxies, sEmbeddedKey, oEmbeddedComponentMeta, oEmbeddedComponent){
			...
			oEmbeddedComponent.applySettings(oContainer.getSettings());
				...
				if (oComponentRegistryEntry.viewLevel > 0){ // The root component does not contain any embedded components
					...
					if (oTreeNode){
						var fnLoadError = function(sEmbeddedKey, oError){
							jQuery.sap.log.error("Failed to load reuse component for key " + sEmbeddedKey, oError instanceof Error ? oError.message : "", "sap.suite.ui.generic.template.lib.TemplateComponent");	
						};
