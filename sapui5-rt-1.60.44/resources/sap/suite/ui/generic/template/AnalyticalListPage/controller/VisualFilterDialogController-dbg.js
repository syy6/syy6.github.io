sap.ui.define([
		...
		"sap/m/Link", "sap/m/List", "sap/m/CustomListItem", "sap/m/TextArea","sap/m/Text", "sap/m/StandardListItem", "sap/m/ListSeparators", "sap/m/Popover",
		...
		"sap/ui/model/FilterOperator", "sap/m/DatePicker", "sap/base/Log"

	], function(Button, ButtonType, Label, Dialog, Bar, SearchField, Toolbar, ToolbarSpacer, Title,
			VBox, HBox, CheckBox, Link, List, CustomListItem, TextArea, Text, StandardListItem, ListSeparators, Popover, SimpleForm,
			...
			OverflowToolbar, OverflowToolbarLayoutData, CustomData, Device, SapMLibrary, SapCoreLibrary, FilterOperator, DatePicker, Log) {
	"use strict";
	...
	var vfdController = Controller.extend("sap.suite.ui.generic.template.AnalyticalListPage.controller.VisualFilterDialogController", {
		...
		*/
		_updateFilterBarFromDialog: function() {
			var	oFilterBarFilterModel = this.oState.alr_visualFilterBar.getModel('_filter'),
				...
				isFiltersSame = jQuery.sap.equal(oFilterBarFilterModel.getJSON(), oDialogFilterModel.getJSON());
			if (!isFiltersSame) {
			...
			var oVisualFilterConfigModel = this.oState.alr_visualFilterBar.getModel('_visualFilterConfigModel'),
				...
				isConfigSame = jQuery.sap.equal(oVisualFilterConfigModel.getJSON(), oDialogConfigModel.getJSON());
			if (!isConfigSame) {
			...
			if (oVariantConfig) {
				...
				this.oConfig.filterCompList.forEach(function (element) {
					...
					if (oVariantConfig[element.component.properties.parentProperty]) {
						jQuery.extend(true, element, oVariantConfig[element.component.properties.parentProperty]);
					}
				});
			} else {
			...
			for (i = 0; i < this.filterCompList.length; i++) {
				if (!Array.isArray(this.filterCompList[i])) {
					...
					if (!(groupList.indexOf(this.filterCompList[i].obj.group.name) > -1)) {
						...
						groupContainer = new List({showSeparators: "None"});
					...
					if (this.filterCompList[i].obj.shownInFilterDialog) {
						...
						groupContainer.addItem(new CustomListItem({content: oChartBox}));
					}
				}
				//add to dialog
				if (groupContainer) {
		...
		*/
		selectCheckBox : function (idx, bVisible) {
			...
			var oVisualFilterDialogModelClone = jQuery.extend(true, {}, oVisualFilterDialogModel);
		...
		_addChartCustomToolbar: function(obj, idx) {
			...
			dimLabel = FilterUtil.getPropertyNameDisplay(this.oState.alr_visualFilterBar.getModel(), obj.component.properties.entitySet, obj.component.properties.dimensionField),
			...
			var bIsValuehelp = oInput.getShowValueHelp && oInput.getShowValueHelp(),
					...
					new Button({
						...
						visible: {
							...
							formatter: function(oContext) {
								if (bIsValuehelp || props.isDropDown || bIsDatePicker) { //for valuehelp,Drop Down & DatePicker
									return true;
		...
		_showChartTypesPopup: function(oEvent) {
			...
			oList.attachSelectionChange(function (oEvent) {
				...
				oModelDataItem = jQuery.extend(true, {}, oModelData[idx]),
