...
sap.ui.define([
	...
	*/
	...
	/**
	*Function to check if dimension is date is Edm.String and sap:semantics="yearmonthday"
	*params {string} Entity set of the property
	*params {string} dimension in the entity set
	*return {boolean} returns true if date is Edm.String and sap:semantics="yearmonthday"
	*/
	VisualFilterProvider.prototype._isDimensionDateFilterStringType = function(sEntitySet, sDimension) {
		...
		sFieldSemantics = oDimensionFieldInfo && oDimensionFieldInfo[V4Terms.CalendarYearMonthDay],
		bDimensionIsDateStringType = false;

		if (oDimensionFieldInfo && oDimensionFieldInfo.type === "Edm.String") {
			if (sFieldSemantics && sFieldSemantics.Bool && sFieldSemantics.Bool === "true") {
				bDimensionIsDateStringType = true;
			}
		}
		return bDimensionIsDateStringType;
	};
	/*
	...
	*/
	VisualFilterProvider.prototype._createConsumeableObjectFromAnnotation = function (presentationVariantAnnotation, collectionPath, isSelected, parameters, parentProperty, filterRestriction, isDropDown, bIsMandatoryProp, displayBehaviour, entityType, selectionVariant) {
		...
		bDimensionIsDateTime, bDimensionIsDateStringType;
