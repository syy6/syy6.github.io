sap.ui.define([
	...
	KpiProvider.prototype._getDataFromAnnotations = function(oConfig) {
		...
		if (!oEntitySet) {
			jQuery.sap.log.error("KPI error details - Incorrect entity set");
			return;
		}
		...
		if (oKpiAnnotation) {
			...
			if (!oSelectionPresentationVariant) {
				jQuery.sap.log.error("KPI error details - SelectionPresentationVariant does not have Path.");
				return;
			}
			...
			if (!sPresentationVariantPath) {
				jQuery.sap.log.error("KPI error details - PresentationVariant does not have Path.");
				return;
			}
		...
		if (!sDataPointPath) {
			jQuery.sap.log.error("KPI error details - DataPoint does not have Path.");
			return;
		}
		//DATAPOINT
		oDataPoint = oEntityType[sDataPointPath];
		if (!sSelectionVariantPath) {
			jQuery.sap.log.error("KPI error details - SelectionVariant does not have a path");
		}
