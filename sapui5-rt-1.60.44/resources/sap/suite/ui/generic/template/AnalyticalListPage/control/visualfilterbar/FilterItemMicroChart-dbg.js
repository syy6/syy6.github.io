...
function(FilterItem, Sorter, oData4Analytics, FilterUtil, NumberFormat, v4Terms, DateFormat) {
	"use strict";
	...
	var FilterItemChart = FilterItem.extend("sap.suite.ui.generic.template.AnalyticalListPage.control.visualfilterbar.FilterItemMicroChart", {
		...
		renderer: function(oRm, oControl) {
			...
			jQuery(document.body).hasClass("sapUiSizeCozy") ? oRm.addStyle("height", "9.9rem") : oRm.addStyle("height", "7.9rem");// Increasing the height of the visual filter chart to make it interactive in cozy formfactor
			oRm.writeStyles();
			oRm.write(">");
	...
	FilterItemChart.prototype._getCustomData = function(ev) {
		...
		if (sCurrentSelectedChartType && aCurrentItem) {
			...
			if (oValue instanceof Date) {
				// convert local time to UTC(onDataRecieved gives in local date format)
				oValue = FilterUtil.convertLocalDatetoUTCDate(oValue);
			}
			var data = {
				dimValue: oValue,
	...
	*/
	...
	FilterItemChart.prototype.getTitle = function() {
		...
		var measureLabel = FilterUtil.getPropertyNameDisplay(model, this.getEntitySet(), this.getMeasureField());
		var dimLabel = FilterUtil.getPropertyNameDisplay(model, this.getEntitySet(), this.getDimensionField());
	...
	FilterItemChart.prototype._getSelected =  function(oContext, sDimFieldValue) {
		...
		if (oContext) {
			if (sFilterRestriction === 'multiple') {
					...
					for (var i = 0; i < oContext.ranges.length; i++) {
						...
						if (oRange.operation ===  "EQ" && oRange.value1 && !oRange.exclude) {
							if (oRange.value1 instanceof Date && sDimFieldValue instanceof Date){
								...
								bIsSelected = true;
								break;
							} else if (this._isDateTimeFieldSelected(oRange.value1, sDimFieldValue)) {
								bIsSelected = true;
								break;
							}
				...
				if (FilterUtil.getDateInMedium(oContext) === FilterUtil.getDateInMedium(sDimFieldValue)) {
					bIsSelected = true;
				}
	...
	FilterItemChart.prototype._getLabel = function(oDimFieldDisplay, sDimField) {
		if (this.getDimensionFieldIsDateTime()) {
			...
			} else if (this._isStringDateType()) {
			...
			oSettings = {
				label : {
					...
					formatter: function(oDimFieldDisplay, sDimField) {
						return me._getLabel(oDimFieldDisplay, sDimField);
					}
				},
				value: {
					path: sMeasureField,
				...
				tooltip: {
					...
					formatter: function(dimLabel, dimValue, sDimField, unitField) {
						...
						dimLabel = me._getLabel(dimLabel, sDimField);
				...
				selected: {
					...
					formatter: function(oContext, sDimFieldValue) {
						if (sDimFieldValue instanceof Date) {
							sDimFieldValue = FilterUtil.convertLocalDatetoUTCDate(sDimFieldValue);
						}
	...
	**/
	FilterItemChart.prototype.resolveParameterizedEntitySet = function(oDataModel, oEntitySet, aParams, oSmartFilterBar) {
		var path = "";
		...
		if (parameterization) {
			...
			if (aParams) { //values for parameters set in the SV
				aParams.forEach(function(oParam) {
					var paramName = oParam.PropertyName.PropertyPath;
					var paramValue = oParam.PropertyValue.String;
					if (paramName && paramValue) {
						queryResultRequest.getParameterizationRequest().setParameterValue(paramName,paramValue);
					}
				});
			}
			if (oSmartFilterBar.getEntitySet() === oEntitySet) {
				var allParams = oSmartFilterBar.getAnalyticalParameters();
				...
				allParams.forEach(function(param) {
					...
					if (sValue) {
						...
						queryResultRequest.getParameterizationRequest().setParameterValue(param.name, sValue);
					}
				}.bind(this));
			}
		}
	...
	**/
	FilterItemChart.prototype.checkSearchable = function(oDataModel, oEntitySet, oSmartFilterBar, aParameters, aSVOptions) {
		var o4a = new oData4Analytics.Model(oData4Analytics.Model.ReferenceByModel(oDataModel));
		var queryResult = o4a.findQueryResultByName(oEntitySet);
		var parameterization = queryResult.getParameterization(), allParamsList = [], bSearchable = true;
		//if the collection path of the VF is a parameterized entity set
		if (parameterization) {
			//all parameters required to obtain the result entity set
			var allMandatoryParameters = parameterization.getAllParameterNames();
		...
		if (oSmartFilterBar.getEntitySet() === oEntitySet) {
			...
			allMandatoryFilters.forEach(function(mFilter) { //mandatory filters can be set via SV
				aSVOptions && aSVOptions.forEach(function(fil) {
					if (fil.PropertyName.PropertyPath === mFilter.getName()) {
						allParamsList.push(fil.PropertyName.PropertyPath);
					}
				});
				oSmartFilterBar.getFiltersWithValues().forEach(function(param) { //mandatory filters can be set in SFB
					if (param.getName() === mFilter.getName() && !allParamsList[param.getName()]) {
						allParamsList.push(param.getName());
					}
				});
			});
			//searchable only if all mandatory filter values are set
			if (allParamsList.length !== allMandatoryFilters.length) {
				bSearchable = false;
				return bSearchable;
			}
		}
		//value returned is after the consideration of both mandatory parameters and filters
		return bSearchable;
	};
	/**
	* This function resolves the searchable state of the visual filter bar. If it is searchable, the binding path is generated.If it is not searchable, an overlay is applied.
	*@param {object} oDataModel model
	*@param {string} oEntitySet entity set name
	*@param {object} oSmartFilterBar smartFilterBar object
	*@param {array} aParameters parameters from SV
	...
	*@returns {boolean} bSearchable true if VFB is searchable, else false
	**/
	FilterItemChart.prototype._getPathForVisualFilter = function(oModel, entityName) {
		var sPath;
		if (this.getSmartFilterId()) {//If it has reference to SmartFilterBar
			...
			var aSVParameters = this.getSelectFilters() && this.getSelectFilters().Parameters;
			var aSVSelectOptions = this.getSelectFilters() && this.getSelectFilters().SelectOptions;
			var bSearchable = this.checkSearchable(oModel, entityName, oSmartFilterBar, aSVParameters, aSVSelectOptions);
			if (bSearchable) {
				sPath = this.resolveParameterizedEntitySet(oModel, entityName, aSVParameters, oSmartFilterBar);
			} else {
	...
	FilterItemChart.prototype._determineHiddenVisualFilter = function(oMetaModel, entitySet, measureField) {
		...
		if (prop[v4Terms.Hidden] && prop[v4Terms.Hidden].Bool === "true") {
			return true;
		}
	};
