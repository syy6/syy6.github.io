sap.ui.define([
	"sap/m/HeaderContainer", "sap/suite/ui/generic/template/AnalyticalListPage/controller/VisualFilterDialogController",
	...
	"sap/ui/model/json/JSONModel", "sap/ui/core/CustomData", "sap/m/Text",
	"sap/m/OverflowToolbarLayoutData", "sap/ui/model/FilterOperator",
	"sap/ui/Device", "sap/ui/model/SimpleType", "sap/ui/core/library", "sap/m/library", "sap/m/DatePicker"
		...
		CustomData, Text, OverflowToolbarLayoutData, FilterOperator, Device, SimpleType, SapCoreLibrary, SapMLibrary, DatePicker) {
	"use strict";
	...
	SmartVisualFilterBar.prototype.init = function() {
		...
		var bIsCozyMode = jQuery(document.body).hasClass("sapUiSizeCozy");
		// Default settings
		//TODO: Get this through CSS rather than hard coding
		this._cellItemHeightNorth = "2.5rem";
		this._cellItemHeightSouth = bIsCozyMode ? "9.9rem" : "7.9rem";
		this._cellHeight = bIsCozyMode ? "12rem" : "10.9rem";
	...
	*/
	...
	SmartVisualFilterBar.prototype._createHeaderItems = function (sPath, sType, oProperties) {
		...
		aSVOptions = oProperties.selectFilters && oProperties.selectFilters.SelectOptions,
	...
	*/
	...
	SmartVisualFilterBar.prototype._getFiltersForFilterItem = function(inParams, parentProperty,oFilter, currencyField, selectOptions, inParameterFilterList) {
		...
		var fnSVFilterList = function(oOption, sParentProperty) {
			...
			oOption.Ranges.forEach(function(oRange) {
				...
				var oSelectFilter = me._getSelectOptionFilters(sParentProperty, sLow, sHigh, sOption, sSign);
					...
					for (var i = 0; i < selectOptions.length; i++) {
						...
						if (mappedSelectionVariant.indexOf(sProperty) === -1) {
							...
							if (sProperty === parentProperty) {
								fnSVFilterList(oOption, sProperty);
	...
	*/
	...
	SmartVisualFilterBar.prototype._createTitleToolbar = function(props, filterItem) {
		var titleObject = filterItem.getTitle();
		...
		if (oInput) {
			var dimLabel = FilterUtil.getPropertyNameDisplay(this.getModel(), filterItem.getEntitySet(), filterItem.getDimensionField()),
			rb = this.getModel("i18n").getResourceBundle(),
			bIsValuehelp = oInput.getShowValueHelp && oInput.getShowValueHelp(),
			bIsDatePicker = oInput instanceof DatePicker,
			sShowDatePicker = bIsDatePicker ? "sap-icon://appointment-2" : "",
			...
			selectedBtn = new Button({
				...
				visible: {
					path: "_filter>/" + filterItem.getParentProperty(),
					formatter: function(oContext) {
						if (bIsValuehelp || props.isDropDown || bIsDatePicker) { //for valuehelp & Drop Down
							return true;
	...
	SmartVisualFilterBar.prototype.getTitleByFilterItemConfig = function(filterConfig, unitValue, scaleValue) { // used when the filter item + data is not present, ideally called on the filter item iteslf
		...
		var measureLabel = FilterUtil.getPropertyNameDisplay(model, entitySet, props.measureField);
		var dimLabel = FilterUtil.getPropertyNameDisplay(model, entitySet, props.dimensionField);
	...
	SmartVisualFilterBar.prototype._onTitleChange = function(ev) {
		...
		var oTitleObj = ev.getSource().getTitle();
	...
	SmartVisualFilterBar.prototype._createFilterItemOfType = function(type, properties) {
		...
		if (type === CHART_TYPE_BAR) {
			compInst  = new FilterItemMicroBar(properties);
		} else if (type === CHART_TYPE_LINE) {
			compInst = new FilterItemMicroLine(properties);  
		} else if (type === CHART_TYPE_DONUT) {
			compInst = new FilterItemMicroDonut(properties);
		}
