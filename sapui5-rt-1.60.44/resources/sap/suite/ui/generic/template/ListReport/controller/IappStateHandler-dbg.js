sap.ui.define(["jquery.sap.global", "sap/ui/base/Object", "sap/ui/core/mvc/ControllerExtension",
	"sap/ui/generic/app/navigation/service/NavError", "sap/ui/generic/app/navigation/service/SelectionVariant", "sap/ui/comp/state/UIState"], function(jQuery, BaseObject, ControllerExtension, NavError, SelectionVariant, UIState) {
	"use strict";
	...
	function fnLogInfo(sMessage, vDetails){
		if (sap.ui.support) { //only if support assistant is loaded	
			...
			if (iLevel < jQuery.sap.log.Level.INFO) {
		...
		jQuery.sap.log.info(sMessage, sDetails, "sap.suite.ui.generic.template.ListReport.controller.IappStateHandler");	
	}
	...
	function getMethods(oState, oController, oTemplateUtils) {
		...
		function getPageState() {
			...
			if (oTableViewData){
				var sPropertyName = oTableViewData.mode === "single" ? "tableViewData" : "tableTabData";
				oGenericData[sPropertyName] = oTableViewData.state;
			}
		...
		function fnAdaptToAppState(oAppData, oURLParameters, sNavType){
			...
			if ((oRealizedAppState.appStateKey !== sAppStateKey ||
				...
				if (!oStoringInformation || oStoringInformation.appStateKey !== sAppStateKey){
					...
					if (!oState.oWorklistData.bWorkListEnabled) {
						...
						if (!bHasOnlyDefaults || oState.oSmartFilterbar.isCurrentVariantStandard()) {
							...
							if (sNavType !== sap.ui.generic.app.navigation.service.NavType.iAppState) {
								...
								applySelectionProperties(oStartupObject.selectionVariant,oRealizedAppState,sSelectionVariant);
								fnSetFiltersUsingUIState(oStartupObject.selectionVariant.toJSONObject(), true, false);
							} else {
								applySelectionProperties(oStartupObject.selectionVariant,oRealizedAppState,sSelectionVariant);
								fnSetFiltersUsingUIState(oStartupObject.selectionVariant.toJSONObject(), !bHasOnlyDefaults, false);
							}
						} else {
							...
							if (!jQuery.sap.equal(oStartupObject.selectionVariant, new SelectionVariant(JSON.stringify(oState.oSmartFilterbar.getUiState().getSelectionVariant())))) {
								applySelectionProperties(oStartupObject.selectionVariant,oRealizedAppState,sSelectionVariant);
								fnSetFiltersUsingUIState(oStartupObject.selectionVariant.toJSONObject(), true, false);
							}
						}
					...
					var sMode = oState.oMultipleViewsHandler.getMode();
					if (sMode) {
						var sPropertyName = sMode === "single" ? "tableViewData" : "tableTabData";
						if (oAppData.customData[dataPropertyNameGeneric] && oAppData.customData[dataPropertyNameGeneric][sPropertyName]) {
							oState.oMultipleViewsHandler.restoreFromIappState(oAppData.customData[dataPropertyNameGeneric][sPropertyName]);
						}
					}
					if (oState.oWorklistData.bWorkListEnabled) {
			...
			if (sNavType !== sap.ui.generic.app.navigation.service.NavType.iAppState) {
				...
				// selected tab from extension must be set in all cases except when navType is iAppstate
				if (oStartupObject.selectedQuickVariantSelectionKey) {
					...
					if (sMode) {
						if (oController && oController.getOwnerComponent && oController.getOwnerComponent().getModel) {
							...
							oTemplatePrivateModel.setProperty("/listReport/multipleViews/selectedKey", oStartupObject.selectedQuickVariantSelectionKey);
						}
					}
				}
			}
			...
			if (sNavType === "initial" && oState.oWorklistData.bWorkListEnabled) {
		...
		function fnParseUrlAndApplyAppState(){
			...
			var oRet = new Promise(function(fnResolve, fnReject){
				...
				oParseNavigationPromise.fail(function(oNavError, oURLParameters, sNavType){
						 ...
						 */
						jQuery.sap.log.warning(oNavError.getErrorCode(), "app state could not be parsed - continuing with empty state", "sap.suite.ui.generic.template.ListReport.controller.IappStateHandler");
