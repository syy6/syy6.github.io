...
sap.ui.define([
	...
	DeltaMicroChart.prototype.getAltText = function(bIsActive) {
	...
	DeltaMicroChart.prototype._isCalcSupported = function() {
		return jQuery(document.getElementById(this.getId() + "-calc")).css("max-width") === "11px";
	};

	DeltaMicroChart.prototype._isRoundingSupported = function() {
		return jQuery(document.getElementById(this.getId() + "-calc1")).width() === 4;
	};
	...
	DeltaMicroChart.prototype._onControlIsVisible = function() {
		...
		if ((!this._bCalc || !this._bRounding) && this._hasData()) {
			var oChart = (this.getId() + "-dmc-chart" ? window.document.getElementById(this.getId() + "-dmc-chart") : null);
	...
	DeltaMicroChart.prototype._adjustRound = function() {
		var iChartWidth = jQuery(document.getElementById(this.getId() + "-dmc-chart")).width();
		var iDeltaWidth = Math.round(iChartWidth * this._oChartData.delta.width / 100);

		jQuery(document.getElementById(this.getId() + "-dmc-bar-delta")).width(iDeltaWidth);

		if (this._oChartData.bar1.isSmaller && !this._oChartData.delta.isMax) {
			jQuery(document.getElementById(this.getId() + "-dmc-bar1")).width(iChartWidth - iDeltaWidth);
		}

		if (this._oChartData.bar2.isSmaller && !this._oChartData.delta.isMax) {
			jQuery(document.getElementById(this.getId() + "-dmc-bar2")).width(iChartWidth - iDeltaWidth);
		}
	};
	...
	DeltaMicroChart.prototype._adjustCalc = function() {
		...
		adjustBar(jQuery(document.getElementById(this.getId() + "-dmc-bar1")));
		adjustBar(jQuery(document.getElementById(this.getId() + "-dmc-bar2")));
		adjustBar(jQuery(document.getElementById(this.getId() + "-dmc-bar-delta")));
	};
	...
	DeltaMicroChart.prototype._hasData = function() {
		return this.getValue1() !== 0 || this.getValue2() !== 0;
	};
