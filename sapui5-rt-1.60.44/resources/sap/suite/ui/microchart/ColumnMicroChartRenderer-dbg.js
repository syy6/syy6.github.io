...
sap.ui.define([
	...
	function(library, encodeXML, MicroChartRenderUtils, Parameters, ValueColor, ValueCSSColor) {
	"use strict";
		...
		if (oControl._hasData()) {
			...
			for (i = 0; i < iColumnsNum; i++) {
				...
				if (bColumnLabels && jQuery.isNumeric(fValue) && fValue >= 0) {
					this._writeColumnLabel(oRm, oColumn, "sapSuiteClMCLabelColumnTop");
				}
				...
				if (bColumnLabels && jQuery.isNumeric(fValue) && fValue < 0) {
					this._writeColumnLabel(oRm, oColumn, "sapSuiteClMCLabelColumnBottom");
				}
	...
	ColumnMicroChartRenderer._writeColumnLabel = function (oRm, oColumn, sClass) {
