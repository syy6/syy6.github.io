	...
	*/
	...
	RadialMicroChartRenderer.render = function(oRm, oControl) {
		...
		RadialMicroChartRenderer._writeMainProperties = function(oRm, oControl) {
			...
			var sAriaLabel = oControl._getTooltipText(bIsActive);
	...
	RadialMicroChartRenderer._writeSVGStartElement = function(control, oRm) {
		...
		oRm.write("<svg class=\"sapSuiteRMC " + encodeXML(sSizeClass) + "\" focusable=\"false" + "\" viewBox=\"0 0 " + RadialMicroChartRenderer.FORM_RATIO + ' ' + RadialMicroChartRenderer.FORM_RATIO + "\" preserveAspectRatio=\"" + encodeXML(sPreserveAspectRatio) + "\" version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\">");
	};
	...
	RadialMicroChartRenderer._writeText = function(control, oRm) {
		oRm.write("<text class=\"sapSuiteRMCFont\" text-anchor=\"middle\" alignment-baseline=\"middle\"" + "\" font-size=\"" + RadialMicroChartRenderer.NUMBER_FONT_SIZE + "\" x=\"" + RadialMicroChartRenderer.SVG_VIEWBOX_CENTER_FACTOR + "\" y=\"" + encodeXML(this._getVerticalViewboxCenterFactorForText()) + "\"> " + encodeXML(this._generateTextContent(control)) + "</text>");
	};
