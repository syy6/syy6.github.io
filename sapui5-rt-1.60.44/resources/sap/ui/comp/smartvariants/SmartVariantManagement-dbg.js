...
sap.ui.define([
	"sap/ui/thirdparty/jquery", 'sap/ui/comp/library', './PersonalizableInfo', './SmartVariantManagementAdapter', 'sap/ui/comp/variants/VariantItem', 'sap/ui/comp/variants/VariantManagement', 'sap/ui/comp/odata/MetadataAnalyser', 'sap/ui/fl/registry/Settings', 'sap/ui/fl/Change', 'sap/ui/fl/Persistence', 'sap/ui/fl/Utils', 'sap/ui/fl/transport/TransportSelection', "sap/base/Log"
], function(jQuery, library, PersonalizableInfo, SmartVariantManagementAdapter, VariantItem, VariantManagement, MetadataAnalyser, Settings, Change, Persistence, FlexUtils, TransportSelection, Log) {
	"use strict";
	 ...
	 *        SAPUI5 flexibility services to manage the variants.<br>
	 *        <h3>Usage</h3>
	 ...
	 * @see {@link topic:a8e55aa2f8bc4127923b20685a6d1621 SAPUI5 Flexibility Services: Adapting UIs Made Easy}
	...
	SmartVariantManagement.prototype.getTransportSelection = function() {
		return new Promise(function(fResolve) {
			sap.ui.require([
				...
				fResolve(new TransportSelection());
			});
		});
	};

	SmartVariantManagement.prototype._loadFlexLibrary = function() {
		...
		if (!this._oFlLibrary) {
			...
			this._oFlLibrary = sap.ui.getCore().loadLibrary('sap.ui.fl', {
				async: true
			}).then(function() {
				this._loadFlexClasses();
			}.bind(this));
		}
	};

	SmartVariantManagement.prototype._loadFlexClasses = function() {

		if (!this._oFlClasses) {

			this._oFlClasses = new Promise(function(fResolve) {
				sap.ui.require([
					"sap/ui/fl/registry/Settings", "sap/ui/fl/Persistence", "sap/ui/fl/Utils"
				], function(fSettings, fPersistence, fUtils) {
					Settings = fSettings;
		...
		return this._oFlClasses;
	};

	SmartVariantManagement.prototype.setPersControler = function(oControl) {
		if (Settings && Persistence && FlexUtils) {
			this._setPersControler(oControl);
		} else {
			this._oFlLibrary.then(function() {
				this._loadFlexClasses().then(function() {
					this._setPersControler(oControl);
				}.bind(this));
			}.bind(this));
		}
	...
	SmartVariantManagement.prototype._setPersControler = function(oControl) {
		if (!this._oPersoControl) {
			if (FlexUtils.getComponentClassName(oControl)) {
...
// }
	...
	SmartVariantManagement.prototype._isVendorLayer = function() {
		return FlexUtils.isVendorLayer();
	};
...
//				} else
			...
			if (this._oPersistencyPromise) {
				...
				this._oPersistencyPromise.then(function() {
					...
					if (this._oControlPromise && this._oPersoControl && oCurrentControlWrapper) {
						Promise.all([
							this._oMetadataPromise, this._oControlPromise, Settings.getInstance()
	...
	SmartVariantManagement.prototype._dataReceived = function(mVariants, oSettings, oCurrentControlWrapper) {
		...
		if (!this._bIsInitialized) {
			...
			if (oSettings && oSettings.isVariantSharingEnabled) {
				this.setShowShare(oSettings.isVariantSharingEnabled());
			}
			...
			if (this._sAppStandardVariantKey) {
				...
				if (oAppStdContent) {
					...
					if (oAppStdContent) {
						oAppStdContent = jQuery.extend(true, {}, oAppStdContent);
					}
				}
...
// oContent = JSON.parse(sContent);
	...
	SmartVariantManagement.prototype._newVariantContent = function(oCurrentControlInfo) {
		...
		var oControlContent = null, oContent, sPersKey;
