...
sap.ui.define([
	...
	SmartTable.prototype._triggerGWExport = function() {
		...
		var fDownloadXls = function() {
			...
			jQuery.sap.openWindow(mExportSettings.url);
		}.bind(this);
	...
	SmartTable.prototype._addExportToExcelToToolbar = function() {
		...
		if (this.getUseExportToExcel() && (this._bTableSupportsExcelExport || this.getExportType() === "UI5Client")) {
			var sButtonLabel, sButtonId, sIconUrl, oResourceBundle = sap.ui.getCore().getLibraryResourceBundle("sap.ui.comp");
			if (!this._oUseExportToExcel) {
				sButtonLabel = oResourceBundle.getText("TABLE_EXPORT_TEXT");
				sButtonId = this.getId() + "-btnExcelExport";
				sIconUrl = "sap-icon://excel-attachment";
				// render OverflowToolbarButton when exportType="GW" else render MenuButton with Export settings option
				if (this.getExportType() === "GW") {
					...
					this._oUseExportToExcel = new MenuButton(sButtonId, {
						icon: sIconUrl,
						tooltip: sButtonLabel,
										...
										press: function() {
											...
											oExportLibLoadPromise.then(function() {
												sap.ui.require([
													...
													// default fileName is the header of the SmartTable
													var oCustomConfig = this._cachedExcelSettings || {
														fileName: this.getHeader()
													};
													var oExportSettingsDialogPromise = ExportUtils.openExportSettingsDialog(oCustomConfig, this);
													oExportSettingsDialogPromise.then(function(oData) {
														oData.getUserInput().then(function(oUserInput) {
															if (oUserInput) {
																...
																if (oUserInput.includeFilterSettings) {
																	ExportUtils.parseFilterConfiguration(this._getRowBinding()).then(function(oFilterConfig) {
																		this._triggerUI5ClientExport(oUserInput, oFilterConfig);
																	}.bind(this));
																} else {
	...
	/**
	 * Returns promise after loading the export library
	 *
	...
	/**
	 * Returns the label/header of the column
	 *
	 * @param {Object} oColumn column object
	 * @returns {Object} column label/header
	 * @private
	 */
	 ...
	 * @param {Object} mFilterConfig filter config to be added to the exported spreadsheet
	 * @private
	 */
	SmartTable.prototype._triggerUI5ClientExport = function(mCustomConfig, mFilterConfig) {
		var oExportLibLoadPromise = this._loadExportLibrary();
		oExportLibLoadPromise.then(function() {
			var aColumns = this._oTable.getColumns(), i, iLen = aColumns.length, oColumn, oColumnData, oLabel, sPath, sTemplate, sAdditionalPath, oAdditionalColumn, sLabel, sSplitLabel, oAdditionalProperty, sHAlign, nWidth, sType, inputFormat, oType, aSheetColumns = [];
			...
			for (i = 0; i < iLen; i++) {
				...
				if (oColumn.getVisible()) {
					...
					if (sPath) {
						oLabel = this._getColumnLabel(oColumn);
						...
						} else if (oColumnData.description) { // check this last as at times numeric, date other fields too might have a description
						...
						if (sAdditionalPath) {
							...
							sSplitLabel = oLabel && oLabel.getText();
							oLabel = null;
							sLabel = null;
							if (oAdditionalColumn) {
								oLabel = this._getColumnLabel(oAdditionalColumn);
								if (oLabel) {
								...
								oAdditionalProperty = null;
								for (var j = 0; j < this._aTableViewMetadata.length; j++) {
									if (this._aTableViewMetadata[j].name === sAdditionalPath) {
										oAdditionalProperty = this._aTableViewMetadata[j];
										break;
									}
								}
			...
			var sFileName = mCustomConfig ? mCustomConfig.fileName : this.getHeader();
			...
			sap.ui.require([
				"sap/ui/export/Spreadsheet", "sap/ui/export/ExportUtils"
				...
				// user export settings are also provided to the application
				this.fireBeforeExport({
					exportSettings: mExportSettings,
	...
	SmartTable.prototype._createTable = function() {
		...
		if (this._oTable.getEnableCustomFilter && this._oTable.getEnableCustomFilter() && this._bIsFilterPanelEnabled) {
			...
			if (this._oTable.attachCustomFilter) {
				this._oTable.attachCustomFilter(this._onCustomFilter.bind(this));
			}
		}
	...
	SmartTable.prototype._createResponsivePopover = function() {
		this._oFilterButton = new OverflowToolbarButton({
			type: "Transparent",
			icon: "sap-icon://filter",
			tooltip: sap.ui.getCore().getLibraryResourceBundle("sap.ui.comp").getText("SMARTTABLE_RP_FILTER"),
			press: [
				function(oEvent) {
		...
		this._oSortButton = new OverflowToolbarButton({
			type: "Transparent",
			icon: "sap-icon://sort",
			tooltip: sap.ui.getCore().getLibraryResourceBundle("sap.ui.comp").getText("SMARTTABLE_RP_SORT"),
			press: [
				function(oEvent) {
					this._oResponsivePopover.close();
		...
		var oToolbar = new OverflowToolbar({
			content: [
				this._oFilterButton, this._oSortButton, new ToolbarSpacer(), new OverflowToolbarButton({
					type: "Transparent",
					icon: "sap-icon://decline",
					tooltip: sap.ui.getCore().getLibraryResourceBundle("sap.ui.comp").getText("SMARTTABLE_RP_CLOSE"),
					press: [
						this._oResponsivePopover.close, this._oResponsivePopover
					]
				})
			]
		});

		this._oResponsivePopover.addContent(oToolbar);
	};
	...
	SmartTable.prototype._onColumnPress = function(oEvent) {
		...
		if ((sFilterProperty && this._bIsFilterPanelEnabled) || sSortProperty) {
			this._oResponsivePopover.removeAllAriaLabelledBy();
			this._oResponsivePopover.addAriaLabelledBy(this._oColumnClicked.getHeader());
			var oActiveHeaderDomRef = this._oColumnClicked.getDomRef().children[0];
			this._oResponsivePopover.setOffsetY(-oActiveHeaderDomRef.clientHeight);
			this._oResponsivePopover.openBy(this._oColumnClicked);
		}
	};
	...
	SmartTable.prototype._onCustomSort = function() {
		var oColumnData = this._oColumnClicked.data("p13nData");
		if (oColumnData && !oColumnData.sorted) {
			oColumnData.sorted = {
				"ascending": false
			};
		...
		this._oTable.fireEvent("sort", {
			...
			sortOrder: bAscending ? "Descending" : "Ascending",
			columnAdded: false
		});

		oSorted.ascending = !bAscending;
	};
	...
	SmartTable.prototype._setUiState = function(oUIState, bApplyAsVariant) {
		...
		if (this._oPersController) {
			if (bApplyAsVariant) {
				this._oPersController.setPersonalizationDataAsDataSuiteFormat(jQuery.extend(true, {}, oUIState.getPresentationVariant(), oUIState.getSelectionVariant()), true);
			} else {
				...
				this._oPersController.setDataSuiteFormatSnapshot(jQuery.extend(true, {}, oUIState.getPresentationVariant(), oUIState.getSelectionVariant()), oPersistentDataVariant, true);
			}
		}
	};
	...
	SmartTable.prototype._personalisationModelDataChange = function(oEvent) {
		...
		// sync sort action on the ResponsiveTable with p13n
		if (this._isMobileTable) {
	...
	SmartTable.prototype._adaptCustomSort = function() {
		if (this._oCurrentVariant) {
			var oColumnData, sColumnKey;
			this._oTable.getColumns().forEach(function(oColumn) {
				oColumnData = oColumn.data("p13nData");
				sColumnKey = oColumnData && oColumnData.columnKey;
				if (sColumnKey && this._oCurrentVariant.sort && this._oCurrentVariant.sort.sortItems) {
					this._oCurrentVariant.sort.sortItems.forEach(function(oMSortItem) {
						if (sColumnKey !== oMSortItem.columnKey) {
							if (oColumnData && oColumnData.sorted) {
						...
						if (oMSortItem.operation === "Ascending") {
							oColumnData.sorted = {
								"ascending": true
							};
						}
				...
				if (sPath) {
					if (bIsTimeField) {
						...
						if (oValue1 instanceof Date && !bIsTypeDateTimeOffset  && this._oTableProvider && this._oTableProvider.getIsUTCDateHandlingEnabled()) {
							...
							// this is similar to the date value handling inside the FilterProvider. For type DateTimeOffset we do NOT correct the time.
	...
	SmartTable.prototype.exit = function() {
		...
		if (this._oResponsivePopover) {
			this._oResponsivePopover.destroy();
		}
		this._oResponsivePopover = null;
