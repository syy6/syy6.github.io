...
sap.ui.define([], function() {
	"use strict";
	// Function to check if the specified type name is related to some UI5 table
	function checkTableTypesMatch(sType) {
		return isTable(sType) || sType === "sap.m.Column";
	}

	function isTable(sType){
		return (sType === "sap.m.Table" || sType.indexOf("sap.ui.table") > -1);
	}

	function isToolbar(sType){
		return (sType === "sap.m.Toolbar" || sType === "sap.m.OverflowToolbar");
	}

	// Check if element is related to SmartLink (allow both SmartLink and ObjectIdentifier for RTA)
	function isSmartLinkRelated(sType){
		return (sType === "sap.ui.comp.navpopover.SmartLink" || sType === "sap.m.ObjectIdentifier");
	}
	return {
		aggregations: {
			customToolbar: {
			...
			items: {
				propagateMetadata: function(oElement) {
					...
					var bIsSmartLinkOrToolbar = (isSmartLinkRelated(sType) || isToolbar(sType));
					// Disable RTA for all other UI5 table related code and sap.m.Title
					if (isToolbar(sType)){
						return {
							actions: {
								addXML : {
					...
					} else if (sType === "sap.m.ColumnListItem"){
						return {
							actionsEffectiveAfter : "RECREATE", //actions like property change & addXML need to be done before SmartTable is initialized
							actions: null,
							aggregations : {
					...
					} else if (!bIsSmartLinkOrToolbar && (sType === "sap.m.Title" || checkTableTypesMatch(sType) || checkTableTypesMatch(oElement.getParent().getMetadata().getName()))) {
						return {
							actions: null
						};
					}
		...
		annotations: {
			 ...
			 *
			 * <b>Note:</b> Currently <code>SortOrder</code> is not evaluated for <code>ResponsiveTable</code> control.
			 *
