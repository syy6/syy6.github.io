...
sap.ui.define([
	...
	DateRangeType.setEndTime = function(oDate, bIgnoreTime) {
		...
		if (!bIgnoreTime) {
			...
			return DateRangeType.setStartTime(oDate);
		}
	};
	...
	DateRangeType.Operations = {
		...
		TOMORROW: {
			...
			order: 3.2,
			defaultValues: function() {
				var aDateRange = DateRangeType.getDateRange(2, "DAY", true);
				aDateRange[0].setDate(aDateRange[0].getDate() + 1);
				return aDateRange;
			},
			type: "range",
			display: "start",
	...
	DateRangeType.prototype.datePickerValidateLastValue = function(oDatePicker) {
		...
		if (!oDatePicker._bValid) {
			sDate = oDatePicker.getValue();
			oDatePicker.resetProperty("value");
			oDatePicker.setValue(sDate);
			if (oDatePicker._bValid) {
				oDatePicker.setValueState(oDatePicker._bValid ? ValueState.None : ValueState.Error);
			}
		}
	};
