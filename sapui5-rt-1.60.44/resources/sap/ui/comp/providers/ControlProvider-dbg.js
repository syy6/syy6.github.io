...
sap.ui.define([
	...
	ControlProvider.prototype._createFieldTemplate = function(oViewField, isEditable) {
		...
		if (this.useSmartField) {
			...
			if (ODataType.isNumeric(oViewField.type) || ODataType.isDateOrTime(oViewField.type) || oViewField.isCalendarDate) {
	...
	ControlProvider.prototype._completeSmartField = function(oViewField) {
		...
		if (oData.annotations.uom || ODataType.isNumeric(oViewField.type) || ODataType.isDateOrTime(oViewField.type) || oViewField.isCalendarDate) {
		...
		ControlProvider._createModelTypeInstance(oViewField, this._oDateFormatSettings);
	};
	...
	ControlProvider.prototype._createEditableTemplate = function(oViewField, bBlockSmartLinkCreation) {
		var oTemplate = null, oType = ControlProvider._createModelTypeInstance(oViewField, this._oDateFormatSettings);
		...
		if (!oTemplate) {
			if (oViewField.type === "Edm.Time") {
				...
				} else if (ODataType.isNumeric(oViewField.type) || ODataType.isDateOrTime(oViewField.type) || oViewField.isCalendarDate) {
					oTemplate.setTextAlign("Right");
					oTemplate.setTextDirection("LTR");
				}
	...
	ControlProvider.prototype._createDisplayOnlyTemplate = function(oViewField, bBlockSmartLinkCreation) {
		var oTemplate = null, oType = null, sAlign, oBindingInfo;
		oType = ControlProvider._createModelTypeInstance(oViewField, this._oDateFormatSettings);
	 ...
	 * @private
	 */
	ControlProvider._createModelTypeInstance = function(oViewField, oDateFormatSettings) {
...
//		var bIsDateFromTypeAndDisplayFormat = oViewField.type === "Edm.DateTime" && oViewField.displayFormat === "Date";
		...
		oConstraints = jQuery.extend({}, oConstraints, {
