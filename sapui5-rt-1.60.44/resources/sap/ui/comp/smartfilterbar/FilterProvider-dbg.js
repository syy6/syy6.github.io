...
// -----------------------------------------------------------------------------
sap.ui.define([
	"sap/ui/thirdparty/jquery", 'sap/m/DateTimePicker', 'sap/m/Select', 'sap/ui/core/Item', 'sap/m/ComboBox', 'sap/m/DatePicker', 'sap/m/DateRangeSelection', 'sap/m/TimePicker', 'sap/m/Input', 'sap/m/MultiComboBox', 'sap/m/MultiInput', 'sap/m/SearchField', 'sap/m/Token', 'sap/ui/comp/odata/MetadataAnalyser', 'sap/ui/comp/providers/ValueHelpProvider', 'sap/ui/comp/providers/ValueListProvider', 'sap/ui/model/Filter', 'sap/ui/model/json/JSONModel', 'sap/ui/comp/odata/ODataType', 'sap/ui/comp/util/FormatUtil', 'sap/ui/base/EventProvider', 'sap/ui/comp/util/IdentifierUtil', 'sap/ui/comp/providers/TokenParser', 'sap/ui/core/format/DateFormat', 'sap/ui/comp/library', 'sap/ui/core/library', 'sap/ui/model/analytics/odata4analytics', 'sap/ui/model/FilterOperator', "sap/base/Log", "sap/base/util/UriParameters", "sap/base/security/encodeURL"
], function(jQuery, DateTimePicker, Select, Item, ComboBox, DatePicker, DateRangeSelection, TimePicker, Input, MultiComboBox, MultiInput, SearchField, Token, MetadataAnalyser, ValueHelpProvider, ValueListProvider, Filter, JSONModel, ODataType, FormatUtil, EventProvider, IdentifierUtil, TokenParser, DateFormat, library, coreLibrary, odata4analytics, FilterOperator, Log, UriParameters, encodeURL) {
	"use strict";
		...
		switch (oFilterFieldMetadata.type) {
			...
			case "Edm.DateTime":
				...
				if (typeof sValue == "string" && oFilterFieldMetadata.displayFormat && oFilterFieldMetadata.displayFormat.toLowerCase() == "date") {
					...
					var vDate = new Date(aParts[0] + "T00:00:00");
					if (vDate == "Invalid Date") {
						Log.error("Default value for " + oFilterFieldMetadata.fieldName + " could not be parsed properly. Trying fallback conversion...");
						vDate = new Date(sValue);
					}
					return vDate;
				} else {
					Log.error("Default value for " + oFilterFieldMetadata.fieldName + " could not be parsed.");
			...
			if (oControl instanceof MultiInput) {
				...
				if (aRanges) {
					...
					while (i--) {
						...
						if ((oRange.value1 === "") && (oRange.operation === FilterOperator.EQ)) {
						...
						if (oRange.tokenText) {
							...
							if (oFilterFieldMetadata) {
								...
								if (aDateTypes.indexOf(oFilterFieldMetadata.filterType) > -1) {
									...
									if (oType) {
										// We can not use the oType (DateTime type instance) to format the value, because we have ensured that
										// UTC=false
										if (value1) {
						...
						if (oRangeData && (oToken.getKey().indexOf("range_") === 0)) {
							if (oFieldViewMetadata.type === "Edm.Time") {
								if (oRangeData.value1 && oRangeData.value1.ms) {
									oRangeData.value1 = new Date(oRangeData.value1.ms);
									oRangeData.value1 = new Date(oRangeData.value1.valueOf() + oRangeData.value1.getTimezoneOffset() * 60 * 1000);
								}
								if (oRangeData.value2 && oRangeData.value2.ms) {
									oRangeData.value2 = new Date(oRangeData.value2.ms);
									oRangeData.value2 = new Date(oRangeData.value2.valueOf() + oRangeData.value2.getTimezoneOffset() * 60 * 1000);
								}
							}
						}
					...
					if (oValue && oValue.hasOwnProperty("low")) {// The data in the model corresponds to low and high Objects
						...
						if (oValue && oValue.hasOwnProperty("ranges")) { // Check if the data is for an unrestricted filter
							...
							while (iLen--) {
								...
								oValue1 = FilterProvider.adaptTime(oValue1, oFieldMetadata);
								oValue2 = FilterProvider.adaptTime(oValue2, oFieldMetadata);
								if (aValue[iLen].exclude) { // Exclude Select Option is not supported entirely except EQ and Empty, which can be
									// changed to NE
									if (aValue[iLen].operation === ValueHelpRangeOperation.Empty) {
										aExcludeFilters.push(new Filter(sField, FilterOperator.NE, ""));
									} else if (aValue[iLen].operation === FilterOperator.EQ) {
										aExcludeFilters.push(new Filter(sField, FilterOperator.NE, oValue1));
									}
								} else {
									if (aValue[iLen].operation === ValueHelpRangeOperation.Empty) {
										aArrayFilters.push(new Filter(sField, FilterOperator.EQ, ""));
									} else if (aValue[iLen].operation === sap.ui.model.FilterOperator.BT) {
...
// continue;
	...
	FilterProvider.adaptTime = function(oDate, oMetadata) {
		var iPrecision, iMilliseconds, oResultDate;
		...
		iPrecision = parseInt(oMetadata.precision, 10);
		iMilliseconds = oDate.getMilliseconds();
