 ...
 */
sap.ui.define([
	"sap/ui/comp/odata/MetadataAnalyser", "sap/ui/comp/smartfield/AnnotationHelper", "sap/base/Log"
], function(MetadataAnalyser, Annotation, Log) {
	"use strict";
	...
	/**
	 * Gets the URL path name to the entity specified in the <code>Path</code> attribute of the
	 * <code>com.sap.vocabularies.Common.v1.Text</code> annotation where the text is fetched.
	 *
	 * @param {object} oMetaData The metadata object
	 * @returns {string} The URL path name where the description value is fetched
	 * @protected
	 */
	ODataHelper.prototype.getTextPathFromValueList = function(oMetaData) {
		var oValueListKeyProperty = oMetaData.property.valueListKeyProperty,
			oValueListTextAnnotation = oValueListKeyProperty && oValueListKeyProperty["com.sap.vocabularies.Common.v1.Text"],
			oValueListEntitySet = oMetaData.property.valueListEntitySet;

		if (oValueListTextAnnotation && oValueListEntitySet) {
			return "/" + oValueListEntitySet.name + "/" + oValueListTextAnnotation.Path;
		}
