...
sap.ui.define([
	...
	var SmartField = Control.extend("sap.ui.comp.smartfield.SmartField", /** @lends sap.ui.comp.smartfield.SmartField.prototype */ {
		metadata: {
			...
			properties: {
				 ...
				 *
				 * <b>Note</b>: This feature is only supported for Entity Data Model (EDM) properties typed as
				 * <code>Edm.String</code> or <code>Edm.Guid</code> which are bound to the
				 * {@link sap.ui.comp.smartfield.SmartField#value} property of the <code>SmartField</code> control.
				 * In addition, the binding mode for the {@link sap.ui.comp.smartfield.SmartField#value} property must
				 * be the two-way binding mode.
				 *
	...
	SmartField.prototype.init = function() {
		...
		this.attachEvent("innerControlsCreated", SmartField.prototype._setOnInnerControl, this);
	};
	...
	SmartField.prototype.setUomEditable = function(bUomEditable) {
		...
		var oUomField = this._getEmbeddedSmartField();

		if (oUomField) {
			oUomField.setEditable(this.getUomEditable());
		}
	...
	SmartField.prototype.setUomEnabled = function(bUomEnabled) {
		...
		if (oUomField) {
			oUomField.setEnabled(this.getUomEnabled());
		}
	...
	SmartField.prototype.setUomVisible = function(bUomVisible) {
		...
		if (oUomField) {
			oUomField.setVisible(this.getUomVisible());
		}
	...
	SmartField.prototype._setOnInnerControl = function() {
		...
		if (oChild) {
			...
			this._setPropertyOnInnerControls("wrapping", this.getWrapping());
			...
			if (this.isInEditMode()) {
				this._setPropertyOnInnerControls("required", this.getMandatory());
			}
	...
	SmartField.prototype._setPropertyOnInnerControls = function(sProperty, vValue, aControls) {
		...
		aControls.forEach(function(oControl) {
			...
			if (typeof oControl[sMutator] === "function") {
				oControl[sMutator](this._convertValueForInnerControl(sProperty, vValue, oControl));
			}
		}, this);
	};
	 ...
	 * @since 1.56
	 */
	SmartField.prototype._convertValueForInnerControl = function(sProperty, vValue, oControl) {
		var oProperty;
	...
	SmartField.prototype.unbindProperty = function(sPropertyName, bSuppressReset) {
		...
		aInnerControls.forEach(function(oInnerControl) {
			var vPropertiesNamesOfInnerControls = ODataControlFactory.getBoundPropertiesMapInfoForInnerControl(oInnerControl.getMetadata().getName(), {
	...
	SmartField.prototype.setValue = function(sValue) {
		...
		if (!this.isPropertyBeingUpdatedByModel("value") && !this.getBindingContext()) {
			var oInnerControl = this.getInnerControls()[0];
			...
			if (oInnerControl) {
				...
				this._setPropertyOnInnerControls(sBoundPropertyNameOfInnerControl, sValue, [oInnerControl]);
			}
		}
		...
		if (this.isTextInEditModeSourceValid() && (sValue !== sOldValue) && !bModelUpdate && oControl) {
		...
		if (oBinding) {
			...
			try {
				...
				if (oBinding.sInternalType && oType && !oType.bValueValidated) {
					var sParsedValue = oType.parseValue(sValue, oBinding.sInternalType, oBinding.getValue());
	...
	SmartField.prototype.onBeforeValidateValue = function(sValue, aFilterFields) {

		// Prevent unnecessary requests to be sent and validation errors to be displayed,
		// if the binding context is not set
		if (!this.getBindingContext()) { // note: the binding context can be null or undefined
			return;
		}
	...
	function getValueListFilters(vValue, oSettings) {
		var aFilters = oSettings.aFilterFields;

		if (aFilters.length === 1) {
			return [SmartField["getValueList" + aFilters[0].charAt(0).toUpperCase() + aFilters[0].slice(1) + "Filter"](vValue, oSettings)];
		}
		...
		return [
			new Filter({
				and: false,
				filters: [
					SmartField.getValueListKeyFieldFilter(vValue, oSettings),
					SmartField.getValueListDescriptionFieldFilter(vValue, oSettings)
				]
			})
		];
	}

	SmartField.getValueListKeyFieldFilter = function(vValue, oSettings) {
		return new Filter({
			value1: vValue,
	...
	SmartField.getValueListDescriptionFieldFilter = function (vValue, oSettings) {
		return new Filter({
			value1: vValue,
