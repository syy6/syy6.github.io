...
sap.ui.define([
...
], function(Link, Control, VBox, HBox, Button, Title, Image, Text, SimpleForm, VBoxRenderer, Factory, LinkData, JSONModel, Util, HorizontalLayout, VerticalLayout, layoutLibrary, PersonalizationUtil, ManagedObjectObserver, FlexItemData, BindingMode, mLibrary, Controller, SelectionWrapper, ColumnWrapper, Log, AddLink, CustomData) {
	"use strict";
	...
	NavigationContainer.prototype.openSelectionDialog = function(bForbidNavigation, bShowReset, fCallbackAfterClose, bIsEndUser, sStyleClass) {
		this.fireBeforePopoverOpen();

		// We have to wait until all changes have been applied to the NavigationContainer
		return sap.ui.fl.FlexControllerFactory.createForControl(this).waitForChangesToBeApplied(this).then(function() {
								...
								payload: {
									callbackSaveChanges: function(oPersistentDeltaData) {
										...
										if (bDialogConfirmedReset) {
											return NavigationContainer._discardChanges(this, bIsEndUser).then(function() {
	 ...
	 * @param {boolean} bIsEndUser <code>true</code> when end user changes have to be discarded
	 * @returns {Promise} A <code>Promise</code> for asynchronous execution
	 * @private
	 */
	NavigationContainer._discardChanges = function(oSelectorControl, bIsEndUser) {
		return new Promise(function(resolve) {
			sap.ui.getCore().loadLibrary('sap.ui.fl', {
				...
				sap.ui.require([
					...
					return FlexConnector.discardChangesForControl(oSelectorControl, bIsEndUser).then(function() {
						return resolve(true);
					}, function(oError) {
						Log.error("Changes could not be discarded in LRep: " + oError.status);
			...
			sap.ui.getCore().loadLibrary('sap.ui.fl', {
				...
				sap.ui.require([
					...
					return FlexConnector.createAndSaveChangesForControl(aMAddedLinks, aMRemovedLinks, oSelectorControl).then(function() {
						...
						Log.error("Changes could not be saved in LRep: " + oError.status);
						return resolve(false);
					});
				});
			});
	...
	NavigationContainer.prototype._createContent = function() {
		...
		this._oPersonalizationButton = new HBox({
			...
			items: new Button({
				...
				press: function() {
					this.openSelectionDialog(false, true, undefined, true, undefined);
				}.bind(this)
			})
		});
