 ...
 */
sap.ui.define([
	'sap/ui/comp/library', 'sap/m/MultiComboBox', 'sap/m/MultiInput', 'sap/m/DatePicker', 'sap/m/DateRangeSelection', "sap/base/Log"
], function(library, MultiComboBox, MultiInput, DatePicker, DateRangeSelection, Log) {
	"use strict";
				...
				if ((oFilterMetaData.type === "Edm.DateTime") || (oFilterMetaData.type === "Edm.Time")) {
					...
					if (sValue.indexOf('Z') !== (sValue.length - 1)) {
						...
						if (oFilterBar && oFilterBar.isInUTCMode && !oFilterBar.isInUTCMode() && oFilterBar.getDateInUTCOffset) {
							oValue = oFilterBar.getDateInUTCOffset(new fcreateDateObject(sValue)).toJSON();
						} else {
