...
sap.ui.define([
	...
	"sap/base/Log"
], function(CompLibrary, Control, ChartProvider, MicroChartLibrary, CustomData, SmartMicroChartCommons, Log) {
	"use strict";
	...
	SmartMicroChart.prototype.onBeforeRendering = function() {
		...
		if (oChart) {
			if (oChart.getMetadata().hasProperty("height") && !this.getIsResponsive()) {
				oChart.setProperty("height", this.getHeight(), true);
			}
			if (oChart.getMetadata().hasProperty("width") && !this.getIsResponsive()) {
	...
	SmartMicroChart.prototype._createInnerChartFromDataPoint = function (oDataPoint) {
		...
		if (SmartMicroChartCommons._isBulletVizualizationType(sType)) {
