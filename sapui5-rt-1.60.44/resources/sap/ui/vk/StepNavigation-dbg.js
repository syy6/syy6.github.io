...
sap.ui.define([
	"jquery.sap.global", "sap/ui/core/Control", "sap/ui/core/ResizeHandler",
	"./Messages", "./DvlException", "./ContentConnector"
], function(
	jQuery, Control, ResizeHandler,
	Messages, DvlException, ContentConnector) {
	"use strict";
		...
		var processStepThumbnail = function(sceneId, dvl, step) {
			...
			try {
				...
				if (thumbnail === sap.ve.dvl.DVLRESULT.NOTFOUND) {
					...
					thumbDataRaw = sap.ui.vk.dvl.getPointer(thumbnail);
				}
	...
	StepNavigation.prototype._setContent = function(content) {
		var scene = null;
		if (content && content instanceof sap.ui.vk.dvl.Scene) {
			scene = content;
		}
		this.setScene(scene);
	};
