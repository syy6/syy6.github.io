			...
			sap.ui.define([
				...
				HitTestToolHandler.prototype._executeHittest = function(event, clicktype) {
					if (this._inside(event) && !this._gesture) {
                        ...
                        var scene = this._tool._viewport.getScene().getSceneRef();
                        var camera = this._tool._viewport.getCamera().getCameraRef();
				...
				HitTestToolHandler.prototype.click = function(event) {
					this._executeHittest(event, sap.ui.vk.tools.HitTestClickType.Single);
                    // Block further handlers from processing this event
                    event.handled = true;
                };

				HitTestToolHandler.prototype.doubleClick = function(event) {
					this._executeHittest(event, sap.ui.vk.tools.HitTestClickType.Double);
                    event.handled = false;
                };

				HitTestToolHandler.prototype.contextMenu = function(event) {
					this._executeHittest(event, sap.ui.vk.tools.HitTestClickType.Context);
                    event.handled = false;
                };
