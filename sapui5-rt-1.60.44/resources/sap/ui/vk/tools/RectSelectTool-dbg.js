...
sap.ui.define([
	...
	], function(jQuery, library, Tool, RectSelectToolHandler, Loco) {
	"use strict";
	...
	*/
	RectSelectTool.prototype.setActive = function(value, activeViewport, gizmoContainer) {
		if (activeViewport && activeViewport.getImplementation) {
			activeViewport = activeViewport.getImplementation();
		}
	...
	*/
	RectSelectTool.prototype._activateTool = function(activeViewport) {
		...
		if (this._prepare()) {
			this._handler.activate(activeViewport);
		}
	};
	...
	*/
	RectSelectTool.prototype.select = function(x1, y1, x2, y2, scene, camera) {
		...
		if (this.isViewportType("sap.ui.vk.dvl.Viewport")) {
			nodes = this._viewport.rectSelect(x1, y1, x2, y2);
			if (nodes.length > 0) {
				var parametersdvl = {
					picked: nodes
				};
				this._viewport.fireNodesPicked(parametersdvl);

				if (this.getSubtractMode()) {
					this._viewport._viewStateManager.setSelectionStates([], nodes);
				} else {
					this._viewport._viewStateManager.setSelectionStates(nodes, []);
				}
			}
