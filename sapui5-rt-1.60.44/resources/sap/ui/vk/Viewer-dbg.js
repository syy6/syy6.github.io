...
sap.ui.define([
	"jquery.sap.global", "./library", "sap/ui/core/Control", "./Scene", "./ContentResource", "./ContentConnector", "./dvl/ContentManager",
	"./FlexibleControl", "sap/m/VBox", "sap/m/FlexItemData", "sap/ui/layout/Splitter", "sap/ui/layout/SplitterLayoutData",
	"./DvlException", "./Messages", "./ProgressIndicator", "./Notifications", "./ViewStateManager", "./Toolbar", "./SceneTree", "./StepNavigation",
	"./Viewport", "./NativeViewport", "./RedlineDesign"
], function(
	jQuery, library, Control, Scene, ContentResource, ContentConnector, DvlContentManager,
	FlexibleControl, VBox, FlexItemData, Splitter, SplitterLayoutData,
	DvlException, Messages, ProgressIndicator, Notifications, ViewStateManager, Toolbar, SceneTree, StepNavigation,
	Viewport, NativeViewport, RedlineDesign
) {
	"use strict";
	...
	Viewer.prototype._setMainScene = function(scene) {
		if (scene) {
			if (scene !== this._mainScene) {
				...
				this.setEnableStepNavigation(scene instanceof sap.ui.vk.dvl.Scene && (this.getEnableStepNavigation() || this._componentsState.stepNavigation.defaultEnable));
	...
	Viewer.prototype._updateLayout = function() {
		...
		if (this._stepNavigation) {
			this._stepNavigation.setVisible(this.getShowStepNavigation() && this.getEnableStepNavigation() && this._mainScene instanceof sap.ui.vk.dvl.Scene);
		}
	...
	Viewer.prototype._instantiateStepNavigation = function() {
		if (!this._stepNavigation) {
			this._stepNavigation = new StepNavigation(this.getId() + "-stepNavigation", {
	...
	Viewer.prototype._handleContentChangesFinished = function(event) {
		...
		}.bind(this), 0); // we need a timeout to hide the progress indicator in Firefox!
		this.setBusy(false);
		...
		if (failureReason) {
			...
			var errorUnknownType = sap.ui.vk.getResourceBundle().getText("VIEWER_UNKNOWN_CONTENT_RESOURCE_TYPE_CAUSE");
			...
			(Array.isArray(failureReason) ? failureReason : [ failureReason ]).forEach(function(reason) {
				log.error(sap.ui.vk.getResourceBundle().getText(Messages.VIT13.summary), reason.errorMessage, "sap.ui.vk.Viewer");
			});
		}
	...
	Viewer.prototype._handleContentChangesProgress = function(event) {
		...
		if (this._progressIndicator.getVisible()) {
			...
			this._progressIndicator.setDisplayValue(source + " " + sap.ui.vk.getResourceBundle().getText("PROGRESS_INDICATOR_DOWNLOADING") + " " + (percentage ? Math.floor(percentage) + "%" : ""));
		}
	};
