...
sap.ui.define([
		...
		RedlineGesturesHandler.prototype._zoom = function(event) {
			...
			if (targetViewport instanceof sap.ui.vk.NativeViewport) {
				// need to check zoom limit so we stay in sync
				var zoomInLimit = targetViewport._getZoomInLimit();
				var zoomOutLimit = targetViewport._getZoomOutLimit();

				var zoomFactor = targetViewport._getZoomFactor();
				var newZoomFactor = zoomFactor * zoomDelta;

				if (newZoomFactor > zoomInLimit) {
					zoomDelta = zoomInLimit / zoomFactor;
				} else if (newZoomFactor < zoomOutLimit) {
					zoomDelta = zoomOutLimit / zoomFactor;
				}
			}
