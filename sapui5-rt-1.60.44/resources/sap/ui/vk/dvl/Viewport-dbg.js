...
sap.ui.define([
	...
	*/
	...
	Viewport.prototype.rectSelect = function(x1, y1, x2, y2) {
		var nodes = [];
		if (this._dvlRendererId) {
			var result = sap.ui.vk.dvl.getJSONObject(this._dvl.Renderer.RectSelect(x1, y1, x2, y2, this._dvlRendererId));
	...
	Viewport.prototype._handleContentChangesFinished = function(event) {
		if (event.getSource().getContentResources().length > 1) {
			this.zoomTo(sap.ui.vk.ZoomTo.Visible, sap.ve.dvl.DVLID_INVALID, 0, 0);
		}
		this.setShouldRenderFrame();
	};
