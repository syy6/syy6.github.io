...
sap.ui.define([
	"sap/ui/thirdparty/jquery", 'sap/ui/base/ManagedObject', 'sap/ui/model/json/JSONModel'
], function(jQuery, ManagedObject, JSONModel) {
	"use strict";
	...
	P13nSortModel.prototype.moveModelItemPosition = function(iIndexFrom, iIndexTo) {
		...
		var fnGetArrayElementByKey = function(sKey, sValue, aArray) {
			...
			aArray.some(function(oElement_) {
				if (oElement_[sKey] !== undefined && oElement_[sKey] === sValue) {
					oElement = oElement_;
					return true;
				}
			});
		...
		aMItems.forEach(function(oMItem, iIndex) {
			var oMItemCopy = fnGetArrayElementByKey("columnKey", oMItem.columnKey, aMItemsSorted);
			this._updateAndSyncProperties(iIndex, {
