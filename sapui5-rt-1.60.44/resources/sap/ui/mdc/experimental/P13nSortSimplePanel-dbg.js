...
sap.ui.define([
	"sap/ui/thirdparty/jquery", 'sap/m/library', './P13nPanel', './P13nSortModel'
	...
	P13nSortSimplePanel.prototype.onChangeSortOrderBySelection = function(oEvent) {
		...
		this._getInternalModel().updateProperties(iIndex, {
	...
	P13nSortSimplePanel.prototype._selectTableItem = function(oTableItem) {
		...
		this._getTable().getItems().forEach(function(oTableItem, iIndex_) {
			this._getInternalModel().updateProperties(iIndex_, {
	...
	P13nSortSimplePanel.prototype._updateInternalModel = function() {
		...
		this._setInternalModel(new P13nSortModel({
		...
		this._getTable().setModel(this._getInternalModel(), "JSONItems");

		// Reduce selected items to only one (special for single sort)
		if (aSelectedItems.length > 1) {
			var oMItem = this._getInternalModel().getModelItemByColumnKey(aSelectedItems[0].getColumnKey());
	...
	P13nSortSimplePanel.prototype._setInternalModel = function(oInternalModel) {
		this.setProperty("internalModel", oInternalModel);
	};
	P13nSortSimplePanel.prototype._getInternalModel = function() {
		return this.getProperty("internalModel");
	};
