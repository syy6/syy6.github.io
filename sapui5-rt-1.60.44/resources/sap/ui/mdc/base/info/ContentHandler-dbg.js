...
sap.ui.define([
	'sap/ui/base/ManagedObject', './Panel', './PanelItem', 'sap/ui/model/json/JSONModel', 'sap/ui/core/InvisibleText', './Factory', 'sap/ui/base/ManagedObjectObserver', 'sap/ui/thirdparty/jquery', 'sap/base/Log', 'sap/ui/layout/library', 'sap/ui/layout/form/SimpleForm', 'sap/ui/core/Title'
], function(ManagedObject, Panel, PanelItem, JSONModel, InvisibleText, Factory, ManagedObjectObserver, jQuery, Log, layoutLibrary, SimpleForm, CoreTitle) {
	"use strict";
	 ...
	 * @private
	 * @since 1.58.0
	 * @alias sap.ui.mdc.base.info.ContentHandler
	 * @ui5-metamodel This control/element also will be described in the UI5 (legacy) designtime metamodel
	 */
	var ContentHandler = ManagedObject.extend("sap.ui.mdc.base.info.ContentHandler", /** @lends sap.ui.mdc.base.info.ContentHandler.prototype */
	{
		metadata: {
			library: "sap.ui.mdc",
			properties: {
			...
			aggregations: {
				 ...
				 */
				linkHandler: {
					type: "sap.ui.mdc.base.info.ILinkHandler",
					multiple: false
				}
			},
			associations: {
	...
	ContentHandler.prototype.setLinkHandler = function(oLinkHandler) {
		...
		if (oLinkHandler && oLinkHandler.isA("sap.ui.mdc.flp.info.LinkHandler")) {
		...
		} else if (oLinkHandler && oLinkHandler.isA("sap.ui.mdc.base.info.LinkHandler")) {
			this._oObserver.observe(oLinkHandler, {
				aggregations: [
					"items"
				]
	 ...
	 * @returns {Promise} Resolve LinkItem of type sap.ui.mdc.base.info.LinkItem of direct link navigation, else null
	 */
			...
			return new Promise(function(resolve) {
				sap.ui.require([
					...
					return this._determineLinkHandlerItems().then(function(aLinkItems) {
						this._setConvertedLinkHandlerItems(aLinkItems);
					...
					}.bind(this)).then(function(aExtraContent) {
						var aLinkHandlerItems = this._getInternalModel().getProperty("/linkHandlerItems");
						var oPanel = new Panel(this._createPanelId(Utils), {
							enablePersonalization: this.getEnablePersonalization(), // brake the binding chain
							items: this._getInternalModel().getProperty("/baselineLinkHandlerItems").map(function(oMLinkItem) {
							...
							extraContent: !aExtraContent.length && !aLinkHandlerItems.length ? ContentHandler._getNoContent() : aExtraContent,
							...
							metadataHelperPath: "sap/ui/mdc/base/info/ContentHandler"
						});
	 ...
	 *
	 * @param {sap.ui.mdc.base.info.Panel} oPanel The instance Panel control
	...
	ContentHandler.prototype._createPanelId = function(Utils) {
		var oControl = sap.ui.getCore().byId(this.getSourceControl());
		if (!oControl) {
			Log.error("Invalid source control: " + this.getSourceControl() + ". The mandatory 'sourceControl' association should be defined due to personalization reasons.");
			return this.getId() + "-idInfoPanel";
		}
		var oAppComponent = Utils.getAppComponentForControl(oControl);
		if (!oAppComponent) {
			Log.error("Invalid component: " + oAppComponent + ". The mandatory 'sourceControl' association should be assigned to the app component due to personalization reasons.");
			return this.getId() + "-idInfoPanel";
		}
		return oAppComponent.createId("idInfoPanel");
	};
	ContentHandler.prototype._setConvertedLinkHandlerItems = function(aLinkItems) {
		var oModel = this._getInternalModel();
		var aMLinkItems = aLinkItems.map(function(oLinkItem) {
			if (!oLinkItem.getKey()) {
				Log.error("sap.ui.mdc.base.info.ContentHandler: undefined 'key' property of the LinkItem " + oLinkItem.getId() + ". The mandatory 'key' property should be defined due to personalization reasons.");
			}
			return {
		...
		// The item marked as superior items should always be shown (according to the UX)
	...
	ContentHandler.prototype._determineLinkHandlerItems = function() {
		...
		return oLinkHandler ? oLinkHandler.determineItems() : Promise.resolve([]);
	};
	ContentHandler._getNoContent = function() {
		...
		oSimpleForm.addStyleClass("mdcbaseinfoPanelDefaultExtraContent");
		return oSimpleForm;
	};
	...
	ContentHandler.prototype._observeChanges = function(oChanges) {
		if (oChanges.object.isA("sap.ui.mdc.base.info.ILinkHandler") || oChanges.object.isA("sap.ui.mdc.base.info.ContentHandler")) {
