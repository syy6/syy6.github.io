...
sap.ui.define([
	 ...
	 * @private
	 * @since 1.58.0
	 * @alias sap.ui.mdc.base.info.Log
	 * @ui5-metamodel This control/element also will be described in the UI5 (legacy) designtime metamodel
	 */
	var Log = BaseObject.extend("sap.ui.mdc.base.info.Log", /** @lends sap.ui.mdc.base.info.Log.prototype */
	{
	...
	Log.prototype.initialize = function(aSemanticObjects, oContextObject) {
		this.reset();
	...
	Log.prototype.updateSemanticObjectAttributes = function(aSemanticObjects, oContextObjectOld, oContextObjectNew) {
		aSemanticObjects.forEach(function(sSemanticObject) {
			if (!this._oLog.semanticObjects[sSemanticObject]) {
				this.createSemanticObjectStructure(sSemanticObject);
			}
			var aTransformations, oTransformationLast;
			var aAttributesNew = Object.keys(oContextObjectNew);
			for ( var sAttributeNew in oContextObjectNew) {
				if (this._oLog.semanticObjects[sSemanticObject].attributes[sAttributeNew]) {
					...
					if (JSON.stringify(oContextObjectNew[sAttributeNew]) !== JSON.stringify(oTransformationLast["value"])) {// could be an javascript object
						this._oLog.semanticObjects[sSemanticObject].attributes[sAttributeNew].transformations.push({
							value: oContextObjectNew[sAttributeNew],
							description: "\u2139 Value of the attribute has been changed in the modifyItemsCallback callback of the LinkHandler control.",
							reason: "\ud83d\udd34 Please check the implementation if the result is not what you expected."
						});
					}
				} else {
					this.addSemanticObjectAttribute(sSemanticObject, sAttributeNew, {
						transformations: [
							{
								value: oContextObjectNew[sAttributeNew],
								description: "\u2139 The attribute has been added in the modifyItemsCallback callback of the LinkHandler control.",
								reason: "\ud83d\udd34 Please check the implementation if the result is not what you expected."
							}
						]
					});
			...
			for ( var sAttributeOld in oContextObjectOld) {
				if (aAttributesNew.indexOf(sAttributeOld) < 0 && this._oLog.semanticObjects[sSemanticObject].attributes[sAttributeOld]) {
					this._oLog.semanticObjects[sSemanticObject].attributes[sAttributeOld].transformations.push({
						value: undefined,
						description: "\u2139 The attribute has been removed in the modifyItemsCallback callback of the LinkHandler control.",
						reason: "\ud83d\udd34 Please check the implementation if the result is not what you expected."
					});
				}
			}
