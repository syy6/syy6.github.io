...
sap.ui.define([
		...
		"sap/ui/model/Filter",
		'sap/ui/mdc/base/type/Boolean',
		'sap/ui/mdc/base/type/String',
		"jquery.sap.global",
		"sap/base/Log",
	...
	function(
		...
		function getModelFilterObject(oCondition) {
			...
			if (oOperator.valueTypes.length == 1) {
				return new Filter({ path: oCondition.fieldPath, operator: oOperator.filterOperator, value1: oCondition.values[0] });
			} else {
				return new Filter({ path: oCondition.fieldPath, operator: oOperator.filterOperator, value1: oCondition.values[0], value2: oCondition.values[1] });
			}
		}
				...
				for (var i = 0; i < this.valueTypes.length; i++) {
					...
					try {
						...
						if (oType) {
							...
							//TODO the uppercase conversion should be done by the type
							if ((oType.oFormatOptions && oType.oFormatOptions.toUpperCase === true) ||
								(oType.oConstraints && oType.oConstraints.toUpperCase === true)) {
		...
		FilterOperatorConfig._addOperatorTo({
			...
			format: function(aValues, oContext, oType, sDisplayFormat) {
				...
				var iCount = this.valueTypes.length + 1, // we get the code and description as input for EEQ
				...
				if (oType &&
						(oType.getMetadata().getName() === "sap.ui.model.type.Unit" || oType.getMetadata().getName() === "sap.ui.model.type.Currency")) {
					var sText = oType.formatValue(aValues, "string");
					sTokenText = sTokenText.replace(new RegExp("\\$0", "g"), sText);
					return sTokenText;
				}
				...
				for (var i = 0; i < iCount; i++) {
					...
					if (i == 0 && oType && (typeof oType.formatValue === "function")) {
						...
						sReplace = oType.formatValue(sReplace, "string");
					}
		...
		*/
		...
		FilterOperatorConfig._addOperatorTo({
			...
			getModelFilter: function(oCondition) {
				var oOperator = this.oFilterOperatorConfig.getOperator(oCondition.operator);
				return new Filter({ path: oCondition.fieldPath, operator: oOperator.filterOperator, value1: "" });
			}
		}, FilterOperatorConfig._mOperators);
		...
		FilterOperatorConfig.createControl = function(oDataType, oOperator, sPath, index) {
			var oNullableType;
			...
			while (sType && !oControl && sType !== "base") {
				switch (sType) {
					case "boolean":
						// if (!BooleanType) {
						// 	//TODO: use async call?
						// 	BooleanType = sap.ui.requireSync("sap/ui/model/odata/type/Boolean");
						// }
						var oBooleanType = oDataType; //new BooleanType();
						...
						var oModel = new sap.ui.model.json.JSONModel({
							"items": [{
								...
								{
									"key": true,
									"text": oBooleanType ? oBooleanType.formatValue(true, "string") : "true"
								},
								{
									"key": false,
									"text": oBooleanType ? oBooleanType.formatValue(false, "string") : "false"
								}
							]
						});

						merge(oDataType.oFormatOptions, { emptyString: null });
						oNullableType = new NullableBoolean(oDataType.oFormatOptions, oDataType.oConstraints);
						oControl = new sap.m.Select({
							selectedKey: { path: sPath, type: oNullableType, mode: 'TwoWay' },
							width: "100%",
							items: {
								path: "/items",
					...
					case "int":
						merge(oDataType.oFormatOptions, { emptyString: null });
						if (oDataType.isA("sap.ui.model.type.Integer")) {
							oNullableType = new sap.ui.model.type.Integer(oDataType.oFormatOptions, oDataType.oConstraints);
						} else {
							oNullableType = new sap.ui.model.odata.type.Integer(oDataType.oFormatOptions, oDataType.oConstraints);
						}
						oControl = new sap.m.Input({
							value: { path: sPath, type: oNullableType, mode: 'TwoWay' }
						});
					...
					case "float":
						merge(oDataType.oFormatOptions, { emptyString: null });
						if (oDataType.isA("sap.ui.model.type.Integer")) {
							oNullableType = new sap.ui.model.type.Float(oDataType.oFormatOptions, oDataType.oConstraints);
						} else {
							oNullableType = new sap.ui.model.odata.type.Double(oDataType.oFormatOptions, oDataType.oConstraints);
						}
						oControl = new sap.m.Input({
							value: { path: sPath, type: oNullableType, mode: 'TwoWay' }
						});
					...
					case "date":
						oControl = new sap.m.DatePicker({
							value: { path: sPath, type: oDataType, mode: 'TwoWay' }
						});

						break;
					case "time":
						oControl = new sap.m.TimePicker({
							width: "100%",
							value: { path: sPath, type: oDataType, mode: 'TwoWay' }
						});
