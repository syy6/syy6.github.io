...
sap.ui.define([
	...
	var DefineConditionPanel = XMLComposite.extend("sap.ui.mdc.base.DefineConditionPanel", {
		...
		onBeforeRendering: function() {
			...
			if (!oVLayout.getBindingInfo("content")) {
				oVLayout.bindAggregation("content", {path:'cm>/conditions/' + this.sFieldPath, filters: [new Filter('operator', 'NE', 'EEQ')], template: this.byId("conditionRow")});
			}
		...
		addCondition: function(oEvent) {
			...
			var oConditionModel = this.getModel("cm");
			var iIndex = oConditionModel.indexOf(this.sFieldPath, oCondition);
		...
		updateDefineConditions: function() {
			var aConditions = this.oConditionModel.getConditions(this.sFieldPath).filter(function(oCondition) {
		...
		onChange: function(oEvent) {
			var oConditionModel = this.getModel("cm");
			oConditionModel._checkIsEmpty(oConditionModel.getConditions(this.sFieldPath));
			oConditionModel._updateValues(oConditionModel.getConditions(this.sFieldPath));
