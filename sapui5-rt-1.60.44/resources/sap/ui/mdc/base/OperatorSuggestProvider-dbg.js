...
//############  TODO #################
//
//The OperatorSuggestProvider is still using the Suggest feature of the Input control.
//This should be changed and the FieldHelp/Table concepts of the Field and FilterField should be used instead.
// 
	...
	function(BaseObject, Log) {
		...
		OperatorSuggestProvider.prototype.associateFilterField = function(oFilterField) {
			...
			if (!(this._oInput instanceof sap.m.MultiInput)) {
				Log.error("mdc:OperatorSuggestProvider", "associateFilterField for " + (this._oInput ? this._oInput.getId() : "null") + " not possible!");
				return;
			}
