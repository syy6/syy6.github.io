	...
	function (EventProvider, ModelUtil, DraftUtil, Queue, Log) {
		...
		BaseController.prototype._checkImplicitError = function (oResponse, mParameters) {
			...
			if (mParameters.pendingChanges && bCounts) {
				...
				// There are cases where looking at the struct __batchResponses is not enough but
				// digging one level deeper seems to be right: go for __changeResponses then
				if (oPart && !oPart.response && oPart.__changeResponses && (oPart.__changeResponses.length === 1) ) {
					oPart = oPart.__changeResponses[0];
				}
