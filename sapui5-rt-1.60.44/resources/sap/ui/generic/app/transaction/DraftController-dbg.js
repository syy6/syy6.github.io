...
sap.ui.define(["sap/ui/thirdparty/jquery", "./BaseController", "./DraftContext", "sap/base/Log"], function(jQuery, BaseController, DraftContext, Log) {
	 ...
	 * 
	 * Starting with version 1.56 an activation is always done in a separate Change Group and a separate "Prepare" is fired before.
	 *
	 * @param {sap.ui.model.Context} oContext The given binding context
	 * @param {map} mParameters Parameters to control the behavior
	...
	DraftController.prototype.activateDraft = function(oContext, mParameters) {
		...
		var mPrepareParameters = jQuery.extend({}, mParameters);

		var pPrepare = this.prepareDraft(oContext, mPrepareParameters);

		mParameters["batchGroupId"] = "Changes";
		mParameters["changeSetId"] = "Activation";
		var oImport = this.getDraftContext().getODataDraftFunctionImportName(oContext, "ActivationAction");
		var pActivate = this._callAction(oImport, oContext, mParameters);
	...
	DraftController.prototype.activateDraftEntity = function(oContext) {
		...
		oPromise = this.activateDraft(oContext, mParameters).then(function(aResponses) {
			var bIsActiveEntity, oResponseEntity, oResponseOut;
