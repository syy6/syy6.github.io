...
sap.ui.define([
	 ...
	 *        The <code>NavigationHandler</code> supports the verification of sensitive information. All filter names that are part of
	 *        <code>selectionVariant</code> and <code>valueTexts</code> will be verified if they are annotated as
	 *        <code>com.sap.vocabularies.PersonalData.v1.IsPotentiallySensitive</code> and will then be removed before the data is persisted in the
	 *        app state. To verify the sensitive information, the <code>NavigationHandler</code> requires an unnamed model of type
	...
	var NavigationHandler = BaseObject.extend("sap.ui.generic.app.navigation.service.NavigationHandler", /** @lends sap.ui.generic.app.navigation.service.NavigationHandler */
	{
		 ...
		 * <code>sap-ushell-next-navmode</code> is taken into account. If set to <code>explace</code> the inner app state will note be changed.
		 ...
		 *        provided the selectionVariant will be cosntructed based on the vNavigationParameters.
		 * @public <br>
		 ...
		 * </code>
		 */
		navigate: function(sSemanticObject, sActionName, vNavigationParameters, oInnerAppData, fnOnError, oExternalAppData) {

			var sSelectionVariant, mParameters, oXAppDataObj, oComponentData, oStartupParameters, bExPlace = false;
			...
			if (typeof vNavigationParameters === "string") {
				...
				mParameters = oTmpData.selectionVariant ? this._getURLParametersFromSelectionVariant(new SelectionVariant(oTmpData.selectionVariant)) : {};
		 ...
		 *          navigation parameters are present (<b>Note:</b> If no navigation parameters are available, bNavSelVarHasDefaultsOnly is<code>true</code>,
		 *          even also no defaulted parameters may be available)</li>
		 *          </ul>
		 ...
		 *          executed; Please note that the appStateKey may be undefined or empty. if an error occurs, an object of type
...
// }
		...
		_removeSensitiveProperties: function(aSensitiveFilterName, aSensitiveParameterName, oData) {

			if (aSensitiveFilterName.length && oData && (oData.selectionVariant || oData.valueTexts)) {
				aSensitiveFilterName.forEach(function(sName) {
			...
			if (aSensitiveParameterName.length && oData && oData.selectionVariant && oData.selectionVariant.Parameters) {
				aSensitiveParameterName.forEach(function(sName) {
		...
		_isPotentiallySensitive: function(oProperty) {
			...
			return (!!oProperty["com.sap.vocabularies.PersonalData.v1.IsPotentiallySensitive"] && fIsTermDefaultTrue(oProperty["com.sap.vocabularies.PersonalData.v1.IsPotentiallySensitive"]));
		},
		 ...
		 * data. Note: This method expects that the metadata are already loaded!
		 * @param {object} oData with potential sensitive information (as oData the oSelectionVariant.toJSONObject() external representation is expected, which has a ParameterContextUrl property)
		 * @return {object} data without sensitive information or <code>null</code> if the odata metadata is not yet fully loaded
		 * @private
		 */
				...
				if (this.oComponent && oModel && oModel.isA("sap.ui.model.odata.v2.ODataModel")) {
					...
					if (oModel.getServiceMetadata() && oMetaModel && oMetaModel.oModel) {
							...
							if (oEntityDef) {
								if (oEntityDef && oEntityDef.property) {
									for (i = 0; i < oEntityDef.property.length; i++) {
										if (this._isPotentiallySensitive(oEntityDef.property[i])) {
										...
										if (oEndRole.multiplicity === "1" || oEndRole.multiplicity === "0..1") {
											...
											if (oSubEntityDef && oSubEntityDef.property) {
												for (var j = 0; j < oSubEntityDef.property.length; j++) {
													if (this._isPotentiallySensitive(oSubEntityDef.property[j])) {
						...
						if (aSensitiveFilterName.length || aSensitiveParameterName.length) {
							...
							this._removeSensitiveProperties(aSensitiveFilterName, aSensitiveParameterName, oAdaptedData);
						}
