...
sap.ui.define(["jquery.sap.global", "sap/ovp/cards/charts/Utils", "sap/ui/model/odata/CountMode",
    ...
    function (jQuery, Utils, CountMode, CardAnnotationHelper, NumberFormat, DateFormat,
              ...
              MeasureDefinition, FeedItem, CoreElement, OvpResources) {
        "use strict";
        ...
        function formatItems(iContext, oEntitySet, oSelectionVariant, oPresentationVariant, oDimensions, oMeasures, chartType) {
            ...
            if (measuresList.length > 0) {
                jQuery.each(measuresList, function(i,m){
                    oSelectItems[m] = true;
                });
            }
		    ...
		    if (sorterList.length > 0) {
                var modSortList = [];
                jQuery.each(sorterList, function(i,s){
        ...
        function checkForecastMeasure(aMeasure, entityTypeObject) {
            ...
            if (boolflag === true) {
                ...
                jQuery.sap.log.warning(errorMessages.CARD_WARNING + errorMessages.INVALID_FORECAST);
                return undefined;
            }
        }
        ...
        function checkTargetMeasure(aMeasure, entityTypeObject) {
            ...
            if (boolflag === true) {
                ...
                jQuery.sap.log.warning(errorMessages.CARD_WARNING + errorMessages.INVALID_TARGET);
                return undefined;
            }
        }
                ...
                jQuery.each(feeds, function (i, v) {
                    if ((v.getUid() === 'valueAxis') || (v.getUid() === 'categoryAxis')) {
                        if (v.getValues().length != 1) {
                            boolFlag = false;
                            jQuery.sap.log.warning(errorMessages.CARD_WARNING + errorMessages.INVALID_DIMMEAS);
                            return false;
                        }
                    }
                });
...
//										showScale: false,
        ...
        function checkNoData(oEvent, cardContainer, vizFrame) {
            /*	var self = sap.ovp.cards.charts.VizAnnotationManager;
             var data, noDataDiv;
             if (!cardContainer) {
             jQuery.sap.log.error(errorMessages.CARD_ERROR + errorMessages.CARD_CONTAINER_ERROR +
             "(" + vizFrame.getId() + ")");
             return;
             }
             ...
             jQuery.sap.log.error(errorMessages.CARD_ERROR + errorMessages.DATA_UNAVAIALABLE  +
             "(" + vizFrame.getId() + ")");
        ...
        function checkRolesForProperty(queue, config, type) {
            ...
            jQuery.each(queue, function (i, val) {
                if (!val || !val[feedtype] || !val[feedtype].PropertyPath) {
                    jQuery.sap.log.error(errorMessages.INVALID_CHART_ANNO);
                    return false;
                }
            ...
            jQuery.sap.log.error(errorMessages.CARD_ERROR + errorMessages.INVALID_REDUNDANT +
                    ...
                    if (oDatapoint && oDatapoint.CriticalityCalculation &&
                        ...
                        if (endsWith(sImproveDirection, "Minimize") ||
                            ...
                            if (toleranceHigh && deviationHigh) {
                                ...
                                jQuery.sap.log.warning(errorMessages.CARD_WARNING + errorMessages.INVALID_CRITICALITY);
                            }
                        ...
                        jQuery.sap.log.warning(errorMessages.CARD_WARNING + errorMessages.INVALID_IMPROVEMENT_DIR);
                    }
                }
            });
            if (boolFlag === true && aMeasures.length > 1) {
                jQuery.sap.log.warning(errorMessages.CARD_WARNING + errorMessages.INVALID_SEMANTIC_MEASURES);
            }
            return boolFlag;
        }
        ...
        function fractionalDigitsExists(val, entityTypeObject) {
            if (val.DataPoint && val.DataPoint.AnnotationPath) {
                ...
                if (oDataPoint && oDataPoint.ValueFormat && oDataPoint.ValueFormat.NumberOfFractionalDigits
                    && parseInt(oDataPoint.ValueFormat.NumberOfFractionalDigits.Int, 10)) {
                    return true;
                } 
...
//		}
            ...
            if (!bEnableStableColors && (chartContext.ChartType.EnumMember === constants.COLUMNSTACKED_CHARTTYPE || chartContext.ChartType.EnumMember === constants.DONUT_CHARTTYPE || chartContext.ChartType.EnumMember === constants.COLUMN_CHARTTYPE ||
                    ...
                    aColorPalette && aColorPalette.length >= 2 && aColorPalette.length <= 4) {
...
//		var minFractionDigits = Number(dataContext.NumberFormat.NumberOfFractionalDigits.Int);
            ...
            */
                ...
                if (feed.type) {
                    ...
                    if (!feed.role) {
                        ...
                        for (var j = 0; j < len && aFeedProperties.length < max; ++j) {
                            ...
                            if (!isEDMINT32) {
                                isEDMINT32 = checkEDMINT32Exists(oMetadata, val, feedtype);
                            }

                            if (!isEDMINT64) {
                                isEDMINT64 = checkEDMINT64Exists(oMetadata, val, feedtype);
                            }
                            ...
                            if (!isFractionalDigitsSet) {
                                isFractionalDigitsSet = fractionalDigitsExists(val, entityTypeObject);
                            }
                        ...
                        if (aFeedProperties.length < min) {
                            jQuery.sap.log.error(errorMessages.CARD_ERROR + errorMessages.MIN_FEEDS + chartType +
                    ...
                    if (aFeedProperties.length) {
                        ...
                        if (!(dataset = vizFrame.getDataset())) {
                            jQuery.sap.log.error(errorMessages.NO_DATASET);
                            return false;
                        }
            ...
            if (isCurrency && isNotCurrency) {
                jQuery.sap.log.warning(OvpResources.getText("Currency_non_currency_measure"));
            }

            if ((isEDMINT32 || isEDMINT64) && !isFractionalDigitsSet) {
                minValCurr = minValue = -1;
                if (oVizProperties && (oVizProperties["valueAxis"] || oVizProperties["valueAxis2"])) {
                    if (oVizProperties["valueAxis"]) {
                        if ( oVizProperties["valueAxis"].label) {
                            oVizProperties["valueAxis"].label.allowDecimals = false;
                        } else {
                            oVizProperties["valueAxis"].label = {
                                allowDecimals: false
                            };
                        }
                    } else {
                    ...
                    if (oVizProperties["valueAxis2"]) {
                        if ( oVizProperties["valueAxis2"].label) {
                            oVizProperties["valueAxis2"].label.allowDecimals = false;
                        } else {
                            oVizProperties["valueAxis2"].label = {
                                allowDecimals: false
                            };
                        }
                    } else {
            ...
            } else if (!(isEDMINT32 || isEDMINT64) && !isFractionalDigitsSet) {
                minValCurr = minValue = -1;
            }
        ...
        function setChartUoMTitle(vizFrame, vizData, chartTitle) {
            ...
            if (result) {
                jQuery.each(measureList, function (i, property) {
                    var feedName = "";
                    if (oMetadata && oMetadata[property]) {
                        if (oMetadata[property][constants.LABEL_KEY_V4]) { //as part of supporting V4 annotation
         ...
         function getLabelFromAnnotationPath(feedName, vizFrame, oMetadata){
            ...
            if (feedName.indexOf("{") === 0) {
                ...
                try {
                    ...
                    jQuery.sap.log.error("Unable to read labels from resource file",err);
                }
            }
            return feedName;
         }
            ...
            vizFrame.attachSelectData(function (oEvent) {
                ...
                if (oEvent.getParameter("data") && oEvent.getParameter("data")[0] && oEvent.getParameter("data")[0].data) {
                    ...
                    if (allData[contextNumber].$isOthers && allData[contextNumber].$isOthers === true) {
                        ...
                        if (finalDimensions){
                            jQuery.each(Object.keys(finalDimensions), function(i, key){
                        ...
                        if (aCustomContext){
                            jQuery.each(Object.keys(aCustomContext), function(i, key){
                                if (oMetadata[key]["type"] === "Edm.String"){
                                    if (oMetadata[key]["sap:semantics"] === "yearmonth" || oMetadata[key]["com.sap.vocabularies.Common.v1.IsCalendarYearMonth"]){
