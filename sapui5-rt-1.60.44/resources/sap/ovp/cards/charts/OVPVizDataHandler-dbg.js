sap.ui.define(["sap/ui/core/Control", "sap/ui/model/json/JSONModel", "jquery.sap.global", "sap/ovp/cards/charts/Utils", "sap/ovp/app/resources"],

    function (Control, JSONModel, jQuery, Utils, OvpResources) {
        "use strict";
...
//			var entityset = this.getEntitySet();
                ...
                if (bData && bData.results) {
                    ...
                    var aColorPalette = oCardModel && oCardModel.getProperty("/colorPalette");
                    var oChartAnnotation = entityType[oCardModel && oCardModel.getProperty("/chartAnnotationPath")];
                    //Only one dimension allowed for non semantic coloring in donut chart
                    if (oChartAnnotation.DimensionAttributes.length === 1 && bEnableStableColors && content.getVizType() === "donut" && aColorPalette && aColorPalette.length <= 10){
                        ...
                        if (content && oVizProps && oVizProps.plotArea){
                            ...
                            if (oDimension){
                                ...
                                var getColorRule = function(i,k, colorObject){
                                    return {
                                            ...
                                            properties: {
                                                color: aColorValues[i]
                                            },
                                            "displayName": oDataClone.results[k][sDisplayProperty]
                                    };
                                };

                                jQuery.each(aColorPalette, function(i, colorObject){
                                ...
                                if (aggregateObject){
                                    oVizProps.plotArea.dataPointStyle['rules'].push({
                                        ...
                                        properties: {
                                            //Provide a color that is from the remainder of the color array
                                            color: aDuplicateColorValues.length ? aColorValues[aColorValues.length - 1] : ""
                                        },
                            ...
                            if (that.getChartType() == "donut" && that.getBinding("aggregateData") !== undefined) {
                                ...
                                if (that.dataSet) {
                                    ...
                                    if (aEntitySetPropKeys && aEntitySetPropKeys.length){
                                        jQuery.each(aEntitySetPropKeys, function(i, property){
