...
sap.ui.define(["jquery.sap.global"],
    function (jQuery) {
        ...
        oUtils.getConfig = function (oChartType) {
            ...
            if (!this.oFullConfig) {
                try {
                    this.oFullConfig = jQuery.sap.loadResource({
                        url: jQuery.sap.getModulePath("sap.ovp.cards.charts") + "/config.json",
                        dataType: "json",
                        async: false
                    });
                } catch (err) {
                    jQuery.sap.log.error(errorMessages.CONFIG_LOAD_ERROR + err);
                }
            }
            ...
            try {
                ...
                jQuery.sap.log.error(errorMessages.CARD_ERROR + errorMessages.INVALID_CHARTTYPE + errorMessages.ANNO_REF);
                return {};
            }

            //Read the configuration for the particular chart type
            if (!this.oFullConfig[sChartType]) {
                jQuery.sap.log.error(errorMessages.INVALID_CONFIG + sChartType + " " + errorMessages.CONFIG_JSON);
                return {};
            }
        ...
        oUtils.cacheODataMetadata = function (oModel) {
            if (!oModel) {
                jQuery.sap.log.error(errorMessages.CARD_ERROR + errorMessages.CACHING_ERROR);
            ...
            if (!oEntityPropertyMap) {
                ...
                jQuery.each(oEntityContainer.entitySet, function (nEntityIndex, oEntitySet) {
                    oEntityType = oMetaModel.getODataEntityType(oEntitySet.entityType);
                    oPropertyMap = {};

                    jQuery.each(oEntityType.property, function (nPropertyIndex, oProperty) {
