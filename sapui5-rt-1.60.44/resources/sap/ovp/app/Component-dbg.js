sap.ui.define(["sap/ui/core/UIComponent", "sap/ovp/cards/rta/SettingsDialogConstants",
        "sap/ui/core/routing/Router", "jquery.sap.global",
        "sap/ui/model/json/JSONModel", "sap/ovp/ui/DashboardLayoutUtil", "sap/ui/core/mvc/ViewType",
        "sap/ui/model/resource/ResourceModel", "sap/ovp/app/resources", "sap/ui/rta/RuntimeAuthoring"/*Don't Remove this. Important for Personalization to work*/],

    function (UIComponent, SettingsDialogConstants, Router, jQuery,
              JSONModel, DashboardLayoutUtil, ViewType, ResourceModel, OvpResources) {
        "use strict";
        ...
        return UIComponent.extend("sap.ovp.app.Component", {
            ...
            /**
             *  Returns true if it is a card setting property and false otherwise
             *
             *  @param {string} sPropertyName - Property Name
             *  @returns {boolean} - returns true or false
             *  @private
             */
            _checkIfCardSetting: function (sPropertyName) {
                var aAllCardSettings = SettingsDialogConstants.allCardSettings,
                    bFlag = false;
                /**
                 *  Looping through all the card settings properties
                 */
                for (var i = 0; i < aAllCardSettings.length; i++) {
                    /**
                     *  If it is a card setting property
                     *  Then return true
                     */
                    if (aAllCardSettings[i] === sPropertyName) {
                        bFlag = true;
                        break;
                    }
                }
            ...
            _isEquivalent: function (oObject1, oObject2) {
                var val, prop, bFlag = true;

                if ((!oObject1 && oObject2) || (oObject1 && !oObject2)) {
                    return false;
                }

                for (prop in oObject1) {
                    if (oObject1.hasOwnProperty(prop)) {
                        ...
                        if (typeof val == "object") { // this also applies to arrays or null!
                             ...
                             */
                            bFlag = bFlag && oObject2.hasOwnProperty(prop) && ((oObject2[prop] == val) || !this._checkIfCardSetting(prop));
                        }
                    }
                }

                for (prop in oObject2) {
                    if (oObject2.hasOwnProperty(prop)) {
                        val = oObject2[prop];
                        if (typeof val == "object") { // this also applies to arrays or null!
                            bFlag = bFlag && this._isEquivalent(val, oObject1[prop]);
                        } else {
                            /**
                             *  Assumption that Vendor/App developer does not
                             *  delivery a card with just text changes in manifest
                             */
                            bFlag = bFlag && oObject1.hasOwnProperty(prop) && ((oObject1[prop] == val) || !this._checkIfCardSetting(prop));
                        }
                    }
                }
