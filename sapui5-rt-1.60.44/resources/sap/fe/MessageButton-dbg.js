...
sap.ui.define(
	[
		'sap/m/Button',
		'sap/m/ButtonType',
		'sap/fe/MessagePopover',
		'sap/ui/core/MessageType',
		'sap/ui/model/Filter',
		'sap/ui/model/FilterOperator'
	],
	function (Button, ButtonType, MessagePopover, MessageType, Filter, FilterOperator) {
		'use strict';
		var MessageButton = Button.extend('sap/fe/MessageButton', {
			metadata: {
				properties: {},
				events: {},
				aggregations: {
					customFilters: {
						type: 'sap.fe.MessageFilter',
						multiple: true,
		...
		function _applyFilters() {
			var aCustomFilters, oValidationFilters, oValidationAndContextFilter, oFilters, oBindingContext, sPath, sViewId,
				aUserDefinedFilter = [];
			//Filter function to verify if the control is part of the current view or not
			function getCheckControlInViewFilter() {
				var fnTest = function (sControlId) {
					if (!sControlId) {
						return false;
					}
			...
			oFilters = new Filter({
				filters: [aUserDefinedFilter, oValidationAndContextFilter],
				and: true
			});
		...
		MessageButton.prototype.init = function () {
			...
			this.oItemBinding.attachChange(_setMessageBtnData.bind(this));
		...
		MessageButton.prototype._applyGrouping = function () {
			...
			this.sGeneralGroupText = this.sGeneralGroupText ? this.sGeneralGroupText : sap.ui.getCore().getLibraryResourceBundle('sap.fe').getText('SAPFE_MESSAGE_GROUP_GENERAL');
			...
			if (!this.oObjectPageLayout) {
				...
				while (oElement && !(oElement instanceof sap.uxap.ObjectPageLayout)) {
			...
			if (aSections) {
				for (i = aMessages.length - 1; i >= 0; --i) { // Loop over all messages
					message = aMessages[i];
					//If the group name is already there then skip for that message
					if (message.getGroupName()) {
						continue;
					}
					for (j = aSections.length - 1; j >= 0; --j) { // Loop over all sections
						section = aSections[j];
						aSubSections = section.getSubSections();
						for (k = aSubSections.length - 1; k >= 0; --k) { // Loop over all sub-sections
							...
							aElements = aAllElements.filter(fnFilterUponId.bind(this, message.getControlId()));
			...
			MessageButton.prototype._getViewId = function (sControlId) {
				var sViewId, oControl = sap.ui.getCore().byId(sControlId);
				while (oControl) {
