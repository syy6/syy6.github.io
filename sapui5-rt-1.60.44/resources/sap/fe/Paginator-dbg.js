...
sap.ui.define([
	...
	"use strict";
	var Paginator = XMLComposite.extend("sap.fe.Paginator", {
		metadata: {
			designTime: true,
			properties: {
				listBinding: {
		...
		alias: "this",
		fragment: "sap.fe.controls._Paginator.Paginator"
	});
	...
	Paginator.prototype.init = function() {
		...
		// i18n for tooltip
		that.setModel(new ResourceModel({
	...
	Paginator.prototype.updateCurrentIndexAndButtonEnablement = function() {
		...
		if (that.getBindingContext() && oListBinding) {
			...
			if ((!that._currentIndex && (that._currentIndex !== 0)) || !oCurrentIndexContext || (that.getBindingContext().getPath() !== oCurrentIndexContext.getPath())) {
	...
	Paginator.prototype.updateCurrentIndex = function() {
		...
		that._currentIndex = that.getListBinding().getCurrentContexts().findIndex(function(oContext) {
