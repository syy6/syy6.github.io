...
sap.ui.define([
    ...
    "sap/ui/Device"
], function (Parameters, URI, fnClone, oType, Device) {
    "use strict";

    /*global dispatchEvent, document, jQuery, localStorage, sap, clearTimeout, setTimeout WeakMap */
    var utils = {};
    ...
    /**
     * allows to safely set a performance mark via window.performance api
     * supported by all browsers and evaluated by performance test tools
     *
     * @param {string} sMarkName the name of the performance mark
     *
     * @param {object} oConfigMarks a configuration object to select the correct
     *      mark in case of several measurements for the same id
     *
     * @param {boolean} oConfigMarks.bUseUniqueMark
     *                  Use only one measurement per mark
     *
     * @param {boolean} oConfigMarks.bUseLastMark
     *                  Only used if bUseUniqueMArk is true.
     *      If true, use the _last_ measurement for a given
     *      mark, if falsy use the first.
     */
    ...
    /**
     * allows to safely set a performance measure via window.performance api
     * supported by all browsers and evaluated by performance test tools
     *
     * @param {string} sMarkName the name of the performance mark
     */

    utils.setPerformanceMeasure = function (sMeasureName) {
        if (performance && performance.measure) {
            performance.measure(sMeasureName);
        }
    };
     ...
     *
     * @param {function | object[]} vTestCondition
     ...
     *
     * @return {boolean}
     *     whether NWBC can accept the navigation mode parameter
     */
    ...
    /**
     * Method to determine whether the given application type is to be embedded
     * in an iframe (like gui or WDA applications).
     *
     * @param {string} sApplicationType
     *   the type of the application
     *
     * @returns {boolean}
     *   whether the ApplicationType is to be rendered embedded into an iframe
     *   and should be able to communicate via postMessage like GUI or WDA
     *   applications.
     *
     * @private
     */
    ...
    utils.isNotificationsEnabled = function () {
        var oConfig = window["sap-ushell-config"],
            bNotificationsEnabled = false;

        bNotificationsEnabled = jQuery.sap.getObject(
            "services.Notifications.config.enabled",
            undefined,
            oConfig);

        return (bNotificationsEnabled && bNotificationsEnabled === true);
    };

    /**
     * Generates a key to store or retrieve an item from the storage
     * (localStorage or sessionStorage).
     * <p>
     * This key allows to reach the information in the local storage
     * starting from the given ids and prefix. This key should not be parsed to
     * detect prefix and ids.
     *
     * @param {string} sPrefix
     *    the key prefix, this prefix may contain #, @, $ characters.
     *
     * @param {array} aIds
     *    a hierarchy of ids that identify the item to be stored or loaded
     *    in/from the storage. Please provide at least one item in this array
