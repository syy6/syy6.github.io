    ...
    return core.defineClass({
        ...
        loadBusinessObjectDataSources: function () {
            ...
            if (this.serviceXML) {
                ...
                var isInternalQueryString = "Schema[Namespace=ESH_SEARCH_SRV]>EntityType[Name=DataSource]>Property[Name=IsInternal]";
                elements = this.serviceXML.querySelectorAll(isInternalQueryString);
                if (elements.length > 0) {
                    // add isInternal filter in data sources request
                    requestUrlTemplate = requestUrlTemplate + " and IsInternal eq false";
                }

                var isClientServiceNameSupported = "Schema[Namespace=ESH_SEARCH_SRV]>EntityType[Name=SearchOptions]>Property[Name=ClientServiceName]";
                elements = this.serviceXML.querySelectorAll(isClientServiceNameSupported);
                if (elements.length === 0) {
                    // remove ClientServiceName, if it is not supported
                    delete ajaxTemplates.searchRequest.d.QueryOptions.ClientServiceName;
                    delete ajaxTemplates.nlqSearchRequest.d.QueryOptions.ClientServiceName;
                }
            }
            ...
            return this.ajaxClient.getJson(requestUrl).then(function (response) {
                ...
                for (var i = 0; i < dataSourcesData.length; ++i) {
                    ...
                    dataSource.annotations = dataSourceData.Annotations && dataSourceData.Annotations.results || [];
