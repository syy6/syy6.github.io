...
sinaDefine(['../../core/core',
    ...
    return core.defineClass({
        ...
        _initAsync: function (properties) {
            ...
            this.federationType = FederationType[properties.federationType] || FederationType['round-robin'];
            this.multiSina = [];
                ...
                return core.Promise.all(querys).then(function (result) {
                    for (var j = 0; j < result.length; j++) {
                        ...
                        for (var k = 0; k < searchResultSet.items.length; k++) {
                            ...
                            var dataSource = that.providerHelper.createMultiDataSource(resultItem.dataSource.id, resultItem.dataSource);
                            resultItem.dataSource = dataSource;
                            resultItem.sina = that.sina;
                        }
                        that.searchResultSet.items = that.federationMethod[that.federationType].mergeMultiResults(that.searchResultSet.items, searchResultSet.items, j + 1);
            ...
            if (query.filter.dataSource === that.sina.allDataSource) {
                ...
                return core.Promise.all(querys).then(function (result) {
                    ...
                    for (var j = 0; j < result.length; j++) {
                        ...
                        mergedSuggestionResultSet.items = that.federationMethod[that.federationType].mergeMultiResults(mergedSuggestionResultSet.items, suggestionResultSet.items, j + 1);
                    }
