sap.ui.define([], function () {
    ...
    return function JsSearch(options) {
                        ...
                        var compiledRegExps = st.parsed.map(function (s) {
                            ...
                            if (s.type === 'phrase') {
                                ...
                                s.children = s.children.filter( // phrase "a * * b" -> "a * b"
                                    ...
                                    /** @param {number} i */
                                    function (y, i) {
                                        return (i === s.children.length - 1 || s.children[i].wOriginal !== '*' || s.children[i + 1].wOriginal !== '*')
                                    }
                                );
                                s.children = s.children.filter( // phrase "* a b *" -> "a b"
                                    /** @param {Word} y */
                                    /** @param {number} i */
                                    function (y, i) {
                                        return ((i !== 0 && i !== s.children.length - 1) || y.wOriginal !== '*');
                                    }
                                );
                                    ...
                                    mapType(s.children, 'word',
                                        ...
                                        function (word, i, pre, succ) {
                                            if (word.wOriginal !== '*') { // no wildcard
                                                ...
                                                middle += word.wRegExp;
                                            } else { // a stand-alone * wildcard - that gets complicated, because of the potential wordbreaks at letter/digit boundaries
                                                if (lastCharacterOriginal(pre).match(regExps.Alpha_atBegin) && firstCharacterOriginal(succ).match(regExps.Alpha_atBegin)) { //  letter * letter
                                                    middle += '(?:[^a]|[^a].*?[^a])';
        ...
        function calcScoresAndHighlights(searchTermCompiled, testTermsCompiled) {
            ...
            var bitmaps = {};
            var highlighted = {};
            for (var j in testTermsCompiled) {
                bitmaps[j] = highlight.initBitmap(testTermsCompiled[j].normalized);
            }
            for (var i = 0; i < searchTermCompiled.length; i++) {
                ...
                for (var j in testTermsCompiled) {
                    ...
                    for (var k = 0; k < searchTermWord.r.length; k++) {
            ...
            for (j in highlighted) {
                ...
                if (testTermsCompiled[j].useNormalizationMap) {
                    for (var k = 0; k < testTermsCompiled[j].normalizationMap.length; k++) {
        ...
        this.search = function (opt) {
            ...
            var top = parseInt(opt.top); // can be NaN
            var skip = parseInt(opt.skip); // can ba NaN
