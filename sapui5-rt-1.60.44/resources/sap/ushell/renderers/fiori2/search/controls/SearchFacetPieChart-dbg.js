        ...
        renderer: function (oRm, oControl) {
                ...
                lp.prototype = {
                    ...
                    update: function () {
                        ...
                        do {
                            ...
                            var c1, c2, c1v, c2v;
                            if (optimizationDirection === 0) { // direction not determined
                                ...
                                if (c1v < c2v && c2v > 0.5) { // further inwards is better
                                ...
                                } else if (c1v > 0.5 || c.labelWidth <= 0) { // outwards is better, or equal,   if text cannot be placed, try going outwards
                                    optimizationDirection = 1;
                        ...
                        if (this.debug) {
                            this.svgBackground.append("svg:circle")
                                .attr("class", "centroid").attr("cx", c.centroidX).attr("cy", c.centroidY).attr("r", 2).style("fill", "blue");
                        }
                        this.svgBackground.selectAll("line.helper").remove();
                        if (this.debug) {
                            this.svgBackground.append("svg:line")
                            ...
                            this.svgBackground.append("svg:line")
                                ...
                                .attr("y2", y + this.labelHeight / 2);
                            this.svgBackground.append("svg:line")
                                .attr("class", "helper").attr("x1", 0).attr("y1", 0).attr("x2", Math.sin(this.startAngle) * (this.r + this.radiusPadding)).attr("y2", -Math.cos(this.startAngle) * (this.r + this.radiusPadding));
                            this.svgBackground.append("svg:line")
                                .attr("class", "helper").attr("x1", 0).attr("y1", 0).attr("x2", Math.sin(this.endAngle) * (this.r + this.radiusPadding)).attr("y2", -Math.cos(this.endAngle) * (this.r + this.radiusPadding));
                            this.svgBackground.append("svg:line")
                                .attr("class", "helper").attr("x1", Math.sin(this.startAngle) * (this.r + this.radiusPadding)).attr("y1", -Math.cos(this.startAngle) * (this.r + this.radiusPadding)).attr("x2", this.isStartAngleOnRight() ? this.backgroundWidth / 2 : -this.backgroundWidth / 2).attr("y2", -Math.cos(this.startAngle) * (this.r + this.radiusPadding));
                            this.svgBackground.append("svg:line")
                                .attr("class", "helper").attr("x1", Math.sin(this.endAngle) * (this.r + this.radiusPadding)).attr("y1", -Math.cos(this.endAngle) * (this.r + this.radiusPadding)).attr("x2", this.isEndAngleOnRight() ? this.backgroundWidth / 2 : -this.backgroundWidth / 2).attr("y2", -Math.cos(this.endAngle) * (this.r + this.radiusPadding));
                        }
                        if (this.labelWidth < 2 * this.labelPaddingX) {
                            // label too short, it makes no sense to draw
                            this.labelWidth = 0; //this.startLabelWidth;
                        } else if (this.labelWidth < this.startLabelWidth) { // label truncated;
                    ...
                    calc: function (outset, labelWidth, labelHeight) {
                        ...
                        if (this.startAngle - this.endAngle + 2 * Math.PI < 1e-9) {
                            ...
                            if (Math.abs(this.startAngle - this.endAngle % (2 * Math.PI)) > 1e-9 && centroidX > 0 && upperLabelBorder <= 0 && lowerLabelBorder >= 0) { // Label beim "Mund von PacMan" (<
                            ...
                            if (Math.abs(this.startAngle - this.endAngle % (2 * Math.PI)) > 1e-9 && centroidX < 0 && upperLabelBorder <= 0 && lowerLabelBorder >= 0) { // Pacman
                        ...
                        if (this.debug) {
                            this.svgBackground.append("svg:line")
                                .attr("class", "left").attr("y1", -this.backgroundHeight / 2).attr("y2", this.backgroundHeight / 2).attr("x1", leftBorder).attr("x2", leftBorder);
                            this.svgBackground.append("svg:line")
                                .attr("class", "right").attr("y1", -this.backgroundHeight / 2).attr("y2", this.backgroundHeight / 2).attr("x1", rightBorder).attr("x2", rightBorder);
                            this.svgBackground.append("svg:line")
                                .attr("class", "perimeterLeft").attr("y1", -this.backgroundHeight / 2).attr("y2", this.backgroundHeight / 2).attr("x1", perimeterLeftBorder).attr("x2", perimeterLeftBorder);
                            this.svgBackground.append("svg:line")
                                .attr("class", "perimeterRight").attr("y1", -this.backgroundHeight / 2).attr("y2", this.backgroundHeight / 2).attr("x1", perimeterRightBorder).attr("x2", perimeterRightBorder);
                        }
                        ...
                        if (x - labelWidth / 2 < perimeterLeftBorder - 1e-9 || x + labelWidth / 2 > perimeterRightBorder + 1e-9) { // centered
                    ...
                    doesFitHeight: function (border, upper, lower, isLeftLabel, isLeftBorder) {
                        ...
                        } else if (border === 0) { // for "Pacman" slices (<  >)
                            y1 = -this.backgroundHeight / 2;
                            y2 = this.backgroundHeight / 2;
                        } else if (border > 0) {
                            if (!this.isStartAngleOnRight()) {
                                y1 = -this.backgroundHeight / 2;
                            } else if (border <= Math.sin(this.startAngle) * (this.r + this.radiusPadding)) { // Schnittpunkt Kante mit this.startAngle im radialen (oder im horizontalen Teil)?
                                y1 = border / -Math.tan(this.startAngle);
                            }
                            if (!this.isEndAngleOnRight()) {
                                y2 = this.backgroundHeight / 2;
                            } else if (border <= Math.sin(this.endAngle) * (this.r + this.radiusPadding)) { // Schnittpunkt Kante mit this.startAngle im radialen (oder im horizontalen Teil)?
                        ...
                        if (!isLeftBorder) {
                            ...
                            if (this.debug) {
                                this.svgBackground.append("svg:rect")
                                    .attr("class", "rightBorder").attr("x", border - 3).attr("y", y1 - 3).attr("width", 6).attr("height", 6).style("fill", "green");
                                this.svgBackground.append("svg:circle")
                                    .attr("class", "rightBorder").attr("cx", border).attr("cy", y2).attr("this.r", 3).style("fill", "green");
                            }
                        } else {
                            this.svgBackground.selectAll(".leftBorder").remove();
                            if (this.debug) {
                                this.svgBackground.append("svg:rect")
                                    .attr("class", "leftBorder").attr("x", border - 3).attr("y", y1 - 3).attr("width", 6).attr("height", 6).style("fill", "red");
                                this.svgBackground.append("svg:circle")
                                    .attr("class", "leftBorder").attr("cx", border).attr("cy", y2).attr("this.r", 3).style("fill", "red");
                            }
                        }
                    ...
                    calcLeftBorder: function (x, y) {
                        ...
                        if (y <= 0) { // upper half
                            if (y > -Math.cos(this.startAngle) * (this.r + this.radiusPadding)) { // Schnittpunkt Kante mit this.startAngle im radialen (oder im horizontalen Teil)?
                    ...
                    calcRightBorder: function (x, y) {
                        ...
                        if (y >= 0) { // lower half
                            if (-y > Math.cos(this.startAngle) * (this.r + this.radiusPadding)) { // Schnittpunkt Kante mit this.endAngle im radialen (oder im horizontalen Teil)?
                                border = Math.tan(this.startAngle) * -y;
                            }
                        } else if (-y < Math.cos(this.endAngle) * (this.r + this.radiusPadding)) {
                            border = Math.tan(this.endAngle) * -y;
            ...
            PieChart.prototype = {
                ...
                notAnimatedUpdate: function (data) {
                    ...
                    if (!sel.empty()) {
                        sel
                            ...
                            .each(function (d, i) {
                                ...
                                d3.select(this).append("svg:title").text("" + d.data.tooltip);
                            });
                    }
                    // create new arcs
                    ...
                    if (!sel.enter().empty()) {
                        // create new labels
                        var textsG = sel.enter().append("svg:g")
                            .style("opacity", 0);
                        textsG.append("svg:text")
