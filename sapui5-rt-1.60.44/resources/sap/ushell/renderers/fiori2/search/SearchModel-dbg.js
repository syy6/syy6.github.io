/* global jQuery, window, document, $, Promise */
// iteration 0 s
    ...
    sap.ui.define([
        ...
        var SearchModel = sap.ui.model.json.JSONModel.extend("sap.ushell.renderers.fiori2.search.SearchModel", {
            ...
            loadDataSources: function () {
                ...
                // exclude transaction search datasource 
                var dataSourcesGroup = this._excludeTransactionDataSource(dataSources);
                dataSources = dataSourcesGroup.dataSources;
                this.setProperty("/appSearchDataSource", dataSourcesGroup.transactionDataSource);
            ...
            _excludeTransactionDataSource: function (dataSources) {
                var transactionDataSource = null;
                for (var i = 0; i < dataSources.length; i++) {
                    if (dataSources[i].id.endsWith("TRANSACTIONS~")) { // better match tp find
                        transactionDataSource = dataSources[i];
            ...
            _prepareTableResults: function (results) {
                ...
                for (i = 0; i < attributesInAll.length; i++) {
                    if (attributesInAll[i].usage.Detail && attributesInAll[i].type !== "GeoJson" && attributesInAll[i].id.match(/latitude|longitude/i) == null) { // filter out Andre's geo-attributes
                    ...
                    for (j = 0; j < attributesToDisplay.length; j++) {
                        for (k = 0; k < attributesInResult.length; k++) {
                            if (attributesToDisplay[j].id === attributesInResult[k].key) {
                                rows[i].cells[j] = {
                                    value: attributesInResult[k].value
                                };
                            }
                        }
                    }
                ...
                for (i = 0; i < attributesToDisplay.length; i++) {
                    ...
                    if (attributesToDisplay[i].id.match(/latitude|longitude/i) == null) {
                        ...
                        columns.push({
                            ...
                            key: "searchColumnKey" + maxKey,
                            attributeId: attributesToDisplay[i].id,
                            index: maxKey // give index, to set initial visible
                        });
                    }
                }
                ...
                columns.unshift({
                    ...
                    key: "searchColumnKey0",
                    attributeId: "SEARCH_DATASOURCE_AS_ID", // used for export row-column mapping
                    index: 0 // to set initial visible
                });
                ...
                if (rows[0].cells[lastCellIndex].isRelatedApps) {
                    columns.push({
                        ...
                        key: "searchColumnKey" + columns.length,
                        attributeId: "SEARCH_APPS_AS_ID", // used for export row-column mapping
                        index: columns.length // to set initial visible
                    });
                }
                ...
                if (that.getProperty("/appCount") && that.getProperty("/appCount") === 1 && that.getProperty("/count") && that.getProperty("/count") === 1) {
                    ...
                    if (aApps && aApps.length > 0 && aApps[0] && aApps[0].url && that.getProperty('/uiFilter/searchTerm') && aApps[0].tooltip && that.getProperty('/uiFilter/searchTerm').toLowerCase().trim() === aApps[0].tooltip.toLowerCase().trim()) {
                        if (aApps[0].url[0] === '#') {
                            ...
                            SearchHelper.openURL(aApps[0].url, '_blank');
                        }
                    }
                }
            },
