...
sap.ui.define(
    [
    ...
    ], function (UIComponent, RendererExtensions, ToolAreaItem, flpmeasure, Config, EventHub, AppLifeCycle, JSONModel, oSharedComponentUtils, AppConfiguration, resources) {
        "use strict";
        ...
        var Renderer = UIComponent.extend("sap.ushell.renderers.fiori2.Renderer", {
            metadata : {
                ...
                routing: {
                    config: {
                        ...
                        controlAggregation: "centerViewPort"
                    },
                    routes: [{
                        ...
                        name: "appfinder",
                        pattern: ["Shell-appfinder?:hashParameters:&/:innerHash*:", "Shell-appfinder?:hashParameters*:", "Shell-appfinder&/:innerHash*:", "Shell-appfinder"], //TODO must come from configuration
                        target: "appfinder"
                    }],
                    targets: {
                        home: {
            ...
            init: function () {
                ...
                oSharedComponentUtils.initializeAccessKeys(oRouter);
            }
        });
    ...
    Renderer.prototype.createContent = function () {
        ...
        oView.loaded().then(function (oView) {
            // initialize the RendererExtensions after the view is created. This also publishes an external event that indicates
            // that sap.ushell.renderers.fiori2.RendererExtensions can be used.
    ...
    Renderer.prototype.showToolAreaItem = function (sId, bCurrentState, aStates) {
        if (bCurrentState) {
            AppLifeCycle.shellElements().addShellModelForApplications ("toolAreaItems", typeof aIds === "string"? [sId]: sId);
        } else {
            this.oShellModel.addToolAreaItem(sId, false, aStates);
        }
    };
    ...
    Renderer.prototype.showActionButton = function (aIds, bCurrentState, aStates, bIsFirst) {
        ...
        if (aActions.length) {
            if (bCurrentState) {
                AppLifeCycle.shellElements().addShellModelForApplications ("actions", aIds, bIsFirst);
            } else {
                this.oShellModel.addActionButton(aIds, false, aStates, bIsFirst);
            }
        }
    ...
    Renderer.prototype.setShellFooter = function (oParameters) {
        ...
        if (oControlProperties && oControlProperties.id && sap.ui.getCore().byId(oControlProperties.id)) {
            ...
            if (oControlInstance) {
                ...
                this.lastFooterId = oInnerControl.getId(); // eslint-disable-line no-undef
...
/*--------------------------Hide ----------------------------------*/
    ...
    Renderer.prototype.removeToolAreaItem = function (aIds, bCurrentState, aStates) {
        EventHub.once("ToolAreaCreated").do( function () {
            if (typeof aIds === "string") {
    ...
    Renderer.prototype.addUserAction = function (oParameters) {
        ...
        if (controlType) {
            ...
            sap.ui.require([ sControlResource ], function (ControlClass) {
                //change model
                var oOrigShellModel = bCurrentState? AppLifeCycle.shellElements().getStateModelToUpdate(): that.oShellModel.getModelToUpdate();
                bCurrentState? AppLifeCycle.shellElements().setStateModelToUpdate(oModelToUpdate): that.oShellModel.setModelToUpdate(oModelToUpdate, true);
                ...
                bCurrentState? AppLifeCycle.shellElements().setStateModelToUpdate(oOrigShellModel): that.oShellModel.setModelToUpdate(oOrigShellModel, false);
                oDeferred.resolve(oControlInstance);
            });
        } else {
    ...
    Renderer.prototype.addFloatingButton = function (oParameters) {
        ...
        if (oControlProperties && oControlProperties.id && sap.ui.getCore().byId(oControlProperties.id)) {
            ...
            if (oControlInstance) {
                if (bIsVisible) {
                    that.showFloatingActionButton(oItem.getId(), bCurrentState, aStates); // eslint-disable-line no-undef
                    that.oShellModel.addElementToManagedQueue(oControlInstance);
                }
                oDeferred.resolve(oControlInstance);
            }
        ...
        sap.ui.require([sControlResource],
            function (oControlObject) {
                oControlInstance = new oControlObject(oControlProperties);
                if (bIsVisible) {
                    this.showFloatingActionButton(oItem.getId(), bCurrentState, aStates); // eslint-disable-line no-undef
                }
                oDeferred.resolve(oControlInstance);
        });
        return oDeferred.promise();
        ...
        if (controlType) {
            ...
            sap.ui.require([sControlResource],
                function (oControlObject) {
                    ...
                    if (bIsVisible) {
                        that.oShellModel.addElementToManagedQueue(oControlInstance);
                        that.showLeftPaneContent(oItem.getId(), bCurrentState, aStates); // eslint-disable-line no-undef
                    }
                    oDeferred.resolve(oControlInstance);
            });
        } else {
...
/*-------------------general---------------------------*/
    ...
    Renderer.prototype.setHeaderTitle = function (sTitle, controlType, oControlProperties) {
        ...
        // If a control instance is alread created - get it by its id
        if (oControlProperties && oControlProperties.id && sap.ui.getCore().byId(oControlProperties.id)) {
            oControlInstance = sap.ui.getCore().byId(oControlProperties.id);
            this.shellCtrl.setHeaderTitle(sTitle, oControlInstance);
        } else if (controlType) { // If the specific control was not created yet, and a control type was given by the caller
            sControlResource = controlType.replace(/\./g, "/");
            // Verify whether the control type is already loaded
            oControlObject = sap.ui.require(sControlResource);
...
/*------------Custom State Entry------------------------------*/
    ...
    Renderer.prototype.addRightViewPort = function (oView) {
        this.shellCtrl.oViewPortContainer.addRightViewPort(oView, false);
    };

    Renderer.prototype.addLeftViewPort = function (oView) {
        this.shellCtrl.oViewPortContainer.addLeftViewPort(oView, false);
    };
    ...
    Renderer.prototype.getViewPortContainerCurrentState = function () {
        return this.shellCtrl.oViewPortContainer.getCurrentState();
    };

    Renderer.prototype.ViewPortContainerNavTo = function (sName, sTargetName, sAction) {
        return this.shellCtrl.oViewPortContainer.navTo(sName, sTargetName, sAction);
    };
     ...
     */
    Renderer.prototype.switchViewPortStateByControl = function (oOpenByControl, sState) {
        ...
        function fAfterAnimationFinishedCB () {
            ...
            // detach the callback
            that.shellCtrl.oViewPortContainer.detachAfterSwitchStateAnimationFinished(fAfterAnimationFinishedCB);
        }

        // attach the CB for after animations finished
        if (bControlValid) {
            this.ViewPortContainerAttachAfterSwitchStateAnimationFinished(fAfterAnimationFinishedCB);
        }
        // call to switch state
        this.shellCtrl.oViewPortContainer.switchState(sState);
    };

    Renderer.prototype.ViewPortContainerAttachAfterSwitchStateAnimationFinished = function (func) {
        this.shellCtrl.oViewPortContainer.attachAfterSwitchStateAnimationFinished(func);
    };
