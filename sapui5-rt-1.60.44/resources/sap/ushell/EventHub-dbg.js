...
/**
 * @fileOverview provides a sticky EventHub implementation to be used
...
(function () {
    ...
    return fnDefine([], function () {
        ...
        function logCallbackError (oError) {
            ...
            var sDetails = "Data passed to the event were: '" + oError.eventData + "'";
        ...
        /*
         * Find and unsubscribe a subscriber from the hub using its event
        ...
        function createDispatchOperation () {
            ...
            var oDispatchPromise = new Promise (function (fnResolve) {
                fnDispatchComplete = fnResolve;
            });

            var oDispatchOperation = {
        ...
        function dispatchEvent (oHub, sEvent) {
            ...
            var aSubscriberGroups = oHub.subscribers[sEvent];
            var aGroupsDispatchedPromise = aSubscriberGroups.map(function (aSubscriberGroup) {
        ...
        function dispatchEventToGroup (oHub, sEvent, aSubscriberGroup, oDispatchOperation, iStartFrom) {
            ...
            var aSubscriberGroupSlice = aSubscriberGroup.slice(iStartFrom);

            return aSubscriberGroupSlice.reduce(function (oPreviousHandlerCalledPromise, oSubscriber, iIdx) {
        ...
        function notifySubscriber (oHub, sEvent, oSubscriber, aSubscriberGroup) {
            ...
            return new Promise(function (fnResolve) {
                ...
                setTimeout(function () {
                    ...
                    // -- unsubscribe offed subscriber after the call
                    // as it was already called and offed
                    if (bOffAfterCall) {
        ...
        function createFnOff (oHub, sEvent, aSubscriberGroup) {
            /*
             * When off is called, it immediately removes any subscribed
             * listener that was already called.
             */
            return function () {
        ...
        function createFnDo (oHub, sEvent, aSubscriberGroup) {
            ...
            /*
             * Create a new subscriber and add it to the given subscriber
             * group.
             */
            return function (fnCallback) {
                ...
                // The dispatching process picks up and calls this listener if
                // dispatching is ongoing.
                ...
                if (oHub.pendingEvents.hasOwnProperty(sEvent)) {
                    ...
                    if (!oDispatchOperation) {
                        ...
                        /*
                         * This subscriber is not called if during dispatching
                         * if it was added to another subscriber group. This is
                         * the case in situations like:
                         *
                         ...
                         *      ...
                         *
                         * Expectation is that sequence is kept during
                         * dispatching. So we wait on the ongoing dispatching
                         * operation and continue afterwards.
                         */
         ...
         *
         * Internally, pushes a group of subscribers onto the 'subscribers'
         * queue. This group is associated to the "on" call. The group has the
         * following properties:
         *
         * - "off": indicates that #off was called on behalf of the whole
         *   group. Indicating that if a handler was already called it
         ...
         *
         * Event data are updated immediately. When new data enter, any pending
         * dispatching is interrupted and a new (asyncronous) dispatching takes
         * place.
         */
        ...
        function join (oHub /*, args */) {
            ...
            var iCount = 0;
            var aOneTimeCounts = new Array(aArgDoables.length)
                .join(",")
                .split(",")
                .map(function () {
                    return 1;
        ...
        function createChannel (oContract) {
            ...
            var oEventHub = createEventHub(oEventHubData);
            var oContractClone = deserialize(oEventHubData, serialize(oEventHubData, oContract));
                ...
                aPath.reduce(function (oCurrentObject, sKey, iIdx) {
                    ...
                    if (iIdx === aPath.length - 1) {
                        if (!isPrimitive(vValue) && !isPrimitive(oDefSoFar) && Object.keys(oDefSoFar).length > 0) {
                            // Before writing, we need to check that the
                            // structure of the value we want to override
                            // allows it.
                            var sExampleValue;
                            var oRemainingPaths = Object.keys(oDefSoFar).reduce(function (o, sKey) {
                                o[sKey] = true;
                                return o;
                            }, {});
                            var bKeysNotInContractOrNonEmpty = Object.keys(vValue).some(function (sKey) {
                                sExampleValue = sKey;
                            ...
                            var bMissingKeys = Object.keys(oRemainingPaths).length > 0;
                            var bIsOverrideForbidden = bKeysNotInContractOrNonEmpty || bMissingKeys;
            ...
            function channelLast (sPath) {
                var aPath = parsePath(sPath);
                var vValueFromOriginalObject = getObjectValue(oContract, aPath);  // throws if path is invalid

                return getObjectValueOrDefault(oContractClone, aPath, vValueFromOriginalObject);
            }

            function channelOn (sPath) {
                // publish event if we did not publish it before and there is a
                // value in the object.
                var aPath = parsePath(sPath);
                var sSerializedPath = serializePath(aPath);

                // ensure on is called on an existing path
                var vLastChangedValue = oEventHub.last(sSerializedPath);
                var bValueWasChanged = oEventHubData.pendingEvents.hasOwnProperty(sSerializedPath);
                if (bValueWasChanged) {
            ...
            function channelWait (sPath) {
                // return oEventHub.wait(sPath);
                var aPath = parsePath(sPath);
                var aParentValues = getParentValues(oContractClone, aPath);
                var aWaitPromises = findEmittableParentEvents(aParentValues).map(function (oParentEventInfo) {
