sap.ui.require.preload({
	"sap/ushell/components/appfinder/AppFinder.controller.js":function(){// ${copyright}
...
sap.ui.define([
    ...
    CatalogsManager
        ...
        sap.ui.controller("sap.ushell.components.appfinder.AppFinder", {
            onInit: function () {
	...
	"sap/ushell/components/appfinder/AppFinder.view.js":function(){// ${copyright}
...
sap.ui.define(function () {
    ...
    sap.ui.jsview("sap.ushell.components.appfinder.AppFinder", {
            ...
            this.oToolbar.addEventDelegate({
                ...
                onsapskipforward: function (oEvent) {
                    ...
                    if (openCloseSplitAppButton.getVisible() && !openCloseSplitAppButton.getPressed()) {
                        oEvent.preventDefault();
                        sap.ushell.components.homepage.ComponentKeysHandler.setFocusOnCatalogTile();
                    }
                },
        ...
        createSegmentedButtons: function (sMenu) {
            ...
            segmentedButtonsArray = [];
            segmentedButtonsArray.push(new sap.m.SegmentedButtonItem("catalog", {
            ...
            if (this.enableEasyAccessUserMenu) {
                segmentedButtonsArray.push(new sap.m.SegmentedButtonItem('userMenu', {
            ...
            if (this.enableEasyAccessSAPMenu) {
                segmentedButtonsArray.push(new sap.m.SegmentedButtonItem('sapMenu', {
	...
	"sap/ushell/components/appfinder/Catalog.controller.js":function(){// ${copyright}

sap.ui.define([
    "sap/ushell/components/_HomepageManager/PagingManager",
    "sap/ushell/utils/WindowUtils"
], function (PagingManager, WindowUtils) {
    "use strict";
...
//                oModel.setProperty('/catalogsNoDataText', sap.ushell.resources.i18n.getText('loadingTiles'));
            ...
            sap.ui.getCore().getEventBus().subscribe("sap.ushell", "appFinderAfterNavigate", that._handleAppFinderAfterNavigate,this);
        },
        ...
        _decodeUrlFilteringParameters: function (sUrlParameters) {
            ...
            /*if (this.tagFilter) {
              this.tagFilter = window.decodeURIComponent(this.tagFilter);
            }*/
        ...
        onShow: function (oEvent) {
            //if the user goes to the catalog directly (not via the homepage)
      ...
      allocateNextPage : function () {
            ...
            // if view ID does not contain the active menu then return
                ...
                if (bSearchMode || bTagMode || this.resetPage) {
                    ...
                    if (aSelectedTags && aSelectedTags.length > 0) {
                        ...
                        otagFilterWrapper = new sap.ui.model.Filter([otagFilter],true);
                    }
                    ...
                    if (sSearchTerm) {
                        ...
                        oSearchFilterWrapper = new sap.ui.model.Filter(aFilters,false); // false mean OR between the search filters
                    }
                  ...
                  if (otagFilterWrapper && otagFilterWrapper.aFilters.length > 0 && oSearchFilterWrapper){
                    oFilters = new sap.ui.model.Filter([oSearchFilterWrapper].concat([otagFilterWrapper]),true);
                  } else if (otagFilterWrapper && otagFilterWrapper.aFilters.length > 0 ){
                    oFilters = new sap.ui.model.Filter([otagFilterWrapper],true);
                  } else if (oSearchFilterWrapper && oSearchFilterWrapper.aFilters.length > 0 ) {
                    oFilters = new sap.ui.model.Filter([oSearchFilterWrapper],true);
                  }
        ...
        onAppBoxPressed: function (oEvent) {
            ...
            if (fnPressHandler) {
                ...
                if (sUrl && sUrl.indexOf("#") === 0) {
                    ...
                    oRecentEntry.title = oAppBox.getProperty("title");
                    oRecentEntry.appType = "App";
                    oRecentEntry.url = sUrl;
                    oRecentEntry.appId = sUrl;
         ...
         * Shows an appropriate message to the user. Don't need to reload the groups model, because groups update only after success API call.
         *
         * @param {object} oCatalogTileModel - The catalog tile model from /catalogTiles array
         * @param {object} oGroupModel - The model of the relevant group
         * @param {boolean} bTileAdded - Whether the performed action is adding or removing the tile to/from the group
	...
	"sap/ushell/components/appfinder/Catalog.view.js":function(){// ${copyright}
                ...
                onsapskipback: function (oEvent) {
                    try {
                        ...
                        if (openCloseSplitAppButton.getVisible()) {
                            ...
                            sap.ushell.components.homepage.ComponentKeysHandler.appFinderFocusMenuButtons(oEvent);
                        }
                    } catch (e) {
                        // continue regardless of error
                    }
            ...
            this.setCategoryFilterSelection = function (sSelection, shouldFocusOnCategory) {
                var oCatalogSelection = that.getCatalogSelect(),
                    ...
                    if (!sSelected || sSelected == ""){
                      sSelected = sap.ushell.resources.i18n.getText('all');
                    }
...
//                categoryAllocateTiles: "{/categoryAllocateTiles}",
        ...
        _calculateDetailPageId: function () {
            ...
            var bNoCatalogs = this.getModel().getProperty("/catalogsNoDataText") ? true : false;
            var sId;
	...
	"sap/ushell/components/appfinder/Component.js":function(){// ${copyright}
sap.ui.define(
    [
        'sap/ushell/components/CatalogsManager',
	...
	"sap/ushell/components/appfinder/EasyAccess.controller.js":function(){// ${copyright}

sap.ui.define([
    "sap/ui/model/odata/ODataUtils",
    "sap/ui/thirdparty/datajs"
], function (ODataUtils, OData) {
	"use strict";
	...
	"sap/ushell/components/appfinder/EasyAccess.view.js":function(){// ${copyright}
...
sap.ui.define(function() {
	...
	"sap/ushell/components/appfinder/GroupListPopover.controller.js":function(){// ${copyright}
...
*/
	...
	"sap/ushell/components/appfinder/GroupListPopover.view.js":function(){// ${copyright}
...
sap.ui.define([
	...
	"sap/ushell/components/appfinder/HierarchyApps.controller.js":function(){// ${copyright}
...
sap.ui.define(function() {
	...
	"sap/ushell/components/appfinder/HierarchyApps.view.js":function(){// ${copyright}
...
sap.ui.define(['sap/ushell/ui/appfinder/AppBox','sap/ushell/ui/appfinder/PinButton'],
	...
	"sap/ushell/components/appfinder/HierarchyFolders.controller.js":function(){// ${copyright}
...
sap.ui.define(function() {
	...
	"sap/ushell/components/appfinder/HierarchyFolders.view.js":function(){// ${copyright}
