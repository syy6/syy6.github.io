sap.ui.define(["sap/ushell/components/tiles/indicatorTileUtils/smartBusinessUtil"], function(util) {
	"use strict";
 ...
 /*eslint-disable block-scoped-var*/
  ...
  /*eslint-disable no-lonely-if*/
	...
	logError: function(err){
		this.oDualComparisonView.oGenericTile.setState(sap.m.LoadState.Failed);
		this.oDualComparisonView.oGenericTile.setState(sap.m.LoadState.Failed);
	...
	flowWithoutDesignTimeCall: function(){
		...
		if (this.oChip.visible.isVisible() && !this.firstTimeVisible){
			...
			this.fetchKpiValue(function(kpiValue){
				...
				this.oDualComparisonView.oGenericTile.setState(sap.m.LoadState.Loaded);
	...
	onInit:function(){
		...
		this.oDualComparisonView.oGenericTile.setState(sap.m.LoadState.Loading);
		try {
			sap.ushell.components.tiles.indicatorTileUtils.util.getParsedChip(
					this.oChip.configuration.getParameterValueAsString("tileConfiguration"), this.oChip.preview.isEnabled(), function(config){
						...
						if (that.oChip.preview.isEnabled()){
							...
							that.oDualComparisonView.oGenericTile.setState(sap.m.LoadState.Loaded);
						} else {
