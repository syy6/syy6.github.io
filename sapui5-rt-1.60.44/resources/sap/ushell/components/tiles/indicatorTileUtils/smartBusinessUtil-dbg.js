	...
	return {
		...
		getODataModelByServiceUri : function(sServiceUri) {
			...
			if (!cache[sServiceUri]) {
				var oModel = new sap.ui.model.odata.ODataModel(sServiceUri,{loadMetadataAsync: true, json: true});
		...
		isCacheValid: function(chipId, chipLastUpdatedOn, cacheMaxAge, cacheMaxAgeUnit, tilePressed) {
			...
			if (!cachedValue) {
				...
				if (jQuery.type(cachedValue.CachedTime) == "date") {
					var cachedTime = cachedValue.CachedTime && cachedValue.CachedTime.getTime();
				} else {
					var cachedTime = cachedValue.CachedTime && parseInt(cachedValue.CachedTime.substr(6), 10); // 10 redix - http://eslint.org/docs/rules/radix
				}
		...
		scheduleFetchDataJob : function(isVisible) {
			//   if(!this.updateDatajobScheduled) {
			var config;
			...
			var cachingTime =  sap.ushell.components.tiles.indicatorTileUtils.util.getCachingTime(config);
			var cachingTimeUnit = sap.ushell.components.tiles.indicatorTileUtils.util.getCachingTimeUnit(config);
			var cacheMaxAge  = sap.ushell.components.tiles.indicatorTileUtils.util.getMillisecond(cachingTime, timeUnitMap[cachingTimeUnit]);
		 ...
		 * always invoke this function in context of individual tile
		 */
		scheduleTimeStampJob : function(isVisible) {

			//  if(!this.updateTimeStampjobScheduled) {
			...
			if (jQuery.type(this.cacheTime) == "date") {
				...
				var tempCacheTime = new Date(parseInt(this.cacheTime.substr(6),10),13);
			}
		...
		getTimeDifference: function(time) {
			...
			if (time) {
				...
				if (minutes < 5) {
					...
					retObject.timer = Math.round(this.getMillisecond(5 - minutes, "minutes")); //if time difference is between 0 to 5(excluded) Minutes
					...
					retObject.unit = "minutes";
					retObject.timer = Math.round(this.getMillisecond(10 - (minutes % 10), "minutes")); //if time difference is between 5 to 60(excluded) Minutes
				} else if (hours >= 1 && hours < 24) {
					retObject.unit = "hours";
					retObject.time = (hours - (hours % 1));
					retObject.timer = Math.round(this.getMillisecond(1 - (hours % 1), "hours")); //if time difference is between 1 to 24(excluded) Hours
				} else if (days >= 1 && days < 2) {
					retObject.unit = "days";
					retObject.time = (days - (days % 1));
					retObject.timer = Math.round(this.getMillisecond(1 - (days % 1), "days")); //if time difference is between 1 to 2(excluded) days
				} else {
					retObject.unit = "days";
					retObject.time = (days - (days % 1));
					retObject.timer = this.getMillisecond(2, "days"); //if time difference is more than 2(included) days
				}
			} else {
...
//			var cacheODataModel;
//			try {
//			cacheODataModel = this.getODataModelByServiceUri("/tmp/SBCachePOC/odata/sbcache.xsodata");
//			var uri  = "/sbcache?$filter=";
//			jQuery.each(oPageChips, function(index, chipId) {
//			if(index > 0) {
//			uri += " or ";
//			}
//			uri += "(USERID eq '" + userId.toString() +"') and (CHIPID eq '" + chipId.toString() + "')";
//			});
...
//			};
//			} catch (exp) {
//			jQuery.sap.log.error("FrontEnd cache Metadata failed");
//			return null;
//			}
		...
		writeFrontendCacheByChipAndUserId: function(oTileApi, chipId, data, bUpdate, callback) {
            ...
            try {
                var cacheODataModel = this.getODataModelByServiceUri("/tmp/SBCachePOC/odata/sbcache.xsodata");
                var uri  = "/sbcache";
                if(bUpdate) {
                    var entity = "/sbcache(USERID='" +  sap.ushell.Container.getUser().getId() + "',CHIPID='" + chipId + "')";
                    cacheODataModel.update(entity ,data ,null, function(){
                        jQuery.sap.log.info("cache update successFully");
                    },function(){
                        jQuery.sap.log.error("cache update failed");
                    },true);
                } else {
                    cacheODataModel.create('/sbcache',data ,null, function(){
                        jQuery.sap.log.info("cache write successFully");
                    },function(){
                        jQuery.sap.log.error("cache update failed");
                    },true);
                }
            } catch (exp) {
                jQuery.sap.log.error(exp);
            } */
				...
				if (oGroups && oGroups instanceof Array && oGroups.length) {
					...
					if (oChipInstances && oChipInstances instanceof Array && oChipInstances.length) {
						jQuery.each(oChipInstances, function(index, oChipInstance) {
							...
							try {
								if (that.isSmartbusinessChipAndCachable(baseChip)) {
									chipIdCollection.push(baseChip.getId());
								}
							} catch (e) {
							} 
						});
					...
					jQuery.sap.log.info("Group Chip fetch");
			}
				...
				this.getKpiChipsOnPage(chipId, function(chipId, oPageChipCollection) {
					if (oPageChipCollection) {
						...
						if (frontendCacheQuery) {
							...
							var isCached = false;
							var frontendCacheQueryReafRef = frontendModel.read(cacheQueriUri, null, null, true, function(data) {
		...
		setTooltipInTile: function(oControl, tileType ,valueObj){
			...
			oControl && oControl.setTooltip && oControl.setTooltip(toolTip);
		},
		/**
		...
		prepareQueryServiceUri : function( sUri, entitySet, measure, dimension, variants, orderByElements, top) {
			...
			try {
				...
				//raja
//				tempModel.attachMetadataLoaded(function(){});
