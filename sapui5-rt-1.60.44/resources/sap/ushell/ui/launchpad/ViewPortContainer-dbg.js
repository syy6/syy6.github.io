...
sap.ui.define([
        ...
        './ViewPortContainerRenderer'
    ], function (jQuery, Device, Control, library, resources, utils, Config, AccessKeysHandler/*, ViewPortContainerRenderer */) {
    "use strict";
 ...
 */
    ...
    ViewPortContainer.prototype.init = function () {
        ...
        this.sLastCenterViewPortFocussedItemId = null;
        this.fnLastAppKeyHandler = null;
        ...
        Device.media.attachHandler(this._handleSizeChange.bind(this), null, Device.media.RANGESETS.SAP_STANDARD);
        Device.orientation.attachHandler(this._handleSizeChange, this);
    ...
    ViewPortContainer.prototype._popFromViewPortNavigationHistory = function (sViewPortId, oControlToPop) {
        var oNavHistory = this._oViewPortsNavigationHistory[sViewPortId],
            ...
            iIndexOfRemovedControl = aVisitedControls.lastIndexOf(oControlToPop);
    ...
    ViewPortContainer.prototype.addCenterViewPort = function (oControl) {
        ...
        utils.setPerformanceMark("FLP -- ViewPortContainer.addCenterViewPort");
    ...
    ViewPortContainer.prototype.addLeftViewPort = function (oControl, bIsHidden) {
        // Measure MeArea
        jQuery.sap.flpmeasure.end(0, "MeArea,(services + rendering)");
        oControl.toggleStyleClass("hidden", bIsHidden === undefined? true: bIsHidden);
        if (this.domRef) {
    ...
    ViewPortContainer.prototype.addRightViewPort = function (oControl, bIsHidden) {
        oControl.toggleStyleClass("hidden", bIsHidden === undefined? true: bIsHidden);
        if (this.domRef) {
    ...
    ViewPortContainer.prototype.navTo = function (viewPortId, controlId, transitionName, data, oTransitionParameters) {
        ...
        var that = this,
            ...
            sCurrentlyDisplayedControlId = this._getCurrentlyDispalyedControl(viewPortId),
            oHomeComponentContainer = oRenderer.byId("Shell-home-component-container"),
            sHomeComponentContainerId = oHomeComponentContainer && oHomeComponentContainer.getId(),
            bNavFromHome = sCurrentlyDisplayedControlId && (sHomeComponentContainerId === sCurrentlyDisplayedControlId),
            bNavToHome = sHomeComponentContainerId === controlId,
        ...
        if (bNavFromHome) {
            this._saveFocus(document.activeElement);
        }
        ...
        if (!bTargetControlFound) {
            ...
            var fnOnTransitionFinished = function () {
                if (bNavToHome && this.sLastCenterViewPortFocussedItemId) {
                    that._restoreFocus(this.sLastCenterViewPortFocussedItemId);
                }
    ...
    ViewPortContainer.prototype._restoreFocus = function (sControlId) {
    ...
    ViewPortContainer.prototype._saveFocus = function (oActiveElementDom) {

        if (oActiveElementDom) {
            var $activeElement = jQuery(oActiveElementDom);
            var oActiveElementControl = $activeElement.control(0);

            if (oActiveElementControl) {
                this.sLastCenterViewPortFocussedItemId = oActiveElementControl.getId();
    ...
    ViewPortContainer.prototype.switchState = function (sStateName) {
        ...
        if (!this.fnSwitchLeftToCenter) {
            this.fnSwitchLeftToCenter = function (e) {
                ...
                //if there is RTL language, we need to check that click was on the padding part (left side)
                var bChangeToCenter;
                if (this.bIsRTL) {
                    bChangeToCenter = e.pageX < parseInt(window.getComputedStyle(e.currentTarget).paddingLeft, 10);
                } else {
                    bChangeToCenter = e.pageX > parseInt(window.getComputedStyle(e.currentTarget).width, 10);
                }
        ...
        if (sStateName !== 'Center')
            jqLeftWrapper.on('click', this.fnSwitchLeftToCenter);
        else
            jqLeftWrapper.off('click', this.fnSwitchLeftToCenter);
    ...
    ViewPortContainer.prototype._animateNotificationAreaShow = function (sState) {
        var viewPorts = this._getViewPorts(),
            transitionEndCB = function (event) {
                ...
                this._reapplyFocusOnNotificationButton();
    ...
    ViewPortContainer.prototype._reapplyFocusOnNotificationButton = function () {
        // The Notification Button is removed and added again each time it is pressed.
        // This function will ensure, that the focus is reapplied again.
        setTimeout(function () {
            if (jQuery(".sapUiBody")[0] === document.activeElement) {
                jQuery("#NotificationsCountButton").focus();
            }
        }, 50);
    };
