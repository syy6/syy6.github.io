...
sap.ui.define(['sap/ushell/services/AppConfiguration', 'sap/ushell/services/_CrossApplicationNavigation/utils'], function (oAppConfiguration, oUtils) {
    "use strict";
    ...
    function CrossApplicationNavigation (oContainerInterface, sParameters, oServiceConf) {
        ...
        * </pre>
        *
        * Since version 1.56 this API accepts a sap-xapp-state-data parameter
        * that can be used generate a url that can be used to launch and
        * application with certain data, for example:
        *
        * <pre>
        * {
        *   target : { semanticObject : "AnObject", action: "action" },
        *   params : { "sap-xapp-state-data" : { a: "b", c: "d" } }
        * }
        * </pre>
        *
        * Using the arguments as in the example above, a link with a
        * sap-xapp-state parameter that encodes the provided data is returned.
        * The sap-xapp-state-data parameter does not appear in the generated
        * link.
        *
        * @since 1.15.0
        * @public
        ...
        */
        this.hrefForExternal = function (oArgs, oComponent, bAsync) {
            ...
            if (sap.ushell && sap.ushell.Container && typeof sap.ushell.Container.getService === "function" && sap.ushell.Container.getService("ShellNavigation")) {
                ...
                // structure (we don't want to create side effects on the
                // original object).
        ...
        *
        * Since version 1.56 this API accepts a sap-xapp-state-data parameter
        * that can be used to launch and application with certain data, for
        * example:
        *
        * <pre>
        * {
        ...
        * </pre>
        *
        * The data specified via "sap-xapp-state-data" are passed to the target
        * application in the sap-xapp-state parameter.<br />
        *
        * Note that the sap-xapp-state-data parameter itself is not passed to
