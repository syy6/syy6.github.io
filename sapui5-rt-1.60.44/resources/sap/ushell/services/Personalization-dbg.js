 ...
 * @version 1.60.40
 */
sap.ui.define(["sap/ushell/utils", "sap/ushell/services/_Personalization/utils", "sap/ushell/services/_Personalization/constants", "sap/ushell/services/_Personalization/ContextContainer", "sap/ushell/services/_Personalization/WindowAdapter",
    ...
    "sap/ushell/services/_Personalization/VariantSetAdapter", "sap/ushell/services/_Personalization/Variant", "sap/ushell/services/_Personalization/VariantSet", "sap/ushell/services/_Personalization/WindowAdapterContainer", "sap/ui/base/ManagedObject"], function (utils, personalizationUtils, publicConstants, ContextContainer, WindowAdapter, TransientPersonalizer, PersonalizationContainer, Personalizer, VariantSetAdapter, Variant, VariantSet, WindowAdapterContainer, ManagedObject) {
    "use strict";
    /*jslint nomen: true, bitwise: false */
    /*jshint bitwise: false */
    /*global Promise jQuery sap setTimeout clearTimeout window Uint32Array */
    ...
    /**
     * Configures the adapter to store app variants. When app variant storage
     * is enabled, personalization on app variants is handled and stored using
     * a separate adapter.
     *
     * @param {object} oAppVariantStorageConfig
     *    The service configuration for app variant storage.
     *
     * @return {object}
     *    An object like:
     *    <pre>
     ...
     *    }
     *    </pre>
     *    Where <function> returns a promise that resolves with the app variant
     *    adapter or rejects with an error message.
     *
     * @private
     */
    Personalization.prototype._configureAppVariantStorage = function (oAppVariantStorageConfig) {
        ...
        if (!oAppVariantStorageConfig) {
            // default
            oAppVariantStorageConfig = {
                adapter: {
                    module: sDefaultAppVariantAdapter
                }
            };
        }
     ...
     * This key is suitably random depending on the platform, but it is viable to brute force attacks
     * and storages based on it shall not be used for sensitive data
     * @returns {string}
     ...
     *            Seed of Math.random is OS Random Seed
     *
     * Actual randomness of browser random is at most 2^128 -1 (Period length), way less on older platforms
     * which is way below the 36^40 ~ 2^(5.16*40) =~ 2^206 suggested by the key.
     * or 2^196) for 38 characters used in the appstate.
     *
     * To support suitable randomness on platforms
     * not having a crypto api we also mix in a seed value generated from the server
    ...
    */
    ...
    Personalization.prototype._createContainer = function (sContainerKey, oScope, bCreateEmpty, oComponent) {
        ...
        personalizationUtils.loadAdapter(oChosenAdapter).then(function (oLoadedAdapter) {
            ...
            //
            // If an adapter supports returning an initialized container
            // without requiring an subsequent load, it can set the flag
            // supportsGetWithoutSubsequentLoad and the load call will be
            // omitted in case an empty container is required.
            //
            var bSupportsGetWithoutSubsequentLoad =
                oLoadedAdapter && oLoadedAdapter.supportsGetWithoutSubsequentLoad === true;
