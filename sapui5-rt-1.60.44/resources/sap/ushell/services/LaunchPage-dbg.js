...
sap.ui.define([
    ...
    function LaunchPage (oAdapter, oContainerInterface) {
        ...
        /**
         * Same as "getGroups", but filters out all groups that can not
        ...
        this.getGroupsForBookmarks = function () {
            var defGroupDeferred = jQuery.Deferred();

            Promise.all([
                this.getGroups(),
                this.getDefaultGroup()
            ]).then(function (result) {
                var aGroups = result[0],
                    oDefaultGroup = result[1];

                if (aGroups.length > 0) {
                    aGroups = aGroups.filter(function (group) {
                        return (!that.isGroupLocked(group) && that.isGroupVisible(group));
                    })//create the model structure
                        .map(function (group) {
                            return {
                                title: (group === oDefaultGroup && sap.ushell.resources.i18n.getText("my_group")) || that.getGroupTitle(group),
                                object: group
                            };
                        });
                }
                defGroupDeferred.resolve(aGroups);
            }).catch(function (error) {
                jQuery.sap.log.error("getGroupsForBookmarks failed: " + error.message);
            });

            return defGroupDeferred.promise();
        };
         ...
         *     the target group index, in case this parameter is not supplied we assume the
        ...
        this.moveTile = function (oTile, iSourceIndex, iTargetIndex, oSourceGroup, oTargetGroup, sNewTileType) {
            var oPromise = oAdapter.moveTile(oTile, iSourceIndex, iTargetIndex, oSourceGroup, oTargetGroup, sNewTileType);
            oPromise.fail(function () {
