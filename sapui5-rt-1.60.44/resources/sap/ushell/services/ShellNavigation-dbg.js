...
sap.ui.define([
    ...
    function ShellNavigation (oContainerInterface, sParameters, oServiceConfiguration) {
        ...
        /**
         * Allows to detect how was the last hash changed at low levels, before
         * events are emitted by the HashChanger. This is useful to handle data
         * loss cancellation. After the confirmation dialog is cancelled, we
         * need to restore the hash correctly based on the direction and on
         * whether setHash (adds a new history entry) or replaceHash (no
         * history entry) was last used.
         *
         * For example, when the user went backwards via browser back button.
         *
         * NOTE: relies on "hasher" available as a global variable. Records
         * the last hash change mode in the "_lastHashChangeMode" member, which
         * can have values:
         * <ul>
        ...
        this._enableHistoryEntryReplacedDetection = function () {
            ...
            var fnOriginalReplaceHash = hasher.replaceHash;
        ...
        /**
         * Returns true if the history entry was replaced immediately after the
         * last navigation. To be useful, this method should be called
         * immediately after the hash enters the URL but before the target
         * application is finally navigated to.
         *
         * This method should not be used externally. It's reserved uniquely
         * for internal shell consumption and its signature or result might change
         * at any time.
         *
         * @returns {boolean}
         *
         * Whether <code>hasher#replaceHash</code> was called after the last
         * navigation.
         *
         * @protected
         *
        ...
        /**
         * Resets the internal flag used to track whether the last navigation
         * is made via hasher#setHash or hasher#replaceHash. This method should
         * be called after a navigation is successfully made to a target
         * application to avoid returning an inconsistent answer when calling
         * <code>#wasHistoryEntryReplaced</code>. An inconsistent answer might
         * occur when a navigation is made via forward/back button without
         * passing via <code>hasher#replaceHash</code> or
         * <code>hasher#setHash</code>.
         *
         * This method should not be used externally. It's reserved uniquely
         * for internal shell consumption and its signature or result might
         * change at any time.
         *
         * @protected
         */
