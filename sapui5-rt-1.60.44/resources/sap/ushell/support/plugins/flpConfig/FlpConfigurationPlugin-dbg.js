 ...
 *
 *
 * Adds a plugin to the SAPUI5 Diagnsotics tool to show the startup
 * configuration (window["sap-ushell-config"]) of the Fiori launchpad in a tree table
 * seperated by IconTabFilters for the first level properties of the configuration
 * object. Also it provides a search for last level properties.</p>
 *
 * @version 1.60.40
 */

sap.ui.define(['sap/ui/core/support/Plugin', 'sap/ui/model/json/JSONModel', 'sap/m/Button', 'sap/m/IconTabBar', 'sap/m/IconTabFilter', 'sap/m/Label', 'sap/m/Text', 'sap/m/MessageStrip', 'sap/m/FlexBox', 'sap/ui/model/resource/ResourceModel', 'sap/m/SearchField', 'sap/m/MessageBox'], function (Plugin, JSONModel, Button, IconTabBar, IconTabFilter, Label, Text, MessageStrip, FlexBox, ResourceModel, SearchField, MessageBox) {
        "use strict";
        var that = this,
            ol18nModel,
            flpConfigPlugin = Plugin.extend("sap.ushell.support.plugins.flpConfig.FlpConfigurationPlugin", {
                constructor: function (oSupportStub) {
                    ...
                    if (this.runsAsToolPlugin()) {
                        ...
                        //tool window events
                        this._aEventIds = [
                            this.getId() + "ReceiveData"
                        ];
                    } else {
                        //application window events
                        this._aEventIds = [
                            this.getId() + "RequestData"
                        ];
                    }
                }
            });
        ...
        flpConfigPlugin.prototype.onsapUiFlpConfigurationPluginReceiveData = function (oEvent) {
            ...
            sap.ui.getCore().loadLibraries(["sap.ui.table"], { async: true }).then(function () {
                sap.ui.require(["sap/ui/table/TreeTable", "sap/ui/table/Column"], function (TreeTable, Column) {
                    that.getFlpConfigurationPluginUI(null,TreeTable, Column);
                });
            });
        };

        // handles press event for expand button
        // two steps:
        ...
        function _onPressExpand (oEvent) {
            ...
            if (jQuery.isArray(oFlpConfig[sSelectedKey])) {
        ...
        flpConfigPlugin.prototype.onPressSearchNewModel = function (oEvent) {
            ...
            // check if already searched, if so the normal model only contains the filtered data
            // in any case get the full config
            if (!oFlpConfig) {
                oFlpConfig = sap.ui.getCore().byId("sap-ui-support-flpConfigurationPlugin-TabBar").getModel().getData();
            } else {
            ...
            if (sSearchQuery) {
                ...
                Object.keys(oLevelOneHierarchyPaths).forEach(function (sLevelOneProp) {
                    ...
                    if (oFlpConfigSearch[sLevelOneProp] && jQuery.isArray(oFlpConfigSearch[sLevelOneProp][0][sLevelOneProp])
                ...
                Object.keys(oTabBarFilter).forEach(function (sLevelOneProp) {
                    ...
                    if (oFlpConfigSearch[sSelectedKey] && jQuery.isArray(oFlpConfigSearch[sSelectedKey])) {
        ...
        function _getPropertyCount (oCurrentObject, sLevelOneProp, iPropertyCounter) {
            if (jQuery.isArray(oCurrentObject)) {
        ...
        function _createNewConfigJSON (oCurrentObject, sPropertyKey) {
            ...
            return Object.keys(oCurrentObject).map(function (sPropertyName) {
                ...
                if (typeof (oCurrentObject[sPropertyName]) === 'object' && Object.keys(oCurrentObject[sPropertyName]).length !== 0) {
                    ...
                    if (!jQuery.isEmptyObject(oCurrentObject[sPropertyName]) || typeof (oCurrentObject[sPropertyName]) !== 'object') {
        ...
        flpConfigPlugin.prototype.getFlpConfigurationPluginUI = function (foo, TreeTable, Column) {
            ...
            if (!sap.ui.getCore().byId("sap-ui-support-flpConfigurationPlugin-TabBar")) {
                ...
                if (!oFlpConfigNew || jQuery.isEmptyObject(oFlpConfigNew)) {
