...
(function (root, factory) {
    {
        factory((root.pdfjsWebPDFEntitiesViewer = {}), root.pdfjsWebPDFJS);
    }
}(this, function (exports, pdfjsLib) {
    ...
    var PDFEntitiesViewer = (function PDFEntitiesViewerClosure() {
        ...
        PDFEntitiesViewer.prototype = {
            ...
            render: function PDFEntitiesViewer_render(params) {
                //var entities = JSON.parse(params);
                var self = this;
                ...
                for (var k = 0; k < entities.length; k++) {
                    ...
                    entityTypesMap[entity.type] = entityTypesMap[entity.type] || {};
                    entityTypesMap[entity.type].count = (entityTypesMap[entity.type].count || 0) + 1;
                    entityTypesMap[entity.type].entities = entityTypesMap[entity.type].entities || {};
                    entityTypesMap[entity.type].entities[entity.text] = entityTypesMap[entity.type].entities[entity.text] || [];
                    entityTypesMap[entity.type].entities[entity.text].push({
                        'from': entity.from,
                        'to': entity.to
                    });
                }
                ...
                while (queue.length > 0) {
                    ...
                    for (var item in entityTypesMap) {
                        ...
                        var button = document.createElement('button');
                        button.className = 'entityTypeButton';
                        button.tabIndex = 38;
                        button.textContent = item + ': ' + entityTypesMap[item].count;
                        button.value = item;
                        button.onclick = function () {
                            var entity = this.value.toLocaleLowerCase();
                            var elements = document.querySelectorAll('.' + entity);
                            if (this.classList.contains('pressed')) {
                                delete self.selectedEntitiesMap[entity];
                                ...
                                for (var singleEntity in entityTypesMap[this.value].entities) {
                                    var entityItem = document.createElement('button');
                                    entityItem.className = 'entityItem';
                                    entityItem.textContent = singleEntity;
                                    entityItem.tabIndex = -1;
                                    this.parentElement.appendChild(entityItem);
                                }
        ...
        PDFFindController.prototype = {
            ...
            calcFindPhraseMatch: function PDFFindController_calcFindPhraseMatch(
                ...
                while (true) {
                    ...
                    matches.push([matchIdx, queryLen]);
                }
                if (Array.isArray(this.pageMatches[pageIndex])) {
                    ...
                    this.minimizeFindsAtSameLocation(pageIndex);
                } else {
    ...
    var TextLayerBuilder = (function TextLayerBuilderClosure() {
        ...
        TextLayerBuilder.prototype = {
            ...
            render: function TextLayerBuilder_render(timeout) {
                ...
                var offsetCalculate = function() {
                    if (window.sap.es.suvDocument.offCalculatePromise) {
                        return window.sap.es.suvDocument.offCalculatePromise;
                    }
                    window.sap.es.suvDocument.highlightsNoOverlaps = sap.es.removeOverlaps(window.sap.es.suvDocument.highlights);
                    window.sap.es.suvDocument.offCalculatePromise = sap.es.pdf2text.convertHighlights({
                ...
                this.textLayerRenderTask.promise.then(function() {
                    return offsetCalculate();
                }).then(function() {
                    var pdfHighlighter = new PdfHighlighter(window.sap.es.suvDocument.highlightsNoOverlaps,
                        self.textDivs,
                        this.textContent,
                        self.pageNumber);
...
(function (root, factory) {
    ...
    function webViewerInitialized() {
        ...
        var addSAPMsgBundle2WebL10nData = function() {
            ...
            var sapMsgBundleUrl = baseUrl + '/resources/resources_' + mapBrowserLocaleToSAPLocale(PDFJS.locale) + '.properties';
