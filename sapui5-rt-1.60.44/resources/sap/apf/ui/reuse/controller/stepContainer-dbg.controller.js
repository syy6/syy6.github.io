...
sap.ui.define([
	...
	function _setAlternateRepIcon(oController) {
		...
		var alternateTableIcon = new Icon({
	...
	function _setToggleDisplayButton(oController, oChartContainerToolbar) {
		...
		if(chartType !== ("TableRepresentation" || "listView")){
			...
			if (!oSwitchDisplayButton) {
					...
					press: function(oEvent) {
						...
						if(chartType == "PieChart"){
							oRepresentation.chart.setVizProperties({
			                    plotArea: {
			                        dataLabel: {
			                            visible: oEvent.getParameter('pressed'),
			                            type: "percentage"
			                        }
			                    }
			                });
				...
				oSwitchDisplayButton.setPressed(false);
				var oRepresentation = oController.getCurrentRepresentation();
				var chartState = oRepresentation.chart.getVizProperties().plotArea.dataLabel.visible;
				oSwitchDisplayButton.setPressed(chartState);
			}
