...
(function() {
	...
	function _selectRowInTreeTable(oEvent) { //mark the selection in tree table and update the filter values
		...
		_updateFiltersInTreetable(this, currentSelectedItemIndex);
	}
	function _updateFiltersInTreetable(oTreetableInstance, currentSelectedItemIndex) {
		...
		var aUI5ChartHeleprFilterValues = oTreetableInstance.oRepresentationFilterHandler.getFilterValues();
		if (bIsSelected) {
	...
	*/
	sap.apf.ui.representations.treeTable = function(oApi, oParameters) {
		...
		this.requiredFilter = this.oParameters.requiredFilters[0];
		this.oKeyTextLookup = {};
