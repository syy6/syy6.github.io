...
sap.zen.crosstab.rendering.RenderEngine = function (oCrosstab) {
	...
	this.renderDataArea = function () {
		var dataAreaId = oDataArea.getId();
		var oDomDataArea = oDomElementProvider.getElement(oDataArea.getId());
		var oDomDataAreaBody = oDomDataArea;
		var sInnerHtml = "";
		...
		/*
		 * C1: set empty HTML on Table Element
		 * 		oDomDataArea.html("");
		 * C2: Add Table Body to Table Element and set empty HTML on Table Body Element
		 * 		oDomDataArea.html("<tbody></tbody>");
		 * 		oDomDataAreaBody = $(document.getElementById(dataAreaId).firstChild);
		 * 		oDomDataAreaBody.html("");
		 */
		oDomDataArea.html("<tbody></tbody>");
		oDomDataAreaBody = $(document.getElementById(dataAreaId).firstChild);
		oDomDataAreaBody.html("");
		...
		/*
		 * C1: retrieve the inner HTML from the Table Element
		 * 		sInnerHtml = oDomDataArea[0].innerHTML || "";
		 * C2: retrieve the innter HTML from the Table Body Element
		 * 		sInnerHtml = oDomDataAreaBody[0].innerHTML || "";
		 */
		sInnerHtml = oDomDataAreaBody[0].innerHTML || "";
		...
		var iHorizontalOverlap = getHorizontalScrollingOverlap(iRenderStartRow, iRenderRowCnt, iRenderStartCol,
				iRenderColCnt);
		iVerticalOverlap = null; // this magic does not work anymore!
		iHorizontalOverlap = null; // this magic does not work anymore!
		
		if (iVerticalOverlap) {
			removeDataAreaRows(iVerticalOverlap);
			renderRemainingDataAreaRows(iVerticalOverlap, iRenderStartRow, iRenderRowCnt, iRenderStartCol, iRenderColCnt, this);
		} else if (iHorizontalOverlap) {
			removeDataAreaCols(iHorizontalOverlap);
			renderRemainingDataAreaCols(iHorizontalOverlap, iRenderStartRow, iRenderRowCnt, iRenderStartCol, iRenderColCnt, this);
		} else {
			this.renderArea(oDataArea, iRenderStartRow, iRenderRowCnt, iRenderStartCol, iRenderColCnt);
		}
		
		/*
		 * C1: flush the Table Element
		 * 		oRenderManager.flush(oDomDataArea[0]);
		 * C2: flush the Table Body Element
		 * 		oRenderManager.flush(oDomDataAreaBody[0]);
		 */
		oRenderManager.flush(oDomDataAreaBody[0]);
	...
	this.renderColHeaderArea = function (iRenderStartCol, bUseFullContentWidth) {
		var colHeaderAreaId = oColHeaderArea.getId();
		var oDomColHeaderArea = oDomElementProvider.getElement(oColHeaderArea.getId());
		var oDomColHeaderAreaBody = oDomColHeaderArea;
		 ...
		 * C2: Add Table Body to Table Element and set empty HTML on Table Body Element
		 * 		oDomColHeaderArea.html("<tbody></tbody>");
		 * 		oDomColHeaderAreaBody = $(document.getElementById(colHeaderAreaId).firstChild);
		 * 		oDomColHeaderAreaBody.html("");
		 */
		oDomColHeaderArea.html("<tbody></tbody>");
		oDomColHeaderAreaBody = $(document.getElementById(colHeaderAreaId).firstChild);
		oDomColHeaderAreaBody.html("");
		...
		while (checkFunclet(iColOffset, iMaxColOffset, iEffectiveWidth, iAvailableWidth, bHScrolledToEnd)) {
			 ...
			 * C2: retrieve the innter HTML from the Table Body Element
			 * 		sInnerHtml = oDomColHeaderAreaBody[0].innerHTML || "";
			 */
			sInnerHtml = oDomColHeaderAreaBody[0].innerHTML;
			if (!sInnerHtml) {
				// We need to render the initial <tr>s
				/*
				 * C1: prepare Col Header Rows using Table Element
				 * 		prepareColHeaderRows(oDomColHeaderArea);
				 * C2: prepare Col Header Rows using Table Body Element
				 * 		prepareColHeaderRows(oDomColHeaderAreaBody);
				 */
				prepareColHeaderRows(oDomColHeaderAreaBody);
			}
			 ...
			 * C2: flush the Table Body Element
			 * 		oRenderManager.flush(oDomColHeaderAreaBody[0]);
			 */
			oRenderManager.flush(oDomColHeaderAreaBody[0]);
	...
	this.renderRowHeaderArea = function (iStartRow) {
		var rowHeaderAreaId = oRowHeaderArea.getId();
		var oDomRowHeaderArea = oDomElementProvider.getElement(oRowHeaderArea.getId());
		var oDomRowHeaderAreaBody = oDomRowHeaderArea;
		...
		/*
		 * C1: set empty HTML on Table Element
		 * 		oDomRowHeaderArea.html("");
		 * C2: Add Table Body to Table Element and set empty HTML on Table Body Element
		 * 		oDomRowHeaderArea.html("<tbody></tbody>");
		 * 		oDomRowHeaderAreaBody = $(document.getElementById(rowHeaderAreaId).firstChild);
		 * 		oDomRowHeaderAreaBody.html("");
		 */
		oDomRowHeaderArea.html("<tbody></tbody>");
		oDomRowHeaderAreaBody = $(document.getElementById(rowHeaderAreaId).firstChild);
		oDomRowHeaderAreaBody.html("");
		...
		while (checkFunclet(iRowOffset, iMaxRowOffset, iTotalHeight, iCrosstabHeight, bVScrolledToEnd)) {
			...
			/*
			 * C1: retrieve the inner HTML from the Table Element
			 * 		sInnerHtml = oDomRowHeaderArea[0].innerHTML || "";
			 * C2: retrieve the innter HTML from the Table Body Element
			 * 		sInnerHtml = oDomRowHeaderAreaBody[0].innerHTML || "";
			 */
			sInnerHtml = oDomRowHeaderAreaBody[0].innerHTML || "";
			oRenderManager.write(sInnerHtml);
			...
			if (iRowOffset > 0) {
				...
				this.renderArea(oRowHeaderArea, iStartRow + iRowOffset, iRowsToBeRendered, 0, oRowHeaderArea.getColCnt(), "sapzencrosstab-HeaderRow");
			} else {
			 ...
			 * C2: flush the Table Body Element
			 * 		oRenderManager.flush(oDomRowHeaderAreaBody[0]);
			 */
			oRenderManager.flush(oDomRowHeaderAreaBody[0]);
