...
sap.ui.define([
	...
	"./CoordinateUtils",
	"./GanttUtils"
], function(jQuery, Log, KeyCodes, Core, GanttExtension, CoordinateUtils, GanttUtils) {
	"use strict";
	...
	GanttResizeExtension.prototype.renderPlainOutline = function(oElement) {
		...
		if (oDomRef) {
			...
			var sElementPathD = bRelationship ? oElement.getD() : this.determineOutlineByFrame(oDomRef.getBBox());
	 ...
	 */
	GanttResizeExtension.prototype.determineOutlineByFrame = function(oRect) {
		var x1 = oRect.x,
			y1 = oRect.y,
			width = oRect.width,
			height = oRect.height,
	...
	GanttResizeExtension.prototype.renderShapeConnectTrigger = function(oElement, oGroup) {
		...
		var aRects = this.getResizeRects(bShowShapeConnectTrigger);
	...
	GanttResizeExtension.prototype.attachMousedownEvent = function() {
		var $ganttChartSvg = jQuery("#" + this.getGantt().getId() + " svg.sapGanttChartSvg");
	...
	GanttResizeExtension.prototype.getResizeRects = function(bShowConnectIndicator) {
		var frame = this.getFrame(),
			iX = frame.x,
			iY = frame.y,
			iWidth = frame.width,
